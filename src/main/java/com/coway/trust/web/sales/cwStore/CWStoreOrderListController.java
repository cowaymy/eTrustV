...
@Controller
@RequestMapping(value = "/sales/cwStore")
public class CWStoreOrderListController {
	...
	@Resource(name = "cwStoreOrderListService")
	private CWStoreOrderListService cwStoreOrderListService;
	
	@RequestMapping(value = "/cwStoreOrderList.do")
	public String main(@RequestParam Map<String, Object> params, ModelMap model, SessionVO sessionVO) {
		...
		return "sales/cwStore/cwStoreOrderList";
	}
	
	@RequestMapping(value = "/selectCwStoreOrderJsonList", method = RequestMethod.GET)
	public ResponseEntity<List<EgovMap>> selectOrderJsonList(@RequestParam Map<String, Object>params, HttpServletRequest request, ModelMap model, SessionVO sessionVO) {
    	...
    	requestUrl.replace(requestUrl.lastIndexOf("/"), requestUrl.lastIndexOf("/") + 1, "/cwStoreOrderList.do/");
		...
		String[] arrCWStoreId = request.getParameterValues("storeCodeId"); //Coway Store
		...
		if(arrCWStoreId     != null && !CommonUtils.containsEmpty(arrCWStoreId))     params.put("arrCWStoreId", arrCWStoreId);
		...
		if( ! StringUtils.isEmpty(params.get("vaNo"))
			...
			try{
				 ...
				 if(null !=arrayCustId  || arrayCustId.length>0){
					 orderList = cwStoreOrderListService.selectCWStoreOrderList(params);
				 }
	 ...
	 @RequestMapping(value="/cwStoreSalesOrderRaw.do")
		public String orderEKeyInListPop(){
			return "sales/cwStore/cwStoreSalesOrderRawPop";
		}
}
