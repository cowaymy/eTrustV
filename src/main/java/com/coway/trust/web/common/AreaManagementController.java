...
public class AreaManagementController {
	...
	public  ResponseEntity<ReturnMessage>  saveCopyAddressMaster(@RequestBody Map<String, ArrayList<Object>> params, Model model) {
		...
		if (addList.size() > 0) {			
			cnt = areaManagementService.addCopyAddressMaster(addList, loginId);
		}
	...
	public  ResponseEntity<ReturnMessage>  saveCopyOtherAddressMaster(@RequestBody Map<String, ArrayList<Object>> params, Model model) {
		...
		if (addList.size() > 0) {			
			cnt = areaManagementService.addCopyOtherAddressMaster(addList, loginId);
		}
