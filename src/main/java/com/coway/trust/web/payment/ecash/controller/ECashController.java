...
public class ECashController {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ECashController.class);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/selectECashList", method = RequestMethod.GET)
	public ResponseEntity<List<EgovMap>> selectECashList(@ModelAttribute("searchVO") SampleDefaultVO searchVO
				, @RequestParam Map<String, Object> params, ModelMap model) {
		// 조회.
        List<EgovMap> resultList = eCashService.selectECashList(params);
    ...
    public ResponseEntity<ReturnMessage> generateNewEDeduction(@RequestBody Map<String, ArrayList<Object>> params,
        ...
        List<EgovMap> isActiveBatchList = eCashService.selectECashList(searchMap);
        ...
        if(isActiveBatchList.size() > 0){
        	...
        	eCashService.createECashDeduction(eCashDeduction);		        	//프로시저 함수 호출
        	List<EgovMap> resultMapList = (List<EgovMap>)eCashDeduction.get("p1");         	//결과 뿌려보기 : 프로시저에서 p1이란 key값으로 객체를 반환한다.
