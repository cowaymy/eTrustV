...
public class OwnershipTransferServiceImpl implements OwnershipTransferService{
    ...
    public int saveRequest(Map<String, Object> params, SessionVO sessionVO) {
        ...
        dParam.put("rotPV", CommonUtils.isNotEmpty(ordDetailsM.get("totPv")) ? (BigDecimal) ordDetailsM.get("totPv") : BigDecimal.ZERO);
        dParam.put("rotAmt", CommonUtils.isNotEmpty(ordDetailsM.get("totAmt")) ? (BigDecimal) ordDetailsM.get("totAmt") : BigDecimal.ZERO);
        dParam.put("rotPromoID", CommonUtils.intNvl(ordDetailsD.get("promoId")));
        dParam.put("rotDefRentAmt", CommonUtils.isNotEmpty(ordDetailsM.get("defRentAmt")) ? (BigDecimal) ordDetailsM.get("defRentAmt") : BigDecimal.ZERO);
        dParam.put("rotRem", "");
        ...
        if(SalesConstants.APP_TYPE_CODE_ID_RENTAL == CommonUtils.intNvl(ordDetailsM.get("appTypeId"))) {
            ...
            rpsParam.put("bankId", paymode == 131 ? CommonUtils.intNvl(params.get("hiddenRentPayCRCBankID")) : paymode == 132 ? CommonUtils.intNvl(params.get("hiddenRentPayBankAccBankID")) : 0);
            ...
            rpsParam.put("custId", "Y".equals(isThirdParty) ? CommonUtils.intNvl(params.get("txtHiddenThirdPartyID")) : CommonUtils.intNvl(params.get("txtHiddenCustID")));
            rpsParam.put("editTypeId", "0");
            rpsParam.put("nricOld", params.get("txtRentPayIC"));
            rpsParam.put("failResnId", "0");
            rpsParam.put("issuNric", CommonUtils.isNotEmpty(params.get("txtRentPayIC").toString().trim()) ? params.get("txtRentPayIC") : "Y".equals(isThirdParty) ? params.get("txtThirdPartyNRIC") : params.get("txtCustIC"));
        ...
        if(CommonUtils.intNvl(params.get("nationNmOwnt")) == 14 && CommonUtils.intNvl(params.get("hiddenCustTypeID")) == SalesConstants.CUST_TYPE_CODE_ID_IND) {
            ccpStatusId = 5;
        ...
        ccpParam.put("ccpSchemeTypeId", CommonUtils.intNvl(params.get("hiddenCustTypeID")) == SalesConstants.CUST_TYPE_CODE_ID_IND ? SalesConstants.CCP_SCHEME_TYPE_CODE_ID_ICS : SalesConstants.CCP_SCHEME_TYPE_CODE_ID_CCS);
        ...
        if(billMap != null) {
            if(CommonUtils.intNvl(billMap.get("custBillSoId")) == CommonUtils.intNvl(ordDetailsM.get("salesOrdId"))) {
                ...
                if(ordDetailsM2 != null) {
                    ...
                    // Insert into SAL0280D current "CUST_BILL_ID" data from SAL0024D into SAL0280D with ROT_ID and updated "CUST_BILL_SO_ID"
        ...
        if(mCnt != 0 && dCnt != 0 && ccpCnt != 0 && instCnt != 0) {
            ...
            if(SalesConstants.APP_TYPE_CODE_ID_RENTAL == CommonUtils.intNvl(ordDetailsM.get("appTypeId")) && rpsCnt != 0) {
    ...
    @Transactional
    /* Function copied from CcpCalculateServiceImpl.calSave
     * Function amended to cater ROT flow
     */
    public int calSave(Map<String, Object> params) throws Exception {
        ...
        if(("5").equals(params.get("statusEdit")) || ("13").equals(params.get("statusEdit"))  || ("14").equals(params.get("statusEdit"))) {
            ...
            /* On Approval ::
             * 1. Update ROT CCP Status to approved
             * 2. Insert into SAL0004D (Sales exchange log table)
             * 3. Merge SAL0074D (RentPaySet table)
             * 4. Insert into SAL0024D (Rental Billing Group table) [If new billing group will insert one record; Existing - to test]
             * 5. Update SAL0001D (Sales Master table)
             * 6. Get Rental Billing Group staging details (For re-updating purpose of "existing order" of "previous"/"existing" billing group)
             * 7. Merge SAL0045D (Installation table)
             * 8. Check eCash, auxiliary, paymode for call entry and progress insertion
             */
            ...
            if(billMap != null) {
                if(CommonUtils.intNvl(billMap.get("custBillSoId")) == CommonUtils.intNvl(exCustBillId.get("rotOrdId"))) {
                    tempParam.put("custBillId", CommonUtils.intNvl(billMap.get("custBillId")));
            ...
            if(eCashMap != null || (params.get("auxAppType") != null && params.get("payMode") != null)) {
                ...
                params.put("callEntrySeq", callSeq);
                ...
                LOGGER.info("4. Insert Call Log Master CCR0006D :: Approve");
                ccpCalculateMapper.insertCallEntry(params);
