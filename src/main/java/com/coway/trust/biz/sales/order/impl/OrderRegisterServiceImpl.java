...
public class OrderRegisterServiceImpl extends EgovAbstractServiceImpl implements OrderRegisterService {
  ...
  public EgovMap selectStockPrice(Map<String, Object> params) {
    ...
    || SalesConstants.APP_TYPE_CODE_ID_TRIAL_RENTAL == Integer.parseInt(String.valueOf(params.get("appTypeId")))) {
      // orderPrice = "₩" +
      ...
      if (isNew == 1) {
        ...
        if (priceInfo != null) {
          if (SalesConstants.PROMO_APP_TYPE_CODE_ID_REN == Integer
              ...
              SalesConstants.PROMO_APP_TYPE_CODE_ID_TRL_REN == Integer
...
//                    int allowToExTrade = rentalPeriod - exTrade_mth;
  ...
  private void preprocOrderLog(List<SalesOrderLogVO> salesOrderLogVOList, int orderAppType, int custTypeId,
    ...
    if (orderAppType != SalesConstants.APP_TYPE_CODE_ID_RENTAL && orderAppType != SalesConstants.APP_TYPE_CODE_ID_TRIAL_RENTAL ) {
  ...
  public void registerOrder(OrderVO orderVO, SessionVO sessionVO) throws ParseException {
    ...
    // Self Service Rebate
    if((orderAppType == SalesConstants.APP_TYPE_CODE_ID_RENTAL || orderAppType ==  SalesConstants.APP_TYPE_CODE_ID_TRIAL_RENTAL) && srvType != null && srvType.equals("SS")){
...
//    	}
    ...
    if(salesOrderMVO.getExTrade() == 4 && orderAppType != SalesConstants.APP_TYPE_CODE_ID_AUX){
    	...
    	//insert into SAL0421D
    	orderRegisterMapper.insertPwp(pwpInfo);
    }

    // SAL0424D - Rebate Info - INSERT INTO SAL0424D [S]
   if(salesOrderMVO.getExTrade() == 5 && orderAppType != SalesConstants.APP_TYPE_CODE_ID_AUX){

	   EgovMap oldData = new EgovMap();
	   oldData.put("salesOrdNo", salesOrderMVO.getSalesOrdNo());
	   oldData.put("custId", salesOrderMVO.getCustId());
	   oldData.put("salesOrdId", orderVO.getRebateOrderId());
	   oldData.put("userId",sessionVO.getUserId());

	   EgovMap existingOldOrder = orderRegisterMapper.selectOldOrder(oldData);

	   if(existingOldOrder != null && Integer.parseInt(existingOldOrder.get("oldOrdCount").toString()) > 0) { //existing Rebate
		   EgovMap oldOrderList = orderRegisterMapper.selectOldOrderList(oldData);
		   EgovMap newOrderList = new EgovMap();

		   newOrderList.put("comboId", oldOrderList.get("comboId"));
		   newOrderList.put("seq", Integer.parseInt(oldOrderList.get("seq").toString()) + 1);
