...
public class InstallationResultListServiceImpl extends EgovAbstractServiceImpl
  ...
  /*
   * @Override public EgovMap getPromoPriceAndPV(int promotionId, int productId)
    ...
    if (entry != null && Integer.parseInt(entry.get("installEntryId").toString()) > 0 && exchange != null
      ...
      if (entry.get("stusCodeId").toString().equals("1") && exchange.get("soExchgStusId").toString().equals("1")) {
        ...
        if ((int) installResult.get("statusCodeId") == 4) {
          ...
          /*
           * //complete if(sirimList != null && sirimList.size() > 0){ for(int i
           * = 0; i< sirimList.size(); i++){
           * //installationResultListMapper.insertSirim(sirimList.get(i)); } }
           *
           * if((int) exchange.get("soCurStusId") != 25 && (int)
           * exchange.get("soCurStusId") != 26){ //membership int pacId = 0;
           * if((int)salesOrderM.get("appTypeId") == 66){ pacId = 4; }else
           * if((int)salesOrderM.get("appTypeId") == 67 &&
           * (int)salesOrderM.get("appTypeId") == 68 &&
           * (int)salesOrderM.get("appTypeId") == 142){ pacId = 3; }else
           * if((int)salesOrderM.get("appTypeId") == 144){ pacId = 10; }else{
           * pacId = 2; } entry.put("pacId", pacId); EgovMap pacSetting =
           * installationResultListMapper.selectPac(entry);
           *
           * int membershipDuration = 0; int membershipFreq = 0; int
           * membershipBSQty = 0;
           *
           * if(pacSetting != null){ membershipDuration =
           * pacSetting.get("srvMemDur") != null ?
           * (int)pacSetting.get("srvMemDur") : 0; membershipFreq =
           * pacSetting.get("srvMemItmPriod") != null ?
           * (int)pacSetting.get("srvMemItmPriod") : 0;
           *
           * if(membershipDuration > 0 && membershipFreq > 0){ membershipBSQty =
           * ((membershipDuration / 12) * (12 / membershipFreq)); } } Calendar
           * calendar = Calendar.getInstance();
           *
           * Calendar startDate = getFirstDayOfMonth(calendar); Calendar
           * endDate=getLastDayOfMonth(calendar);
           *
           * EgovMap membershipNo = getDocNo("12"); int MembershipDocID = 12;
           * String nextDocNo
           * =getNextDocNo("SM",membershipNo.get("docNo").toString());
           ...
           *
           * membership.put("srvMemId", 0); membership.put("srvMemQuotId", 0);
           * membership.put("srvSalesOrderId", salesOrderM.get("salesOrdId"));
           * membership.put("srvMemNo", membershipNo.get("docNo").toString());
           * membership.put("srvMemBillNo", ""); membership.put("srvMemPacId",
           * pacId); membership.put("srvMemPacAmt", 0);
           * membership.put("srvMemBSAmt", 0); membership.put("srvMemPV",0);
           ...
           * membership.put("srvRemark",""); membership.put("srvCreateAt",new
           * Date());
           ...
           * if((int) exchange.get("soExchgNwPromoId") == 31330 || (int)
           * exchange.get("soExchgNwPromoId") == 31331 || (int)
           * exchange.get("soExchgNwPromoId") == 31332 ||(int)
           * exchange.get("soExchgNwPromoId") == 31333 || (int)
           * exchange.get("soExchgNwPromoId") == 31371 || (int)
           * exchange.get("soExchgNwPromoId") == 31372 || (int)
           * exchange.get("soExchgNwPromoId") == 31373 || (int)
           * exchange.get("soExchgNwPromoId") == 31374 ) {
           *
           * EgovMap memNoFree = getDocNoNumber("12"); updateDocNoNumber("12");
           *
           * Calendar srvStartDate = endDate;//.add(calendar.DATE,1 );
           * srvStartDate.add(calendar.DATE,1 ); Calendar srvExpireDate =
           * startDate;//.add(calendar.MONTH, 12)
           * srvExpireDate.add(calendar.MONTH, 12); membership.put("srvMemNo",
           * memNoFree.get("docNo")); membership.put("srvMemPacId", 19);
           ...
           *
           * EgovMap list =
           * installationResultListMapper.selectList(salesOrderM);
           *
           * if((int) salesOrderM.get("appTypeId") == 66){
           *
           *
           * Date salesDate = df.parse(list.get("salesDt").toString()); Date
           * SalesGSTcutOffDate = df.parse("04/01/2015" );
           *
           * if(salesDate.compareTo(SalesGSTcutOffDate) == -1){ Map<String,
           * Object> rentLedger = new HashMap<String, Object>();
           * rentLedger.put("rentRunId", 0); rentLedger.put("rentId", 0);
           * rentLedger.put("rentSOId", salesOrderM.get("salesOrdId"));
           * rentLedger.put("rentDocNo", entry.get("installEntryId"));
           * rentLedger.put("rentDocTypeId",161); rentLedger.put("rentDateTime",
           * new Date()); rentLedger.put("rentAmount",
           * exchange.get("soExchgNwPrc")); rentLedger.put("rentBatchNo", "");
           * rentLedger.put("rentInstNo", 0); rentLedger.put("rentUpdateBy",
           * sessionVO.getUserId()); rentLedger.put("rentUpdateAt", new Date());
           * rentLedger.put("rentIsSync", false);
           * rentLedger.put("rentBillRunningTotal", 0); rentLedger.put("R01",
           * 0); rentLedger.put("R02", ""); logger.debug("rentLedger : {}",
           * rentLedger); //insert문 추가
           *
           * Map<String, Object> orderbill = new HashMap<String, Object>();
           * orderbill.put("accBillTaskId", 0); orderbill.put("accBillRefDate",
           * new Date()); orderbill.put("accBillRefNo", "1000");
           ...
           * orderbill.put("accBillScheduleAmount",(int)exchange.get(
           * "soExchgNwPrc")); orderbill.put("accBillAdjustmentAmount",0);
           * orderbill.put("accBillTaxesAmount",(int)exchange.get(
           * "soExchgNwPrc") - (int)exchange.get("soExchgNwPrc") * 100/106);
           ...
           * orderbill.put("accBillTaxCodeId",32); logger.debug("orderbill : {}"
           * , orderbill); //쿼리 추가
           *
           * if(list != null){ EgovMap invoiceNum = new EgovMap();
           * if(list.get("typeId").toString() == "965"){ //Soo Modified Form GST
           * 2015-01-05
           *
           ...
           * taxInvoiceD.put("invoiceItemGSTTaxes",
           * (int)exchange.get("soExchgNwPrc") -
           * (int)exchange.get("soExchgNwPrc") * 100/106);
           * taxInvoiceD.put("invoiceItemRentalFee",
           * (int)exchange.get("soExchgNwPrc"));
           * taxInvoiceD.put("invoiceItemHandlingFee", 0);
           * taxInvoiceD.put("invoiceItemHandlingFeeTaxes", 0);
           * taxInvoiceD.put("invoiceItemProductCategories",
           * list.get("codeDesc")); taxInvoiceD.put("invoiceItemProductModel",
           * list.get("stkDesc")); taxInvoiceD.put("invoiceItemProductSerialNo",
           * installResult.get("serialNo")); taxInvoiceD.put("invoiceItemAdd1",
           * list.get("add11")); taxInvoiceD.put("invoiceItemAdd2",
           * list.get("add21")); taxInvoiceD.put("invoiceItemAdd3",
           * list.get("add31")); taxInvoiceD.put("invoiceItemPostCode",
           * list.get("postCode1")); taxInvoiceD.put("invoiceItemStateName",
           * list.get("name4")); taxInvoiceD.put("invoiceItemCountry",
           * list.get("name5")); taxInvoiceD.put("invoiceItemInstallationDate",
           * installResult.get("installDate"));
           ...
           * logger.debug("taxInvoiceD : {}", taxInvoiceD); //쿼리추가 } } }
           *
           * EgovMap rentScheme = if(rentScheme != null){
           * rentScheme.put("renSchDate", new Date());
           * rentScheme.put("statusCodeID","REG");
           * rentScheme.put("IsSync",false); //쿼리 추가 }
           *
           * } else if((int)salesOrderM.get("appTypeId") == 67 ||
           * (int)salesOrderM.get("appTypeId") == 68){ double outRightPreBill =
           * getOutRightPreBill(salesOrderM); double outRightBalance =
           * (int)exchange.get("soExchgNwPrc") - outRightPreBill;
           *
           * if((int)exchange.get("soExchgNwPrc") > outRightPreBill){ int
           * taxCodeId = 0; int taxRate = 0;
           *
           * int zrLocationId = (int)getZRLocationId(salesOrderM); int
           * RLCertificateID = (int)getRSCertificateID(salesOrderM);
           *
           * if(zrLocationId != 0){ taxCodeId = 39; taxRate = 0; }else{
           * if(RLCertificateID != 0){ taxCodeId = 28; taxRate = 0; }else{
           * taxCodeId = 32; taxRate = 6; } }
           *
           * Map<String, Object> orderbill = new HashMap<String, Object>();
           * orderbill.put("accBillTaskId", 0); orderbill.put("accBillRefDate",
           * new Date()); orderbill.put("accBillRefNo", "1000");
           ...
           * orderbill.put("accBillTaxesAmount",(int)exchange.get(
           * "soExchgNwPrc") - (int)exchange.get("soExchgNwPrc") * 100/106);
           * }else{ orderbill.put("accBillTaxesAmount",0); }
           ...
           * orderbill.put("accBillContractId", 0); logger.debug(
           * "orderbill : {}", orderbill);
           * installationResultListMapper.insertAccOrderBill(orderbill); //쿼리
           *
           ...
           *
           * Map<String, Object> taxInvocieOutright = new HashMap<String,
           * Object>(); taxInvocieOutright.put("taxInvoiceRefNo",
           * invoiceNum.get("docNo"));
           * taxInvocieOutright.put("taxInvoiceRefDate", new Date());
           * taxInvocieOutright.put("taxInvoiceCustName", list.get("name"));
           * taxInvocieOutright.put("taxInvoiceContactPerson",
           * list.get("name1")); taxInvocieOutright.put("taxInvoiceAddress1",
           * list.get("add1")); taxInvocieOutright.put("taxInvoiceAddress2",
           * list.get("add2")); taxInvocieOutright.put("taxInvoiceAddress3",
           * list.get("add3")); taxInvocieOutright.put("taxInvoiceAddress4",
           * list.get("add4")); taxInvocieOutright.put("taxInvoicePostCode",
           * list.get("postCode"));
           ...
           * taxInvocieOutright.put("taxInvoiceRemark",entry.get(
           * "installEntryNo")); if(taxRate > 0){
           * taxInvocieOutright.put("taxInvoiceCharges",new
           * DecimalFormat("0.00").format(outRightBalance * 100 / 106)); }else{
           * taxInvocieOutright.put("taxInvoiceCharges",new
           * DecimalFormat("0.00").format(outRightBalance)); }
           * taxInvocieOutright.put("taxInvoiceOverdue",0);
           * taxInvocieOutright.put("taxInvoiceAmountDue",new
           * DecimalFormat("0.00").format(outRightBalance));
           *
           * logger.debug("taxInvocieOutright : {}", taxInvocieOutright);
           * //installationResultListMapper.insertTaxInvocieOutright(
           * taxInvocieOutright);
           *
           *
           * Map<String, Object> taxInvocieOutrightD = new HashMap<String,
           * Object>(); taxInvocieOutrightD.put("taxInvoiceID",
           * "바로 위 테이블에서 값가져오야함"); taxInvocieOutrightD.put("invoiceItemOrderNo",
           * list.get("salesOrdNo"));
           * taxInvocieOutrightD.put("invoiceItem_PONo", "");
           * taxInvocieOutrightD.put("invoiceItemGSTRate", taxRate); if(taxRate
           * > 0){ taxInvocieOutrightD.put("invoiceItemGSTTaxes", new
           * DecimalFormat("0.00").format((int)entry.get("soExchgNwPrc") -
           * (int)entry.get("soExchgNwPrc"))); }else{
           * taxInvocieOutrightD.put("invoiceItemGSTTaxes",0); }
           * taxInvocieOutrightD.put("invoiceItemAmountDue", outRightBalance);
           * taxInvocieOutrightD.put("invoiceItemRentalFee",
           * (int)taxInvocieOutrightD.get("invoiceItemAmountDue") -
           * (int)taxInvocieOutrightD.get("invoiceItemGSTTaxes"));
           * taxInvocieOutrightD.put("invoiceItemProductCategories",list.get(
           * "codeDesc")); taxInvocieOutrightD.put("invoiceItemProductModel",
           * list.get("stkDesc"));
           * taxInvocieOutrightD.put("invoiceItemProductSerialNo",installResult.
           * get("sirialNo"));
           ...
           * tradeLedger.put("tradeBatchNo", ""); tradeLedger.put("tradeInstNo",
           * 0); tradeLedger.put("tradeUpdateBy", installResult.get("creator"));
           * tradeLedger.put("tradeUpdateAt", new Date());
           * tradeLedger.put("tradeIsSync", false); logger.debug(
           * "tradeLedger : {}", tradeLedger);
           * //installationResultListMapper.insertTradeLedger(tradeLedger);
           *
           * } }
           *
           * //CONFIG Map<String, Object> config = new HashMap<String,
           * Object>(); config.put("srvConfigId", 0); config.put("srvSOId",
           * salesOrderM.get("salesOrdId")); config.put("srvCodyId",0);
           * config.put("srvPreviousDate",installResult.get("installDate"));
           * config.put("srvRemark",""); config.put("srvBSGen",true);
           ...
           * config.put("srvStatusId",1); config.put("srvBSWeek",0);
           * logger.debug("config : {}", config);
           * //installationResultListMapper.insertConfig(config);
           *
           * for(int i=279; i<=281;i++){ Map<String, Object> configSet = new
           * HashMap<String, Object>(); configSet.put("srvSettId", 0);
           * configSet.put("srvConfigId", "위에 쿼리에 시퀀스 가져와야힘");
           * configSet.put("srvSettTypeID", i); configSet.put("srvSettStatusID",
           * 1); configSet.put("srvSettRemark", "");
           ...
           * configPeriod.put("srvPrdId", 0); configPeriod.put("srvConfigId",
           * "위에위에 테이블 시퀀스"); configPeriod.put("srvMembershipId", 0);
           ...
           * exchange.get("soExchgNwPromoId") == 31373 || (int)
           * exchange.get("soExchgNwPromoId") == 31374 ) {
           * configPeriod.put("srvPrdStartDate",
           * configPeriod.get("srvPrdExpireDate"));
           * configPeriod.put("srvPrdExpireDate",
           * configPeriod.get("srvPrdStartDate")); //쿼리추가 }
           *
           * List<EgovMap> filter =
           * installationResultListMapper.selectFilter(entry);
           *
           * if(filter != null && filter.size() > 0){ for(int i=0;
           * i<filter.size(); i++){ Map<String, Object> configFilter = new
           * HashMap<String, Object>(); configFilter.put("srvFilterId", 0);
           * configFilter.put("srvConfigId",config.get("시퀀스") );
           * configFilter.put("srvFilterStkId", filter.get(i).get(("bomPartId"))
           * != null ? filter.get(i).get(("bomPartId")) : 0 );
           * configFilter.put("srvFilterPeriod",
           * filter.get(i).get("bomPartPriod"));
           * configFilter.put("srvFilterPrvChgDate",(Date)installResult.get(
           * "installDate")); configFilter.put("srvFilterStatusId",1);
           ...
           * } }else{ //qryCurrentConfig 여기있는곳 }
           *
           * EgovMap salesDet =
           * installationResultListMapper.selectSalesDet(salesOrderM);
           * if(salesDet != null){ salesDet.put("itemStkId",
           * entry.get("installStkId")); salesDet.put("itemPrice",
           * exchange.get("soExchgNwPrc")); salesDet.put("itemPV",
           * exchange.get("soExchgNwPv")); salesDet.put("itemPriceId",
           * exchange.get("soExchgNwPrcId")); salesDet.put("updated", new
           * Date()); salesDet.put("updator", installResult.get("creator"));
           * logger.debug("salesDet : {}", salesDet);
           * //installationResultListMapper.updateSalesDet(salesDet); }
           *
           * salesOrderM.put("statusCodeId", 4); salesOrderM.put("updated", new
           * Date()); salesOrderM.put("updator", installResult.get("creator"));
           ...
           * if((int)salesOrderM.get("appTypeId") == 66) {
           * if(df.parse(salesOrderM.get("salesDt").toString()).compareTo(df.
           * parse("08/01/2014")) == 1 ||
           * df.parse(salesOrderM.get("salesDt").toString()).compareTo(df.parse(
           * "08/01/2014")) == 0){ salesOrderM.put("mthRentAmt",
           * exchange.get("soExchgNwRentAmt")); salesOrderM.put("defRentAmt",
           * exchange.get("soExchgNwDefRentAmt")); } } logger.debug(
           * "salesOrderM : {}", salesOrderM);
           * //installationResultListMapper.updateSalesOrderM(salesOrderM);
           *
           * exchange.put("soExchgNwStkId", entry.get("installStkId"));
           * exchange.put("soExchgStatusId", 4); exchange.put("soExchgUpdateAt",
           * new Date()); exchange.put("soExchgUpdateBy",
           * sessionVO.getUserId()); logger.debug("exchange : {}", exchange);
           * //installationResultListMapper.updateExchange(exchange);
           *
           * Map<String, Object> happyCall = new HashMap<String, Object>();
           * happyCall.put("HCID", 0); happyCall.put("HCSOID",
           * salesOrderM.get("salesOrdId")); happyCall.put("HCCallEntryId", 0);
           * happyCall.put("HCTypeNo", entry.get("installEntryNo"));
           * happyCall.put("HCTypeId", 508); happyCall.put("HCStatusId", 33);
           * happyCall.put("HCRemark", ""); happyCall.put("HCCommentTypeId", 0);
           * happyCall.put("HCCommentGId", 0); happyCall.put("HCCommentSId", 0);
           * happyCall.put("HCCommentDId", 0); happyCall.put("creator",
           * sessionVO.getUserId()); happyCall.put("created", new Date());
           * happyCall.put("updator", sessionVO.getUserId());
           * happyCall.put("updated", new Date()); happyCall.put("HCNoService",
           * false); happyCall.put("HCLock", false); happyCall.put("HCCloseId",
           * 0); logger.debug("happyCall : {}", happyCall);
           * //installationResultListMapper.insertHappyCall(happyCall);
           *
           * EgovMap mobWH =
           * installationResultListMapper.selectMobWh(installResult);
           *
           * Map<String, Object> card = new HashMap<String, Object>();
           * card.put("SRCardID", 0); card.put("locationId",
           * mobWH.get("whLocId")); card.put("stockId",
           * entry.get("installStkId")); card.put("entryDate", new Date());
           * card.put("typeId", 463); card.put("refNo",
           * entry.get("installEntryNo")); card.put("salesOrderId",
           * salesOrderM.get("salesOrdId")); card.put("itemNo", 1);
           * card.put("sourceId", 477); card.put("projectId", 0);
           * card.put("batchNo", 0); card.put("Qty", -1); card.put("currId",
           * 479); card.put("currRate", -1); card.put("cost", 0);
           * card.put("price", 0); card.put("remark", ""); card.put("serialNo",
           * ""); card.put("installNo", entry.get("installEntryNo"));
           * card.put("costDate", new Date()); card.put("appTypeId",
           * salesOrderM.get("appTypeId")); card.put("stkGrade", "A");
           * card.put("installFail", false); card.put("isSynch", false);
           * card.put("entryMethodId", 0); card.put("origin", ""); logger.debug(
           * "card : {}", card);
           * //installationResultListMapper.insertRecordCard(card);
           *
           * orderProgressIsLook = false; orderProgressId = 5;
           ...
           * if((int)DO.get("movStusId") == 4){ Map<String, Object> mov = new
           * HashMap<String, Object>(); mov.put("movId", 0);
           * mov.put("installEntryId", entry.get("installEntryId"));
           * mov.put("movFromLocId", DO.get("movToLocId"));
           * mov.put("movToLocId", 0); mov.put("movTypeId", 262);
           * mov.put("movStatusId", 1); mov.put("movConfirm", 0);
           * mov.put("movCreateAt", new Date()); mov.put("movCreateBy",
           * sessionVO.getUserId()); mov.put("movUpdateAt", new Date());
           * mov.put("movUpdateBy", sessionVO.getUserId());
           * mov.put("stkCrdPost", false); mov.put("stkCrdPostDate",
           * "1900-01-01"); mov.put("stkCrdPostToWebOnTime", true);
           *
           * logger.debug("mov : {}", mov);
           * //installationResultListMapper.insertMovement(mov);//프로시저 호출로
           * 수정******* }else if((int)DO.get("movStusId") == 1){
           * DO.put("movStusId", 8); DO.put("movUpdateAt", new Date());
           * DO.put("movUpdateBy",sessionVO.getUserId()); //쿼리 }
           */
  ...
  private double getZRLocationId(Map<String, Object> params) {
    ...
    /*
     * if(outRightPreBill != null){ for(int i = 0; i< outRightPreBill.size();
     * i++){
     *
     * result += (int)outRightPreBill.get(i).get("tradeAmt");
     *
  ...
  private Map<String, Object> Save_2(boolean isfreepromo, Map<String, Object> params, SessionVO sessionVO)
    ...
    /*
     * APPLICATION TYPE -------------------------------- 66 - RENTAL 67 -
     * OUTRIGHT 68 - INSTALLMENT 1412 - OUTRIGHT PLUS
     */
