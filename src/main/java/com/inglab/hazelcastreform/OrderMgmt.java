...
//        return sql;
    ...
    public static void debugPassInParams(Map<String, Object> params) {
      ...
      for (String paramKey : paramKeys) {
        ...
        if (isList) {
          logger.debug("Param key: " + paramKey + ", isList: true");
        }
        else {
          try {
            String[] strArr = (String[]) params.get(paramKey);

            logger.debug("KKKKKKKKKKK " + strArr[0]);
          }
          catch (Exception ex) {
          }
          logger.debug("Param key: " + paramKey + ", isList: false, value: " + params.get(paramKey).toString());
        }
      }
    }
