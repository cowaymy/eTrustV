...
$(document).ready(function () {
    aprvGridID = AUIGrid.create("#approveView_grid_wrap", aprvColumnLayout, aprvGridPros);
    AUIGrid.setGridData(aprvGridID, myGridData);

    AUIGrid.bind(aprvGridID, "cellDoubleClick", function( event )
            {
...
function fn_preloadData() {
	$("#viewClmNo").text(myGridData[0].clmNo);
    $("#viewClmType").text(myGridData[0].clmType);
    $("#viewDscCode").text(myGridData[0].dscCode + "/" + myGridData[0].dscName);
    ...
    $("#viewMemAccId").val(myGridData[0].memAccCode);
    $("#viewMemAccNameTd").text(myGridData[0].memAccCode + " / " + myGridData[0].memAccName);
...
function cellStyleFunction(rowIndex, columnIndex, value, headerText, item, dataField){
    console.log("atchFileId ma " + item.atchFileId);
    if(item.atchFileId != ''){
        return '';
    }else{
        return "edit-column";
...
<h1 id="submit">View Submit</h1>
<h1 id="approve" style="display: none;">View Approve</h1>
<ul class="right_opt">
