...
function fn_appvLineGridAddDefaultRow() {

	Common.ajaxSync("GET", "/eAccounting/ctDutyAllowance/selectMemberViewByMemCode.do", {memAccId : memAccId}, function(memInfo) {

		console.log("approvesubmit");
        if(memInfo == null) {
            Common.alert('Manager not found.');
        }
        else {
            console.log(memInfo[0]);
            var data = {
            		memCode : memInfo[0].memCode1
            		,name : memInfo[0].name1
            };
            AUIGrid.addRow(approveLineGridID, data, "first");

            //default dsc care pic
            var data = {
                    memCode : 'P1872'
                    ,name : 'NUR SYAHIRAH BINTI MOHAMAD LAZIM'
            };
            AUIGrid.addRow(approveLineGridID, data, "last");
        }
    });
...
function fn_reqstSubmit() {
	...
	var approveLineGridList = AUIGrid.getOrgGridData(approveLineGridID);
	...
	if(checkMemCode) {
		var data = {
                ...
                ,apprGridList : approveLineGridList
        };

		//Common.popupDiv("/eAccounting/ctDutyAllowance/registrationMsgPop.do", data, null, true, "registrationMsgPop");

		Common.confirm("Are you sure you want to submit this CT Duty Allowance Claim Expense form?",function (){
			console.log("click yes");
			 Common.ajax("POST", "/eAccounting/ctDutyAllowance/approveLineSubmit.do", data, function(result) {
			        console.log(result);
