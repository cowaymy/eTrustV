...
$(document).ready(function () {
	...
	AUIGrid.bind(supplierGridID, "cellDoubleClick", function( event ) {
        ...
        $("#search_dscCode").val(event.item.dscCode);
        $("#search_dscName").val(event.item.dscName);
...
function fn_selectMember() {
	Common.ajax("GET", "/eAccounting/ctDutyAllowance/selectSupplier.do", $("#form_supplier").serializeJSON(), function(result) {
...
<input type="hidden" id="search_dscCode">
<input type="hidden" id="search_dscName">
...
<tr>
	...
	<select class="w100p" id="accGrp" name="accGrp">
        ...
        <option value="VM12" <c:if test="${accGrp eq 'VM12'}">selected</c:if>><spring:message code="memAcc.ht" /></option>
