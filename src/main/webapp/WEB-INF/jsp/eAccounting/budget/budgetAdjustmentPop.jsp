...
//console.log(fileList);
    ...
    var adjPLayout = [{
        ...
        editable : false,
        cellMerge : true ,
        mergeRef : "budgetGrpSeq", // 이전 칼럼(대분류) 셀머지의 값을 비교해서 실행함. (mergePolicy : "restrict" 설정 필수)
        mergePolicy : "restrict",
        width : 130
    },{
    ...
    AUIGrid.bind(adjPGridID, "cellClick", function( event )
    {
        ...
        budgetDocNo = event.item.budgetDocNo;
        budgetGrpSeq = event.item.budgetGrpSeq;
        budgetAdjType = event.item.budgetAdjType;

        console.log("CellClick rowIndex :: " + event.rowIndex + ", columnIndex : " + event.columnIndex + " clicked");
        console.log("CellClick budgetDocNo :: " + event.item.budgetDocNo);
        console.log("CellClick budgetGrpSeq :: " + event.item.budgetGrpSeq);
    });

    AUIGrid.bind(adjPGridID, "cellDoubleClick", function( event )
    {
        budgetFlg = "U";
        ...
        console.log("cellDoubleClick budgetDocNo :: " + event.item.budgetDocNo);
        console.log("cellDoubleClick budgetGrpSeq :: " + event.item.budgetGrpSeq);
...
function fn_RemoveRow(){
     ...
     var gridLen = AUIGrid.getGridData(adjPGridID).length;
     for(var i = 0; i < gridLen; i++) {
         if(budgetGrpSeq == AUIGrid.getCellValue(adjPGridID, i, "budgetGrpSeq")) {
             console.log("budgetGrpSeq :: " + i + " :: " + budgetGrpSeq);
             console.log("Next rowId :: " + i);

             AUIGrid.removeRow(adjPGridID, i);
         }
     }
}
...
function fn_AddRow()
{
	console.log("pre :: groupSeq :: " + groupSeq);
	console.log("pre :: rowid :: " + rowid);
 ...
 /*
    ...
    if(budgetFlg == "U") {
    	//AUIGrid.removeRow(adjPGridID, budgetGrpSeq);
    	AUIGrid.updateRow(adjPGridID, item, rowid);
        ...
        for(var i = 0; i < gridLen; i++) {
            if(i != rowid) {
                if(budgetGrpSeq == AUIGrid.getCellValue(adjPGridID, i, "budgetGrpSeq")) {
                    ...
                    AUIGrid.updateRow(adjPGridID, item2, budgetGrpSeq);
                }
            }
        }
    }
	    ...
	    fn_clearData();
	    console.log("post :: groupSeq :: " + groupSeq);
...
function fn_setBudgetView() {
	Common.ajax("GET", "/eAccounting/budget/getAdjView.do", {budgetDocNo : budgetDocNo, budgetGrpSeq : budgetGrpSeq, budgetAdjType : budgetAdjType}, function(result) {
		console.log(result);

		$("#pAdjustmentType").val(budgetAdjType);

		$("#sendYearMonth").val(result.senderBudgetPeriod);
		$("#sendCostCenter").val(result.senderCostCenter);
		$("#sendBudgetCode").val(result.senderBudgetCode);
		$("#sendBudgetCodeName").val(result.senderBudgetDesc);
		$("#sendGlAccCode").val(result.senderGLAcc);
		...
		if(budgetAdjType != "01" && budgetAdjType != "02") {
			...
			$("#recvBudgetCodeName").val(result.receiverBudgetDesc);
			$("#recvGlAccCode").val(result.receiverGLAcc);
			$("#recvGlAccCodeName").val(result.receiverGLDesc);
