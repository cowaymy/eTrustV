...
function fn_addMyGridRow() {
    if(AUIGrid.getRowCount(myGridID) > 0) {
        AUIGrid.addRow(myGridID, {clamUn:AUIGrid.getCellValue(myGridID, 0, "clamUn"),taxCode:"OP (Purchase(0%):Out of scope)",expGrp:"0",cur:"MYR",gstBeforAmt:0,gstAmt:0,taxNonClmAmt:0,totAmt:0}, "last");
    } else {
        Common.ajax("GET", "/eAccounting/webInvoice/selectClamUn.do?_cacheId=" + Math.random(), {clmType:"J5"}, function(result) {
            console.log(result);
            AUIGrid.addRow(myGridID, {clamUn:result.clamUn,taxCode:"OP (Purchase(0%):Out of scope)",expGrp:"0",cur:"MYR",gstBeforAmt:0,gstAmt:0,taxNonClmAmt:0,totAmt:0}, "last");
        });
    }
}
...
function fn_selectStaffClaimInfo() {
    ...
    Common.ajax("GET", "/eAccounting/scmActivityFund/selectScmActivityFundInfo.do?_cacheId=" + Math.random(), obj, function(result) {
        console.log(result);
...
function fn_editRejected() {
    ...
    var gridObj = AUIGrid.getSelectedItems(staffClaimGridID);
    var list = AUIGrid.getCheckedRowItems(staffClaimGridID);
    ...
    if(gridObj != "" || list != "") {
        ...
        if(status == "Rejected") {
            Common.ajax("POST", "/eAccounting/scmActivityFund/editRejected.do", {clmNo : selClmNo}, function(result1) {
                console.log(result1);
...
<!-- report Form -->
<form id="dataForm">
    <input type="hidden" id="reportFileName" name="reportFileName" value="/e-accounting/SCMClaim.rpt" /><!-- Report Name  -->
