...
var staffClaimColumnLayout = [ {
    dataField : "memAccId",
    headerText : '<spring:message code="codyClaim.codyId" />'
}, {
    dataField : "memAccName",
    headerText : '<spring:message code="codyClaim.codyBrName" />',
...
function fn_mileageAdd() {
	if(AUIGrid.getRowCount(mileageGridID) > 0) {
        ...
        Common.ajax("GET", "/eAccounting/webInvoice/selectClamUn.do?_cacheId=" + Math.random(), {clmType:"J7"}, function(result) {
            console.log(result);
...
function fn_selectStaffClaimList() {
	Common.ajax("GET", "/eAccounting/codyClaim/selectCodyClaimList.do?_cacheId=" + Math.random(), $("#form_staffClaim").serialize(), function(result) {
        console.log(result);
...
function fn_newStaffClaimPop() {
	Common.popupDiv("/eAccounting/codyClaim/newCodyClaimPop.do", {callType:"new"}, null, true, "newStaffClaimPop");
}
...
function fn_addRow() {
    ...
    if(fn_checkEmpty()) {
        ...
        if($(":input:radio[name=expGrp]:checked").val() == "1") {
        	...
        	if(clmSeq == 0) {
        		Common.ajaxFile("/eAccounting/codyClaim/attachFileUpload.do", formData, function(result) {
                    console.log(result);
        		...
        		Common.ajaxFile("/eAccounting/codyClaim/attachFileUpdate.do", formData, function(result) {
                    console.log(result);
...
function fn_insertStaffClaimExp(st) {
    ...
    if(gridDataList.length > 0){
        ...
        console.log(data);
        Common.ajax("POST", "/eAccounting/codyClaim/insertCodyClaimExp.do", data, function(result) {
            console.log(result);
...
function fn_selectStaffClaimItemList() {
    ...
    Common.ajax("GET", "/eAccounting/codyClaim/selectCodyClaimItemList.do?_cacheId=" + Math.random(), obj, function(result) {
        console.log(result);
...
function fn_viewStaffClaimPop(clmNo) {
    ...
    Common.popupDiv("/eAccounting/codyClaim/viewCodyClaimPop.do", data, null, true, "viewStaffClaimPop");
}
...
function fn_selectStaffClaimInfo() {
    ...
    Common.ajax("GET", "/eAccounting/codyClaim/selectCodyClaimInfo.do", obj, function(result) {
        console.log(result);
        // Expense Type Name == Car Mileage Expense
        //$("#expTypeName").val() == "Car Mileage Expense"
        // WebInvoice Test는 Test
...
function fn_updateStaffClaimExp(st) {
    ...
    if(gridDataList.length > 0){
        ...
        Common.ajax("POST", "/eAccounting/codyClaim/updateCodyClaimExp.do", gridDataObj, function(result) {
            console.log(result);
            clmNo = result.data.clmNo;
            fn_selectStaffClaimItemList();
            if(st == "view"){
...
function fn_deleteStaffClaimExp() {
	...
	console.log(data);
	Common.ajax("POST", "/eAccounting/codyClaim/deleteCodyClaimExp.do", data, function(result) {
        console.log(result);
...
<h2><spring:message code="codyClaim.title" /></h2>
<ul class="right_btns">
