        		...
        		Common.ajaxFile("/eAccounting/codyClaim/attachFileUpload.do", formData, function(result) {
                    ...
                    for(var i = 0; i < gridDataList.length; i++) {
                        var data = {
                                ...
                                glAccCode : "61130300",
                                glAccCodeName : "TRAVELING CLAIM (CODY)",
                                budgetCode : "05015",
                                budgetCodeName : "H/S services-Travelling",
...
function fn_approveLinePop(memAccId, clmMonth) {
    ...
    Common.ajax("POST", "/eAccounting/staffClaim/checkOnceAMonth.do?_cacheId=" + Math.random(), {clmType:"J7", memAccId:memAccId, clmMonth:clmMonth}, function(result) {
        console.log(result);
        if(result.data > 0) {
...
function fn_checkClmMonthAndMemAccId() {
    var checkResult = true;
    if(FormUtil.isEmpty($("#newMemAccId").val())) {
        Common.alert('Please enter the Cody Code');
        checkResult = false;
        return checkResult;
    }
    if(FormUtil.isEmpty($("#newClmMonth").val())) {
...
<!-- report Form -->
<form id="dataForm">
    <input type="hidden" id="reportFileName" name="reportFileName" value="/e-accounting/codyClaim.rpt" /><!-- Report Name  -->
...
<aside class="link_btns_wrap"><!-- link_btns_wrap start -->
    ...
    <dl class="link_list">
        ...
        <ul class="btns">
            ...
            <li><p class="link_btn"><a href="#" id="_staffClaimBtn">Cody Claim</a></p></li>
            </c:if>
        </ul>
        <ul class="btns">
        </ul>
