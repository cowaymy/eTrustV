...
function fn_popSupplierSearchPop() {
    Common.popupDiv("/eAccounting/webInvoice/supplierSearchPop.do", {pop:"pop",accGrp:"VM08"}, null, true, "supplierSearchPop");
}
...
function fn_PopExpenseTypeSearchPop() {
    Common.popupDiv("/eAccounting/expense/expenseTypeSearchPop.do", {popClaimType:'J8'}, null, true, "expenseTypeSearchPop");
}
...
function fn_mileageAdd() {
	if(AUIGrid.getRowCount(mileageGridID) > 0) {
        AUIGrid.addRow(mileageGridID, {clamUn:AUIGrid.getCellValue(mileageGridID, 0, "clamUn"),cur:"MYR",carMilag:0,carMilagAmt:0,tollAmt:0,parkingAmt:0}, "last");
    } else {
        Common.ajax("GET", "/eAccounting/webInvoice/selectClamUn.do?_cacheId=" + Math.random(), {clmType:"J8"}, function(result) {
            console.log(result);
            AUIGrid.addRow(mileageGridID, {clamUn:result.clamUn,cur:"MYR",carMilag:0,carMilagAmt:0,tollAmt:0,parkingAmt:0}, "last");
        });
    }
}

function fn_popSubSupplierSearchPop() {
    Common.popupDiv("/eAccounting/webInvoice/supplierSearchPop.do", {pop:"sPop",accGrp:"VM08"}, null, true, "supplierSearchPop");
}
...
function fn_selectStaffClaimList() {
	Common.ajax("GET", "/eAccounting/ctClaim/selectCtClaimList.do?_cacheId=" + Math.random(), $("#form_staffClaim").serialize(), function(result) {
        console.log(result);
...
function fn_newStaffClaimPop() {
	Common.popupDiv("/eAccounting/ctClaim/newCtClaimPop.do", {callType:"new"}, null, true, "newStaffClaimPop");
}
...
function fn_checkEmpty() {
    ...
    if(FormUtil.isEmpty($("#newMemAccName").val())) {
        Common.alert('<spring:message code="ctClaim.ctCode.msg" />');
        checkResult = false;
        return checkResult;
    }
...
function fn_addMyGridRow() {
    if(AUIGrid.getRowCount(myGridID) > 0) {
        AUIGrid.addRow(myGridID, {clamUn:AUIGrid.getCellValue(myGridID, 0, "clamUn"),cur:"MYR",gstBeforAmt:0,gstAmt:0,taxNonClmAmt:0,totAmt:0}, "last");
    } else {
        Common.ajax("GET", "/eAccounting/webInvoice/selectClamUn.do?_cacheId=" + Math.random(), {clmType:"J8"}, function(result) {
            console.log(result);
            AUIGrid.addRow(myGridID, {clamUn:result.clamUn,cur:"MYR",gstBeforAmt:0,gstAmt:0,taxNonClmAmt:0,totAmt:0}, "last");
        });
    }
}
...
function fn_addRow() {
    ...
    if(fn_checkEmpty()) {
        ...
        if($(":input:radio[name=expGrp]:checked").val() == "1") {
        	...
        	if(clmSeq == 0) {
        		Common.ajaxFile("/eAccounting/ctClaim/attachFileUpload.do", formData, function(result) {
                    console.log(result);
        		...
        		Common.ajaxFile("/eAccounting/ctClaim/attachFileUpdate.do", formData, function(result) {
                    console.log(result);
...
function fn_insertStaffClaimExp(st) {
    ...
    if(gridDataList.length > 0){
        ...
        console.log(data);
        Common.ajax("POST", "/eAccounting/ctClaim/insertCtClaimExp.do", data, function(result) {
            console.log(result);
...
function fn_selectStaffClaimItemList() {
    ...
    Common.ajax("GET", "/eAccounting/ctClaim/selectCtClaimItemList.do?_cacheId=" + Math.random(), obj, function(result) {
        console.log(result);
...
function fn_viewStaffClaimPop(clmNo) {
    ...
    Common.popupDiv("/eAccounting/ctClaim/viewCtClaimPop.do", data, null, true, "viewStaffClaimPop");
}
...
function fn_selectStaffClaimInfo() {
    ...
    Common.ajax("GET", "/eAccounting/ctClaim/selectCtClaimInfo.do", obj, function(result) {
        console.log(result);
        // Expense Type Name == Car Mileage Expense
        //$("#expTypeName").val() == "Car Mileage Expense"
        // WebInvoice Test는 Test
...
function fn_updateStaffClaimExp(st) {
    ...
    if(gridDataList.length > 0){
        ...
        Common.ajax("POST", "/eAccounting/ctClaim/updateCtClaimExp.do", gridDataObj, function(result) {
            console.log(result);
            clmNo = result.data.clmNo;
            fn_selectStaffClaimItemList();
            if(st == "view"){
...
function fn_approveLinePop() {
    ...
    Common.popupDiv("/eAccounting/ctClaim/approveLinePop.do", null, null, true, "approveLineSearchPop");
}
...
function fn_deleteStaffClaimExp() {
	...
	console.log(data);
	Common.ajax("POST", "/eAccounting/ctClaim/deleteCtClaimExp.do", data, function(result) {
        console.log(result);
...
<h2><spring:message code="ctClaim.title" /></h2>
<ul class="right_btns">
