...
function fn_uploadCsvFile(){
	if(fn_uploadValid()){
		...
		Common.ajaxFile("/eAccounting/smGmClaim/uploadEntitlement", formData, function (result) {
			fn_searchUpload();
            $('#entClose').click();
            //document.newForm.reset();
            Common.alert('<spring:message code="pay.alert.entitlement.new.success" arguments="'+result+'" htmlEscape="false"/>');
        });

		/* Common.ajax("GET", "/commission/calculation/csvFileOverlapCntNonIncnt", $("#newForm").serialize(), function(result) {
			if(result>0){
			   ...
			   var formData = new FormData();
		        ...
		        Common.ajaxFile("/commission/csv/uploadNonIncnt", formData, function (result) {
		        	$("#search").click();
		        	document.newForm.reset();
		        	Common.alert('<spring:message code="commission.alert.nonIncentive.new.success" arguments="'+result+'" htmlEscape="false"/>');
		        });
			}
		}); */
	}
...
<div id="popup_wrap" class="popup_wrap size_mid">
	...
	<header class="pop_header">
		<h1>Monthly Entitlementt - NEW UPLOAD</h1>
		<ul class="right_opt">
			<li><p class="btn_blue2"><a href="#" id="entClose"><spring:message code='sys.btn.close'/></a></p></li>
		</ul>
	</header>
	...
	<section class="pop_body">
		...
		<ul class="center_btns">
			...
			<li><p class="btn_blue"><a href="${pageContext.request.contextPath}/resources/download/eAccounting/SmGmEntitlement_format.csv"><spring:message code='commission.button.dwCsvFormat'/></a></p></li>
		</ul>
