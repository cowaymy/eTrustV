...
<script type="text/javaScript">
	...
	$(document).ready(
			function() {
				...
				AUIGrid.bind(myGridID, "cellDoubleClick", function(event) {
					$("#groupBatchId").val(event.item.groupBatchId);
					Common.popupDiv("/organization/groupDetailViewPop.do", $(
							"#searchForm").serializeJSON(), null, true,
							'detailPop');
				});
	...
	function createAUIGrid() {
		...
		var columnLayout = [ {
			...
			dataField : "groupTotItm",
			headerText : "Total",
			width : 130,
			editable : false,
			style : 'left_style'
		}, {
			dataField : "groupTotSucces",
	...
	function fn_searchLoyaltyHpListAjax() {
		Common.ajax("GET", "/organization/selectGroupMstList", $("#searchForm")
	...
	function fn_NewUpload() {
		// Common.popupDiv("/sales/customer/loyaltyHpUploadPop.do", $("#searchForm").serializeJSON(), null, true, 'savePop');

		Common.popupDiv("/organization/groupUploadPop.do", $("#searchForm")
				.serializeJSON(), null, true, 'savePop');
	}
...
<section id="content">
	...
	<aside class="title_line">
		...
		<ul class="right_btns">
			<li><p class="btn_blue">
					<a href="#" onClick="fn_NewUpload()">New Upload</a>
				</p></li>
			<li><p class="btn_blue">
					<a href="#" onClick="fn_searchLoyaltyHpListAjax()"><span
						class="search"></span>
			...
			<table class="type1">
				...
				<tbody>
					<tr>
						...
						<td><input id="groupBatchId" name="groupBatchId" type="text"
							title="groupBatchId" placeholder="" class="w100p" /></td>
						<th scope="row"><spring:message
								code="sal.title.text.batchStus" /></th>
						<td><select id="groupBatchStus" name="groupBatchStus"
							class="w100p">
								<option value="1"><spring:message
										code="sal.combo.text.active" /></option>
								<option value="4" selected><spring:message
										code="sal.combo.text.compl" /></option>
								<option value="8"><spring:message
										code="sal.combo.text.inactive" /></option>
						</select></td>
						<th scope="row"><spring:message code="sal.text.creator" /></th>
						<td><input id="crtUserName" name="crtUserName" type="text"
							title="crtUserName" placeholder="Upload (Username)" class="w100p" />
						</td>
						<th scope="row"></th>
						<td></td>
					</tr>
					...
					<tr>
						...
						<td><input id="uploadDateF" name="uploadDateF" type="text"
							title="Upload Date(From)" placeholder="DD/MM/YYYY" class="j_date" />
						</td>
						<th scope="row">Create Date(To)</th>
						<td><input id="uploadDateT" name="uploadDateT" type="text"
							title="Upload Date(To)" placeholder="DD/MM/YYYY" class="j_date" />
						</td>
						<th scope="row">Member Type</th>
						<td><select id="groupMemberType" name="groupMemberType"
							class="w100p">
								<option value="">--SELECT--</option>
								<option value="Health Planner">Health Planner</option>
								<option value="Coway Lady">Coway Lady</option>
								<option value="Coway Technician">Coway Technician</option>
								<option value="Homecare Technician">Homecare Technician</option>
								<option value="Homecare Delivery Technician">Homecare
									Delivery Technician</option>
						</select></td>
						<th scope="row"></th>
						<td></td>
					</tr>
    ...
    <td>
			...
			<aside class="link_btns_wrap">
				...
				<dl class="link_list">
					...
					<dd>
						<ul class="btns">
							<!-- <li><p class="link_btn"><a href="#" onclick="javascript:fn_confirmPopup();">Confirm Upload</a></p></li> -->
