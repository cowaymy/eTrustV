...
$(document).ready(function () {
    viewGridID = AUIGrid.create("#group_view_grid_wrap", viewColumnLayout, viewGridPros);

    $("#close_btn").click(fn_closePop);

    $('#excelDown').click(function() {
        GridCommon.exportTo("group_view_grid_wrap", 'xlsx', 'Group Transfer File Upload List');
    });

    AUIGrid.setGridData(viewGridID, $.parseJSON('${groupBatchItem}'));
});
...
function fn_confirm(){
	...
	if('${groupBatchInfo.stusCode}' == 'Completed' || '${groupBatchInfo.stusCode}' == 'Inactive'){
		Common.alert("Cannot Proceed. Batch status is Completed or Inactive.");

	} else {

        Common.ajax("GET", "/organization/groupConfirm", $("#form_GroupView").serializeJSON() , function(result) {
...
function fn_reject() {
	   if('${groupBatchInfo.stusCode}' == 'Completed' || '${groupBatchInfo.stusCode}' == 'Inactive'){
	        ...
	        Common.ajax("GET", "/organization/groupReject", $("#form_GroupView").serializeJSON() , function(result) {
...
<input type="hidden" name="groupBatchId" id="groupBatchId" value="${groupBatchInfo.groupBatchId }">
...
<article class="grid_wrap" id="group_view_grid_wrap"><!-- grid_wrap start -->
