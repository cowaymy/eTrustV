...
<script type="text/javaScript">
	...
	$(document).ready(function() {
		...
		$("#search_22T").click(function(){  
			Common.ajax("GET", "/commission/calculation/selectDataCMM022T", $("#form_22").serialize(), function(result) {
				console.log("성공.");
   ...
   function fn_downFile() {
	   Common.ajax("GET", "/commission/calculation/cntCMM0022T", $("#form_22").serialize(), function(result) {
           ...
           if(cnt > 0){
		       ...
		       Common.showLoader();
              $.fileDownload("/commExcelFile.do?fileName=" + fileName + "&year="+year+"&month="+month+"&code="+code+"&ordId="+ordId+"&salesPersonCd="+salesPersonCd+"&useYnCombo="+useYnCombo+"&codeId="+codeId)
              .done(function () {
   ...
   function fn_AlldownFile() {
	   var data = { "searchDt" : $("#CMM0022T_Dt").val() , "code": $("#code_22T").val(), "codeId": $("#orgGroup_22").val() };
	   Common.ajax("GET", "/commission/calculation/cntCMM0022T", data, function(result) {
           var cnt = result;
           if(cnt > 0){
        	   var fileName = $("#fileName").val() +"_"+today;
               fileName=fileName+".xlsx";
...
<div id="popup_wrap2" class="popup_wrap"><!-- popup_wrap start -->
	...
	<section class="pop_body" style="max-height:600px;"><!-- pop_body start -->
	   <aside class="title_line"><!-- title_line start -->
		...
		<form id="form_22">
		   <input type="hidden" name="code" id="code_22T" value="${code}"/>
		   <input type="hidden" id="fileName" name="fileName" value="RentalMembership.xlsx"/>
		   <ul class="right_btns">
			  <li><p class="btn_blue"><a href="#" id="search_22T"><span class="search"></span><spring:message code='sys.btn.search'/></a></p></li>
			</ul>
