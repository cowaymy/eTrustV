...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_60T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_60T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_60T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      Common
                          .ajax(
                              "GET",
                              "/commission/calculation/selectDataCMM060T",
                              $("#form_60")
                                  .serialize(),
                              function(result) {
                                console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_60T.do",
        GridCommon.getEditData(myGridID_60T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    ...
    myGridID_60T = AUIGrid
        .create("#grid_wrap_60T", columnLayout3, gridPros);
  }
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_60T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0060T",
            $("#form_60").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
  ...
  function fn_AlldownFile() {
    var data = {
      "searchDt" : $("#CMM0060T_Dt").val(),
      "code" : $("#code_60T").val(),
      "codeId" : $("#orgGroup_60").val()
    };
    Common
        .ajax(
            "GET",
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_60">
   <input type="hidden" name="code" id="code_60T" value="${code}" /> <input
    type="hidden" id="fileName" name="fileName" value="Award.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_60T"><span class="search"></span>
   ...
   <table class="type1 mt10">
    ...
    <tbody>
     <tr>
      ...
      <td><input type="text" id="emplyCd_60T" name="emplyCd"
     ...
     <!-- <tr>
      ...
      <td colspan=5><select id="useYnCombo_60T" name="useYnCombo"
       style="width: 100px;">
