...
<script type="text/javaScript">
    ...
    $(document).ready(function() {
        ...
        // ready 이벤트 바인딩
        AUIGrid.bind(myGridID_08T, "ready", function(event) {
            gridDataLength = AUIGrid.getGridData(myGridID_08T).length; // 그리드 전체 행수 보관
        });
        ...
        // 헤더 클릭 핸들러 바인딩
        AUIGrid.bind(myGridID_08T, "headerClick", function(event) {
        ...
        // 셀 수정 완료 이벤트 바인딩
        AUIGrid.bind(myGridID_08T, "cellEditEnd", function(event) {
            
            // isActive 칼럼 수정 완료 한 경우
            if(event.dataField == "isExclude") {
                
                // 그리드 데이터에서 isActive 필드의 값이 Active 인 행 아이템 모두 반환 
                var activeItems = AUIGrid.getItemsByValue(myGridID_08T, "isExclude", "1");
    ...
    function fn_saveExculde(){
        Common.ajax("POST", "/commission/calculation/updatePrdData_08T.do", GridCommon.getEditData(myGridID_08T) , function(result) {
            // 공통 메세지 영역에 메세지 표시.
   ...
   function checkAll(isChecked) {
       ...
       if(isChecked){   // checked == true == 1
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_08T, { "isExclude" : 1 }, i);
         }
       }else{   // unchecked == false == 0
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_08T, { "isExclude" : 0 }, i);
         }
       }
   ...
   function fn_AlldownFile() {
	   var data = { "searchDt" : $("#CMM0008T_Dt").val() , "code": $("#code_08T").val(), "codeId": $("#orgGroup_08").val() };
