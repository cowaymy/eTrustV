...
<script type="text/javaScript">
    ...
    $(document).ready(function() {
        ...
        // ready 이벤트 바인딩
        AUIGrid.bind(myGridID_20T, "ready", function(event) {
            gridDataLength = AUIGrid.getGridData(myGridID_20T).length; // 그리드 전체 행수 보관
        });
        
        // cellClick event.
        AUIGrid.bind(myGridID_20T, "cellClick", function(event) {
              console.log("rowIndex : " + event.rowIndex + ", columnIndex : " + event.columnIndex + " clicked");          
        });
         
        // 헤더 클릭 핸들러 바인딩
        AUIGrid.bind(myGridID_20T, "headerClick", function(event) {
        ...
        $("#search_20T").click(function(){  
            Common.ajax("GET", "/commission/calculation/selectDataCMM020T", $("#form_20T").serialize(), function(result) {
                console.log("성공.");
                console.log("data : " + result);
                AUIGrid.setGridData(myGridID_20T, result);
                //AUIGrid.addCheckedRowsByValue(myGridID_20T, "isExclude", "1");
            });
        });
        
        $("#save_20T").click(function(){  
    ...
    function fn_saveExculde(){
        Common.ajax("POST", "/commission/calculation/updatePrdData_20T.do", GridCommon.getEditData(myGridID_20T) , function(result) {
            // 공통 메세지 영역에 메세지 표시.
   ...
   function checkAll(isChecked) {
       ...
       if(isChecked){   // checked == true == 1
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_20T, { "isExclude" : 1 }, i);
         }
       }else{   // unchecked == false == 0
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_20T, { "isExclude" : 0 }, i);
         }
       }
   ...
   function fn_downFile() {
       Common.ajax("GET", "/commission/calculation/cntCMM0020T", $("#form_20T").serialize(), function(result) {
           var cnt = result;
           if(cnt > 0){
   ...
   function fn_AlldownFile() {
	   var data = { "searchDt" : $("#CMM0020T_Dt").val() , "code": $("#code_20T").val()};
	   Common.ajax("GET", "/commission/calculation/cntCMM0020T", data, function(result) {
           var cnt = result;
           if(cnt > 0){
        	   var fileName = $("#fileName").val() +"_"+today;
               fileName=fileName+".xlsx";
    ...
    <section class="pop_body" style="max-height:600px;"><!-- pop_body start -->
       <aside class="title_line"><!-- title_line start -->
            ...
            <table class="type1 mt10"><!-- table start -->
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                        <input type="text" title="Create start Date" placeholder="DD/MM/YYYY" name="searchDt" id="CMM0020T_Dt" class="j_date2" value="${searchDt_pop }" />
                        </td>
                        <th scope="row"><spring:message code='commissiom.text.excel.ordId'/></th>
                        <td>
                              <input type="text" id="ordId_20T" name="ordId" style="width: 100px;" maxlength="10" onkeydown="onlyNumber(this)">
                        </td>
                        <th scope="row"><spring:message code='commission.text.asEntryCd'/></th>
                        <td>
        ...
        <article class="grid_wrap3"><!-- grid_wrap start -->
            ...
            <!-- grid_wrap start -->
            <div id="grid_wrap_20" style="width: 100%; height: 334px; margin: 0 auto;"></div>
