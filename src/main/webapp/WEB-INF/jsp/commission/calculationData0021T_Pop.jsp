...
<script type="text/javaScript">
    ...
    function fn_saveExculde(){
    	var checkdata = AUIGrid.getCheckedRowItemsAll(myGridID_21T);
        var check     = AUIGrid.getCheckedRowItems(myGridID_21T);
        var formList = $("#form_21").serializeJSON();       //폼 데이터
        ...
        Common.ajax("POST", "/commission/calculation/updatePrdData_21T.do", data , function(result) {
            // 공통 메세지 영역에 메세지 표시.
   ...
   function fn_downFile() {
	   Common.ajax("GET", "/commission/calculation/cntCMM0021T", $("#form_21").serialize(), function(result) {
           ...
           if(cnt > 0){
		       ...
		       var retId = $("#retId_21T").val();
		       var useYnCombo = $("#useYnCombo_21T").val();
		       //window.open("<c:url value='/sample/down/excel-xls.do?aaa=" + fileName + "'/>");
		       //window.open("<c:url value='/sample/down/excel-xlsx.do?aaa=" + fileName + "'/>");
		       window.location.href="<c:url value='/commission/down/excel-xlsx-streaming.do?fileName=" + fileName + "&year="+year+"&month="+month+"&code="+code+"&ordId="+ordId+"&retId="+retId+"&useYnCombo="+useYnCombo+"'/>";
	       }else{
            ...
            <table class="type1 mt10"><!-- table start -->
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                              <input type="text" id="retId_21T" name="retId" style="width: 100px;" maxlength="10" >
                        </td>
                     </tr>
                     <tr>
