...
<script type="text/javaScript">
    ...
    $(document).ready(function() {
        ...
        // ready 이벤트 바인딩
        AUIGrid.bind(myGridID_07T, "ready", function(event) {
            gridDataLength = AUIGrid.getGridData(myGridID_07T).length; // 그리드 전체 행수 보관
        });
        ...
        // 헤더 클릭 핸들러 바인딩
        AUIGrid.bind(myGridID_07T, "headerClick", function(event) {
        ...
        // 셀 수정 완료 이벤트 바인딩
        AUIGrid.bind(myGridID_07T, "cellEditEnd", function(event) {
            
            // isActive 칼럼 수정 완료 한 경우
            if(event.dataField == "isExclude") {
                
                // 그리드 데이터에서 isActive 필드의 값이 Active 인 행 아이템 모두 반환 
                var activeItems = AUIGrid.getItemsByValue(myGridID_07T, "isExclude", "1");
    ...
    function fn_saveExculde(){
        Common.ajax("POST", "/commission/calculation/updatePrdData_07T.do", GridCommon.getEditData(myGridID_07T) , function(result) {
            // 공통 메세지 영역에 메세지 표시.
   ...
   function checkAll(isChecked) {
       ...
       if(isChecked){   // checked == true == 1
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_07T, { "isExclude" : 1 }, i);
         }
       }else{   // unchecked == false == 0
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_07T, { "isExclude" : 0 }, i);
         }
       }
   ...
   function fn_downFile() {
	   Common.ajax("GET", "/commission/calculation/cntCMM0007T", $("#form_07T").serialize(), function(result) {
           ...
           if(cnt > 0){
		       ...
		       Common.showLoader();
                $.fileDownload("/commExcelFile.do?fileName=" + fileName + "&year="+year+"&month="+month+"&code="+code+"&codeId="+codeId+"&memberCd="+memberCd+"&ordId="+ordId+"&useYnCombo="+useYnCombo)
                .done(function () {
   ...
   function fn_AlldownFile() {
	   var data = { "searchDt" : $("#CMM0007T_Dt").val() , "code": $("#code_07T").val(), "codeId": $("#codeId_07T").val() };
	   Common.ajax("GET", "/commission/calculation/cntCMM0007T", data, function(result) {
		   var cnt = result;
           if(cnt > 0){
        	   var fileName = $("#fileName").val() +"_"+today;
                fileName=fileName+".xlsx";
    ...
    <section class="pop_body" style="max-height:600px;"><!-- pop_body start -->
       <aside class="title_line"><!-- title_line start -->
        ...
        <form id="form_07T">
           ...
           <input type="hidden" id="fileName" name="fileName" value="outrightInstallment.xlsx"/>
           <ul class="right_btns">
            ...
            <table class="type1 mt10"><!-- table start -->
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                              <input type="text" id="memberCd_07T" name="memberCd" style="width: 100px;" maxlength="10">
