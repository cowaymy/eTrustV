...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_18T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_18T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_18T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      Common
                          .ajax(
                              "GET",
                              "/commission/calculation/selectDataCMM018T",
                              $("#form_18T")
                                  .serialize(),
                              function(result) {
                                console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_18T.do",
        GridCommon.getEditData(myGridID_18T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    var columnLayout3 = [
        ...
        {
          dataField : "instId",
          headerText : "<spring:message code='commissiom.text.excel.instId'/>",
          style : "my-column",
          editable : false
        },
        {
          dataField : "instNo",
          headerText : "<spring:message code='commissiom.text.excel.instNo'/>",
          style : "my-column",
          editable : false
        },
        {
        ...
        },
        {
          ...
          headerText : "<spring:message code='commissiom.text.excel.InstPersonId'/>",
          style : "my-column",
          editable : false
        },
        {
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_18T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0018T",
            $("#form_18T").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
  ...
  function fn_AlldownFile() {
    var data = {
      "searchDt" : $("#CMM0018T_Dt").val(),
      "code" : $("#code_18T").val(),
      "actionType" : $("#actionType18T").val()
    };
    Common
        .ajax(
            "GET",
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_18T">
   <input type="hidden" name="actionType" id="actionType18T"
    value="${actionType }" /> <input type="hidden" name="code"
    id="code_18T" value="${code}" /> <input type="hidden" id="fileName"
    name="fileName" value="InstallationSales.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_18T"><span class="search"></span>
    ...
    <tbody>
     <tr>
      ...
      <td><input type="text" title="Create start Date"
       placeholder="DD/MM/YYYY" name="searchDt" id="CMM0018T_Dt"
       class="j_date2" value="${searchDt_pop }" /></td>
      <th scope="row"><spring:message
        code='commissiom.text.excel.ordId' /></th>
      <td><input type="text" id="ordId_18T" name="ordId"
       style="width: 100px;" maxlength="10" onkeydown="onlyNumber(this)">
      </td>
      <th scope="row"><spring:message
        code='commission.text.instPersonCd' /></th>
      <td><input type="text" id="instPersonCd_18T"
       name="instPersonCd" style="width: 100px;" maxlength="10">
     ...
     <!-- <tr>
      ...
      <td colspan="5"><select id="useYnCombo_18T" name="useYnCombo"
       style="width: 100px;">
