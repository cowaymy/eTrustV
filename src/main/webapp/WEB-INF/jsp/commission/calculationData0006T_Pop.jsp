...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_06T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_06T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_06T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      Common
                          .ajax(
                              "GET",
                              "/commission/calculation/selectDataCMM006T",
                              $("#form_06T")
                                  .serialize(),
                              function(result) {
                                console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_06T.do",
        GridCommon.getEditData(myGridID_06T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    ...
    myGridID_06T = AUIGrid
        .create("#grid_wrap_06T", columnLayout3, gridPros);
  }
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_06T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    ...
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0006T",
            $("#form_06T").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
  ...
  function fn_AlldownFile() {
    var data = {
      "searchDt" : $("#CMM0006T_Dt").val(),
      "code" : $("#code_06T").val(),
      "codeId" : $("#codeGroupId").val()
    };
    Common
        .ajax(
            "GET",
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_06T">
   <input type="hidden" name="code" id="code_06T" value="${code}" /> <input
    type="hidden" id="fileName" name="fileName"
    value="organizationHierarchy" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_06T"><span class="search"></span>
   ...
   <table class="type1 mt10">
    ...
    <tbody>
     <tr>
      ...
      <td><input type="text" id="memberCd_06T" name="memberCd"
     ...
     <!-- <tr>
      ...
      <td colspan="5"><select id="useYnCombo_06T" name="useYnCombo"
       style="width: 100px;">
