...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_12T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_12T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_12T, "cellClick",
                function(event) {
            ...
            $("#search_12T")
                .click(
                    function() {
                      var clctrCd = $("#clctrCd_12T")
                          .val();

                      if (clctrCd == ""
                          || clctrCd == null) {
                        Common
                            .alert("<spring:message code='sys.msg.necessary' arguments='clctr Cd' htmlEscape='false'/>");
                        return false;
                      } else {
                        Common
                            .ajax(
                                "GET",
                                "/commission/calculation/selectDataCMM012T",
                                $("#form_12T")
                                    .serialize(),
                                function(result) {
                                  console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_12T.do",
        GridCommon.getEditData(myGridID_12T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    ...
    myGridID_12T = AUIGrid
        .create("#grid_wrap_12T", columnLayout3, gridPros);
  }
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_12T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    ...
    if (clctrCd == "" || clctrCd == null) {
      ...
      Common
          .ajax(
              "GET",
              "/commission/calculation/cntCMM0012T",
              $("#form_12T").serialize(),
              function(result) {
                var cnt = result;
                if (cnt > 0) {
  ...
  function fn_AlldownFile() {
    var data = {
      "searchDt" : $("#CMM0012T_Dt").val(),
      "code" : $("#code_12T").val(),
      "codeId" : $("#orgGroup_12").val()
    };
    Common
        .ajax(
            "GET",
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_12T">
   <input type="hidden" name="code" id="code_12T" value="${code}" /> <input
    type="hidden" id="fileName" name="fileName"
    value="rentalCollection.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_12T"><span class="search"></span>
   ...
   <table class="type1 mt10">
    ...
    <tbody>
     <tr>
      ...
      <th scope="row"><spring:message
        code='commission.text.collectorCd' /><span class="must">*</span></th>
      <td><input type="text" id="clctrCd_12T" name="clctrCd"
     ...
     <tr>
      ...
      <td colspan='5'><input type="text" id="ordId_12T" name="ordId"
       style="width: 100px;" maxlength="10" onkeydown="onlyNumber(this)">
      </td>
      <!-- <th scope="row"><spring:message
        code='commission.text.isExclude' /></th>
      <td colspan="3"><select id="useYnCombo_12T" name="useYnCombo"
