...
var itemGridID;
	...
	function fn_itemDetailSearch(val){
		if(val == "0"){
			$("#vStusId").val("");
			Common.ajax("GET", "/commission/calculation/nonIncentiveItemList", $("#conForm").serializeJSON(), function(result) {
	...
	//item Add Pop
	function fn_itemAdd(){
		Common.popupDiv("/commission/calculation/commNonInctivItemAddPop.do",$("#conForm").serializeJSON());
	}

	//item RemoveAjax
	function fn_removeItem(){
		Common.ajax("POST", "/commission/calculation/removeNonIncentiveItem.do", GridCommon.getEditData(itemGridID) , function(result) {
	...
	function fn_deactivate(){
		...
		Common.ajax("GET", "/commission/calculation/nonIncentiveDeactivateCheck", $("#conForm").serializeJSON() , function(result) {
			if(result > 0){
				Common.ajax("GET", "/commission/calculation/nonIncentiveDeactivate", $("#conForm").serializeJSON() , function(result) {
	...
	function fn_confirm(){
		if(Number($("#cntValid").val()) < 1){
			...
			Common.ajax("GET", "/commission/calculation/nonIncentiveConfirm", $("#conForm").serializeJSON() , function(result) {
...
<h1><spring:message code='commission.title.pop.head.nonIncentiveConfirm'/></h1>
<ul class="right_opt">
