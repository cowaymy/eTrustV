...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_19T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_19T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_19T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      Common
                          .ajax(
                              "GET",
                              "/commission/calculation/selectDataCMM019T",
                              $("#form_19T")
                                  .serialize(),
                              function(result) {
                                console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_19T.do",
        GridCommon.getEditData(myGridID_19T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_19T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0019T",
            $("#form_19T").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_19T">
   <input type="hidden" name="actionType" id="actionType19T"
    value="${actionType }" /> <input type="hidden" name="code"
    id="code_19T" value="${code}" /> <input type="hidden" id="fileName"
    name="fileName" value="BSsalse.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_19T"><span class="search"></span>
    ...
    <tbody>
     <tr>
      ...
      <td><input type="text" title="Create start Date"
       placeholder="DD/MM/YYYY" name="searchDt" id="CMM0019T_Dt"
       class="j_date2" value="${searchDt_pop }" /></td>
      <th scope="row"><spring:message
        code='commissiom.text.excel.ordId' /></th>
      <td><input type="text" id="ordId_19T" name="ordId"
       style="width: 100px;" maxlength="10" onkeydown="onlyNumber(this)">
      </td>
      <th scope="row"><spring:message
        code='commission.text.bsPersonCd' /></th>
      <td><input type="text" id="bsPersonCd_19T" name="bsPersonCd"
     ...
     <!-- <tr>
      ...
      <td colspan="5"><select id="useYnCombo_19T" name="useYnCombo"
       style="width: 100px;">
