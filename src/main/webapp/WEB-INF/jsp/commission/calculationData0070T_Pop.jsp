...
<script type="text/javaScript">
    ...
    $(document).ready(function() {
        ...
        // ready 이벤트 바인딩
        AUIGrid.bind(myGridID_70T, "ready", function(event) {
            gridDataLength = AUIGrid.getGridData(myGridID_70T).length; // 그리드 전체 행수 보관
        });
        
        // cellClick event.
        AUIGrid.bind(myGridID_70T, "cellClick", function(event) {
              console.log("rowIndex : " + event.rowIndex + ", columnIndex : " + event.columnIndex + " clicked");          
        });
        
     // 헤더 클릭 핸들러 바인딩
        AUIGrid.bind(myGridID_70T, "headerClick", function(event) {
        ...
        $("#search_70T").click(function(){
            Common.ajax("GET", "/commission/calculation/selectDataCMM070T", $("#form_70").serialize(), function(result) {
                console.log("성공.");
                console.log("data : " + result);
                AUIGrid.setGridData(myGridID_70T, result);
                AUIGrid.addCheckedRowsByValue(myGridID_70T, "isExclude", "1");
            });
        });
        
        $("#save_70T").click(function(){  
           	Common.confirm("<spring:message code='sys.common.alert.save'/>",fn_saveExculde);
        });
        
        $('#memBtn').click(function() {
            //Common.searchpopupWin("searchForm", "/common/memberPop.do","");
            Common.popupDiv("/common/memberPop.do", $("#form_70").serializeJSON(), null, true);
        });
    ...
    function fn_saveExculde(){
        Common.ajax("POST", "/commission/calculation/updatePrdData_70T.do", GridCommon.getEditData(myGridID_70T) , function(result) {
            // 공통 메세지 영역에 메세지 표시.
   ...
   function createAUIGrid() {
    ...
    myGridID_70T = AUIGrid.create("#grid_wrap_70T", columnLayout3,gridPros);
   }
   ...
   function checkAll(isChecked) {
       ...
       if(isChecked){   // checked == true == 1
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_70T, { "isExclude" : 1 }, i);
         }
       }else{   // unchecked == false == 0
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_70T, { "isExclude" : 0 }, i);
         }
       }
   ...
   function fn_downFile() {
       Common.ajax("GET", "/commission/calculation/cntCMM0070T", $("#form_70").serialize(), function(result) {
           var cnt = result;
           if(cnt > 0){
   ...
   function fn_AlldownFile() {
	   var data = { "searchDt" : $("#CMM0070T_Dt").val() , "code": $("#code_70T").val(), "codeId": $("#orgGroup_70").val() };
	   Common.ajax("GET", "/commission/calculation/cntCMM0070T", data, function(result) {
           var cnt = result;
           if(cnt > 0){
        	   var fileName = $("#fileName").val() +"_"+today;
               fileName=fileName+".xlsx";
...
<div id="popup_wrap2" class="popup_wrap"><!-- popup_wrap start -->
    ...
    <section class="pop_body" style="max-height:600px;"><!-- pop_body start -->
       <aside class="title_line"><!-- title_line start -->
        ...
        <form id="form_70">
           <input type="hidden" name="code" id="code_70T" value="${code}"/>
           <input type="hidden" id="fileName" name="fileName" value="Award.xlsx"/>
           <ul class="right_btns">
              <li><p class="btn_blue"><a href="#" id="search_70T"><span class="search"></span><spring:message code='sys.btn.search'/></a></p></li>
            </ul>
        ...
        <article class="grid_wrap3"><!-- grid_wrap start -->
            ...
            <!-- grid_wrap start -->
            <div id="grid_wrap_70T" style="width: 100%; height: 334px; margin: 0 auto;"></div>
