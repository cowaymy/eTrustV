...
/* 인풋 파일(멀티) end */
	...
	function fn_sampleFormat(val){
		if(val == null || val ==""){
			...
			//Common.alert('No Sample to show for this upload type.');
			  ...
			  Common.popupDiv("/commission/calculation/advanceUploadSamplePop.do");
		};
	}
	...
	function fn_uploadValid(){
        ...
        if( $("#cmbMemberType").val() == null || $("#cmbMemberType").val() == ""){
            Common.alert('<spring:message code="sys.msg.first.Select" arguments="member type" htmlEscape="false"/>');
            //Common.alert("Please select the member type first.");
            return false;
        }
	...
	function fn_uploadCsvFile(){
		if(fn_uploadValid()){
			Common.ajax("GET", "/commission/calculation/csvFileOverlapCntAdvIncnt", $("#newForm").serialize(), function(result) {
				if(result>0){
			        ...
			        Common.ajaxFile("/commission/csv/uploadAdvIncnt", formData, function (result) {
			        	$("#search").click();
