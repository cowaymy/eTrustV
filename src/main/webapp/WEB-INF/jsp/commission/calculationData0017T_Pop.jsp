...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_17T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_17T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_17T, "cellClick",
                function(event) {
            ...
            $("#search_17T")
                .click(
                    function() {
                      ...
                      if (memCd == "" || memCd == null) {
                        Common
                            .alert("<spring:message code='sys.msg.necessary' arguments='mem Cd' htmlEscape='false'/>");
                        return false;
                      } else {
                        Common
                            .ajax(
                                "GET",
                                "/commission/calculation/selectDataCMM017T",
                                $("#form_17T")
                                    .serialize(),
                                function(result) {
                                  console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_17T.do",
        GridCommon.getEditData(myGridID_17T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    ...
    myGridID_17T = AUIGrid
        .create("#grid_wrap_17T", columnLayout3, gridPros);
  }
  ...
  function fn_downFile() {
    ...
    if (memCd == "" || memCd == null) {
      ...
      Common
          .ajax(
              "GET",
              "/commission/calculation/cntCMM0017T",
              $("#form_17T").serialize(),
              function(result) {
                var cnt = result;
                if (cnt > 0) {
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_17T">
   <input type="hidden" name="code" id="code_17T" value="${code}" /> <input
    type="hidden" id="fileName" name="fileName" value="SHIindex.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_17T"><span class="search"></span>
   ...
   <table class="type1 mt10">
    ...
    <tbody>
     <tr>
      ...
      <th scope="row"><spring:message
        code='commission.text.search.memCode' /><span class="must">*</span></th>
      <td><input type="text" id="memCd_17T" name="memCd"
     ...
     <tr>
      ...
      <td colspan='5'><input type="text" id="ordId_17T" name="ordId"
       style="width: 100px;" maxlength="10" onkeydown="onlyNumber(this)">
      </td>
      <!-- <th scope="row"><spring:message
        code='commission.text.isExclude' /></th>
      <td colspan=3><select id="useYnCombo_17T" name="useYnCombo"
       style="width: 100px;">
