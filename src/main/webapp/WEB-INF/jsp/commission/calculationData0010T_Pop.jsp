...
<script type="text/javaScript">
    ...
    $(document).ready(function() {
        ...
        // ready 이벤트 바인딩
        AUIGrid.bind(myGridID_10T, "ready", function(event) {
            gridDataLength = AUIGrid.getGridData(myGridID_10T).length; // 그리드 전체 행수 보관
        });
        
        // cellClick event.
        AUIGrid.bind(myGridID_10T, "cellClick", function(event) {
              console.log("rowIndex : " + event.rowIndex + ", columnIndex : " + event.columnIndex + " clicked");          
        });
        
        // 헤더 클릭 핸들러 바인딩
        AUIGrid.bind(myGridID_10T, "headerClick", function(event) {
        ...
        // 셀 수정 완료 이벤트 바인딩
        AUIGrid.bind(myGridID_10T, "cellEditEnd", function(event) {
            
            // isActive 칼럼 수정 완료 한 경우
            if(event.dataField == "isExclude") {
                
                // 그리드 데이터에서 isActive 필드의 값이 Active 인 행 아이템 모두 반환 
                var activeItems = AUIGrid.getItemsByValue(myGridID_10T, "isExclude", "1");
        ...
        $("#search_10T").click(function(){  
            Common.ajax("GET", "/commission/calculation/selectDataCMM010T", $("#form_10T").serialize(), function(result) {
                console.log("성공.");
                console.log("data : " + result);
                AUIGrid.setGridData(myGridID_10T, result);
                //AUIGrid.addCheckedRowsByValue(myGridID_10T, "isExclude", "1");
            });
        });
        
        $("#save_10T").click(function(){  
           	Common.confirm("<spring:message code='sys.common.alert.save'/>",fn_saveExculde);
        });
        
        $('#memBtn').click(function() {
            //Common.searchpopupWin("searchForm", "/common/memberPop.do","");
            Common.popupDiv("/common/memberPop.do", $("#form_10T").serializeJSON(), null, true);
        });
    ...
    function fn_saveExculde(){
        Common.ajax("POST", "/commission/calculation/updatePrdData_10T.do", GridCommon.getEditData(myGridID_10T) , function(result) {
            // 공통 메세지 영역에 메세지 표시.
   ...
   function createAUIGrid() {
    ...
    myGridID_10T = AUIGrid.create("#grid_wrap_10T", columnLayout3,gridPros);
   }
   ...
   function checkAll(isChecked) {
       ...
       if(isChecked){   // checked == true == 1
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_10T, { "isExclude" : 1 }, i);
         }
       }else{   // unchecked == false == 0
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_10T, { "isExclude" : 0 }, i);
         }
       }
   ...
   function fn_downFile() {
	   Common.ajax("GET", "/commission/calculation/cntCMM0010T", $("#form_10T").serialize(), function(result) {
           var cnt = result;
           if(cnt > 0){
		       ...
		       var salesPersonCd = $("#salesPersonCd_10T").val();
   ...
   function fn_AlldownFile() {
	   var data = { "searchDt" : $("#CMM0010T_Dt").val() , "code": $("#code_10T").val(), "codeId": $("#orgGroup_10").val() };
	   Common.ajax("GET", "/commission/calculation/cntCMM0010T", data, function(result) {
           var cnt = result;
           if(cnt > 0){
        	   var fileName = $("#fileName").val() +"_"+today;
               fileName=fileName+".xlsx";
    ...
    <section class="pop_body" style="max-height:600px;"><!-- pop_body start -->
       <aside class="title_line"><!-- title_line start -->
            ...
            <table class="type1 mt10"><!-- table start -->
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                        <input type="text" title="Create start Date" placeholder="DD/MM/YYYY" name="searchDt" id="CMM0010T_Dt" class="j_date2" value="${searchDt_pop }" />
                        </td>
                        <th scope="row"><spring:message code='commission.text.search.orgGroup'/><span class="must">*</span></th>
                        <td><select id="orgGroup_10" name="codeId" style="width: 100px;">
                                <c:forEach var="list" items="${orgGrList }">
                                    <option value="${list.cdid}">${list.cd}</option>
                                </c:forEach>
                        </select></td>
                        <th scope="row"><spring:message code='commissiom.text.excel.salesPersonCd'/></th>
                        <td>
                              <input type="text" id="salesPersonCd_10T" name="salesPersonCd" style="width: 100px;" maxlength="10">
        ...
        <article class="grid_wrap3"><!-- grid_wrap start -->
            ...
            <!-- grid_wrap start -->
            <div id="grid_wrap_10T" style="width: 100%; height: 334px; margin: 0 auto;"></div>
