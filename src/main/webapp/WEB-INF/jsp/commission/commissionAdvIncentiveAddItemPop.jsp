...
<script type="text/javaScript">
	...
	//nonIncentive Item Save button
	function fn_nonIncenItemSavs(){
		if(fn_ValidIncenItem()){
			Common.ajax("GET", "/commission/calculation/advIncentiveItemValid", $("#addForm").serializeJSON(), function(result) {
				if(result.code != "00"){
					...
					$("#hiddenMemberTypeID").val(result.data.MEM_TYPE);
			 		 ...
			 		 Common.ajaxSync("GET", "/commission/calculation/advIncentiveItemInsert", $("#addForm").serializeJSON(), function(result2) {
	...
	function fn_ValidIncenItem(){
		...
		if($("#itemTAmt").val() == null || $("#itemTAmt").val() == ""){
			Common.alert('<spring:message code="commission.alert.incentive.add.null" arguments="advance amount" htmlEscape="false"/>');
			//Common.alert("lease key in the advance amount.");
			$("#itemTAmt").focus();
			return false;
		}else{
			var amt = $("#itemTAmt").val();
			if(Number(amt) <= 0){
				Common.alert('Advance amount must greater than 0.');
				$("#itemTAmt").focus();
				return false;
			}
		}
...
<section class="pop_body" style="min-height:150px;"><!-- pop_body start -->
	...
	<ul class="center_btns">
		<li><p class="btn_blue"><a href="javascript:fn_nonIncenItemSavs();"><spring:message code='sys.btn.save'/></a></p></li>
	</ul>
