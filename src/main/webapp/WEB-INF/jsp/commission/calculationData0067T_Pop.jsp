...
<script type="text/javaScript">
    ...
    $(document).ready(function() {
        ...
        // ready 이벤트 바인딩
        AUIGrid.bind(myGridID_67T, "ready", function(event) {
            gridDataLength = AUIGrid.getGridData(myGridID_67T).length; // 그리드 전체 행수 보관
        });
        
        // cellClick event.
        AUIGrid.bind(myGridID_67T, "cellClick", function(event) {
              console.log("rowIndex : " + event.rowIndex + ", columnIndex : " + event.columnIndex + " clicked");          
        });
        
     // 헤더 클릭 핸들러 바인딩
        AUIGrid.bind(myGridID_67T, "headerClick", function(event) {
        ...
        $("#search_67T").click(function(){
            Common.ajax("GET", "/commission/calculation/selectDataCMM067T", $("#form_67").serialize(), function(result) {
                console.log("성공.");
                console.log("data : " + result);
                AUIGrid.setGridData(myGridID_67T, result);
                AUIGrid.addCheckedRowsByValue(myGridID_67T, "isExclude", "1");
            });
        });
        
        $("#save_67T").click(function(){  
           	Common.confirm("<spring:message code='sys.common.alert.save'/>",fn_saveExculde);
        });
        
        $('#memBtn').click(function() {
            //Common.searchpopupWin("searchForm", "/common/memberPop.do","");
            Common.popupDiv("/common/memberPop.do", $("#form_67").serializeJSON(), null, true);
        });
        
    });
    
    function fn_loadOrderSalesman(memId, memCode) {
        $("#emplyCd_67T").val(memCode);
        console.log(' memId:'+memId);
        console.log(' memCd:'+memCode);
    }
    
    function fn_saveExculde(){
        Common.ajax("POST", "/commission/calculation/updatePrdData_67T.do", GridCommon.getEditData(myGridID_67T) , function(result) {
            // 공통 메세지 영역에 메세지 표시.
            Common.setMsg("<spring:message code='sys.msg.success'/>");
            $("#search_67T").trigger("click");
        });
    }
   function createAUIGrid() {
    ...
    myGridID_67T = AUIGrid.create("#grid_wrap_67T", columnLayout3,gridPros);
   }
   
   // 전체 체크 설정, 전체 체크 해제 하기
   function checkAll(isChecked) {
       var rowCount = AUIGrid.getRowCount(myGridID_67T);
       
       if(isChecked){   // checked == true == 1
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_67T, { "isExclude" : 1 }, i);
         }
       }else{   // unchecked == false == 0
         for(var i=0; i<rowCount; i++){
            AUIGrid.updateRow(myGridID_67T, { "isExclude" : 0 }, i);
         }
       }
   ...
   function fn_downFile() {
       Common.ajax("GET", "/commission/calculation/cntCMM0067T", $("#form_67").serialize(), function(result) {
           var cnt = result;
           if(cnt > 0){
				var fileName = $("#fileName").val() +"_"+today;
				fileName=fileName+".xlsx";
				var searchDt = $("#CMM0067T_Dt").val();
				var year = searchDt.substr(searchDt.indexOf("/")+1,searchDt.length);
				var month = searchDt.substr(0,searchDt.indexOf("/"));
				var code = $("#code_67T").val();
				
				var codeId= $("#orgGroup_67").val();
				var emplyCd = $("#emplyCd_67T").val();
				var useYnCombo = $("#useYnCombo_67T").val();
   ...
   function fn_AlldownFile() {
	   var data = { "searchDt" : $("#CMM0067T_Dt").val() , "code": $("#code_67T").val(), "codeId": $("#orgGroup_67").val() };
	   Common.ajax("GET", "/commission/calculation/cntCMM0067T", data, function(result) {
           var cnt = result;
           if(cnt > 0){
        	   var fileName = $("#fileName").val() +"_"+today;
               fileName=fileName+".xlsx";
...
<div id="popup_wrap2" class="popup_wrap"><!-- popup_wrap start -->
    ...
    <section class="pop_body" style="max-height:600px;"><!-- pop_body start -->
       <aside class="title_line"><!-- title_line start -->
        ...
        <form id="form_67">
           <input type="hidden" name="code" id="code_67T" value="${code}"/>
           <input type="hidden" id="fileName" name="fileName" value="Award.xlsx"/>
           <ul class="right_btns">
              <li><p class="btn_blue"><a href="#" id="search_67T"><span class="search"></span><spring:message code='sys.btn.search'/></a></p></li>
            </ul>
            ...
            <table class="type1 mt10"><!-- table start -->
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                        <input type="text" title="Create start Date" placeholder="DD/MM/YYYY" name="searchDt" id="CMM0067T_Dt" class="j_date2" value="${searchDt_pop }" />
                        </td>
                        <th scope="row"><spring:message code='commission.text.search.orgGroup'/><span class="must">*</span></th>
                        <td><select id="orgGroup_67" name="codeId" style="width: 100px;" >
                        ...
                        <td>
                              <input type="text" id="emplyCd_67T" name="emplyCd" style="width: 100px;" maxlength="10" >
                     ...
                     <tr>
                        ...
                        <td colspan=5>
                          <select id="useYnCombo_67T" name="useYnCombo" style="width:100px;">
        ...
        <article class="grid_wrap3"><!-- grid_wrap start -->
            ...
            <ul class="right_btns">
                ...
                <li><p class="btn_grid">
                    <a href="#" id="save_67T"><spring:message code='sys.btn.save'/></a>
                </p></li>
            </ul>
            <!-- grid_wrap start -->
            <div id="grid_wrap_67T" style="width: 100%; height: 334px; margin: 0 auto;"></div>
