             ...
             , function(result) 
               {
                  ...
                  if(result != null && result.selectQuarterRateList.length > 0)
                  {
                      ...
                      $("#paramChartExt").val("1");  // Chart Retry prevent..
                  }
               }
    ...
    else if(event.type == "cellEditEnd") 
    {
        ...
        if (event.columnIndex == 2 && event.headerText == "SEQ NO") // SEQ NO
        {
          if (parseInt(event.value) < 1)
          {
            //Common.alert("Menu Level is not more than 4. ");
                ...
                AUIGrid.restoreEditedCells(gChartGridID, [event.rowIndex, "seqNo"] );
                return false;
          }  
        }
        ...
        if (event.columnIndex == 1 && event.headerText == "CATEGORY NAME") // CATEGORY NAME
        {
          if (parseInt(event.value) < 1)
          {
             ...
             AUIGrid.restoreEditedCells(gChartGridID, [event.rowIndex, "stusCtgryName"] );
             return false;
          }
          else
          {
            AUIGrid.setCellValue(gChartGridID, event.rowIndex, 2, AUIGrid.getCellValue(gChartGridID, event.rowIndex, "stusCtgryName"));
          }  
        }
...
function fnSelectQtMonthlyGrid(quarterNo)
{
	  ...
	  $("#paramChartExt").val("1");  //reTry prevent.
