...
var defaultcTPackage = ["39","40"];
var SSTValue = "${sstValue}";
var subscriptionTime = [{"codeId": "1","codeName": "1"}];
var subscriptionYear = [{"codeId": "12","codeName": "1"},{"codeId": "24","codeName": "2"}];
...
$(document).ready(function(){
    ...
    doDefCombo(subscriptionYear, '', 'cYear', 'S', '');
    //doGetComboData('/common/selectCodeList.do', {groupCode :'603'}, '', 'cSubsType', 'S');
...
// 		$("#custNric").html(result.basic.custNric);
	...
	function fn_cTPackage_onchangeEvt() {
        ...
        if($("#cTPackage").val() == '36'){
            doDefCombo(subscriptionYear, '', 'cYear', 'S', '');
        }else if($("#cTPackage").val() == '37'){
            doDefCombo(subscriptionTime, '', 'cYear', 'S', '');
        }
	...
	/* function fn_onChange_cSubsType() {

		var careServType = $("#cSubsType").val();
		$("#subsTypeId").val(careServType);
        if(careServType == '01'){
            doDefCombo(subscriptionTime, '', 'cYear', 'S', '');
        }else if(careServType == '02'){
            doDefCombo(subscriptionYear, '', 'cYear', 'S', '');
        }
...
//                                 var t1 = Math.floor(oriprice - (oriprice * (promoPrcPrcnt / 100))) * pacYear;
	...
	function fn_getMembershipPackageInfo(_id) {
		...
		Common.ajax("GET", "/sales/membership/mPackageInfo", $("#getDataForm").serialize(), function(result) {
			...
			if (result.packageInfo.srvMemPacId == null || result.packageInfo.srvMemPacId == "") {
				...
				var pacYear;
				if($("#cTPackage").val() == '37'){
					pacYear = parseInt($("#DUR").val(), 10);
				}else{
				    pacYear = parseInt($("#DUR").val(), 10) / 12;
				}
	...
	function fn_PacPromotionCode() {
		...
		var E_YN = $("#cEmplo").val();
		//var CARE_SERV_TYPE = $("#subsTypeId").val();
...
// 		$("#cPromo").val("");
	...
	function fn_DoSaveProcess(_saveOption) {
		...
		$("#srvMemPv").val(0);
		if($("#cTPackage").val() == '37'){
		    var subsMonth = Number($("#cYear").val()) * Number($("#hiddentxtBSFreq").val());
		    $("#srvDuration").val(subsMonth);
		}else{
...
<!--         </article>tap_area end -->
        ...
        <tr>
            ...
            <th scope="row">Subscription Frequency</th>
            <td width='80px'>
            <select   id="cYear" name="cYear"  style="width:80px"  onChange="fn_cYear_onChageEvent()"> </select>
        ...
        <tr>
            ...
            <td>
                    ...
                    <select   class="w100p"  id="cPromotionpac" name="cPromotionpac"  onChange="fn_onChange_cPromotionpac()"> </select>
            </td>
            <!-- <th scope="row">Subscription Type</th>
            <td  colspan="3">
                <select   class="w100p"  id="cSubsType" name="cSubsType"  onChange="fn_onChange_cSubsType()"> </select>
                    <span id='txtSubsType'></span>
                    <input id="subsTypeId"  name="subsTypeId" type="hidden" />
            </td> -->
