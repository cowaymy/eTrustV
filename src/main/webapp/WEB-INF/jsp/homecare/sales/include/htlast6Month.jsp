<script type="text/javaScript" language="javascript">
    ...
    function fn_callSvcPop(rowIdx, colIdx) {
        if(rowIdx == 0 || colIdx == 0) {
            ...
            if(sVal.substr(0, 2) == "AS" || sVal.substr(0, 2) == "AA") {
                ...
                if(schdulId != 0) {
                    Common.popupDiv("/homecare/sales/hsBasicInfoPop.do", {schdulId : schdulId, salesOrdId : '${orderDetail.basicInfo.ordId}', MOD : 'VIEW'}, null , true , '_bsBasicPop');
                }
                else {
                    Common.alert('<spring:message code="sal.alert.msg.noRsltView" />' + DEFAULT_DELIMITER + '<b><spring:message code="sal.alert.msg.noAsRsltView" /></b>');
                }
    ...
    function fn_getCurrentBSResultByBSNo(bsNo) {
        var schdulId = 0;
        Common.ajaxSync("GET", "/homecare/sales/selectCurrentBSResultByBSNo.do", {no : bsNo}, function(result) {
            if(result != null) {
    ...
    function fn_selectTransList() {
        Common.ajax("GET", "/homecare/sales/selectLast6MonthTransJsonList.do", {salesOrdId : '${orderDetail.basicInfo.ordId}', appTypeId : '${orderDetail.basicInfo.appTypeId}'}, function(result) {
