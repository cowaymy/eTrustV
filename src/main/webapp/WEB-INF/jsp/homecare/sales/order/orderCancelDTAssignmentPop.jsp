...
<script type="text/javascript">
	...
	var prodReturnGridID;      // Product Return Transaction list
	 ...
	 function cancelLogGrid(){
        ...
        cancelLogGridID = GridCommon.createAUIGrid("#cancelLog", cancelLogColumnLayout, '', cancelLogGridPros);
    }
    ...
    function prodReturnGrid() {
        ...
        prodReturnGridID = GridCommon.createAUIGrid("#productReturn", prodReturnColumnLayout, '', productRtnGridPros);
    }
    ...
    $('#memBtn').click(function() {
        Common.popupDiv("/homecare/sales/order/orderCancelDTSearchPop.do", $("#tabForm").serializeJSON(), null, true, '_memSearchDiv');
    });
    ...
    function fn_loadOrderSalesman(rowItem) {
    	var itemObj = rowItem[0].item;
        Common.ajax("GET", "/sales/order/selectMemberByMemberIDCode.do", {memId : itemObj.memId, memCode : itemObj.memCode}, function(memInfo) {

            if(memInfo == null) {
                Common.alert('<b><spring:message code="sal.alert.msg.memNotFound" />'+itemObj.memCode+'</b>');
            } else {
                $('#hiddenSalesmanId').val(memInfo.memId);
                $('#salesmanCd').val(memInfo.memCode);
                $('#memName').html(memInfo.name + ' (' +memInfo.nric + ')');

                $('#salesmanCd').removeClass("readonly");
                _memSearchDiv.remove();
            }
        });
    }
    ...
    function fn_saveCT(){
    	...
    	Common.ajax("GET", "/homecare/sales/order/saveDTAssignment.do", $("#ctForm").serializeJSON(), function(result) {
...
<h2>Coway Technician (DT)Assignment</h2>
...
<input type="hidden" id="stkRetnOrdNo" name="stkRetnOrdNo" value="${salesOrderNo}">
<table class="type1"><!-- table start -->
<caption>table</caption>
<colgroup>
...
<tr>
    ...
    <th scope="row">DT Group(Current)</th>
    <td><span>${ctAssignmentInfo.ctGrp}</span></td>
</tr>
<tr>
    <th scope="row">DT Code(Current)</th>
    <td><span>${ctAssignmentInfo.memCode}</span></td>
    <th scope="row">DT Name(Current)</th>
    <td><span>${ctAssignmentInfo.memName} (${ctAssignmentInfo.nric})</span></td>
</tr>
<tr>
    <th scope="row">Assign DT<span class="must">*</span></th>
    ...
    <td><span id="memName" >${ctAssignmentInfo.memName} (${ctAssignmentInfo.nric})</span></td>
</tr>
<tr>
    <th scope="row">DT Group<span class="must">*</span></th>
    <td>
