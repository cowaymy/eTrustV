...
<script type="text/javaScript" language="javascript">
    ...
    var modelOrdId = "${basicInfo.ordId}";
    var modelOrdNo = "${basicInfo.ordNo}";
    var modelProductId = "${basicInfo.productId}";
    var modelProductNameFull =  "${basicInfo.productNameFull}";
    var modelCustId = "${basicInfo.custId}";
    var modelCustAddId = "${basicInfo.custAddId}"
    var modelSalesmanCode = "${basicInfo.salesmanCode}";
    var modelRefNo = "${basicInfo.ordRefNo}";
    var modelInstallEntryNo = "${basicInfo.installEntryNo}";
    var modelInstallDt = "${basicInfo.installDt}";
    ...
    $(function() {
        ...
        $('#gstChk').change(function(event) {
            if($("#gstChk").val() == '1') {
                ...
                var srvPacId       = appTypeVal == '66' || appTypeVal == '7759' ? $('#srvPacId').val() : 0;
        ...
        $('#appType').change(function() {
            ...
            if(idx > 0) {
                if(FormUtil.isEmpty($('#hiddenCustId').val())) {
                    ...
                    var pType = $("#appType").val() == '66'  || $("#appType").val() == '7759' ? '1' : '2';
                    var custTypeId  = $('#typeId').val();
                    var nationalityId = $('#nation').val();
                    var postcode = $("#instPostCode").val();
                    var stkId = $("#ordProduct1").val();
        ...
        $('#ordProduct1, #ordProduct2').change(function(event) {
            ...
            var srvPacId      = appTypeVal == '66' || appTypeVal == '7759' ? $('#srvPacId').val() ||  '${orderInfo.basicInfo.srvPacId}'  : 0;
            if(stkIdx > 0) {
                fn_loadProductPrice(appTypeVal, stkIdVal, srvPacId, _tagNum);

                if(copyChangeYn == 'Y') {
                    if(_tagNum == '1') {
                        fn_loadProductPromotion_chg(appTypeVal, stkIdVal, empChk, custTypeVal, '${orderInfo.basicInfo.ordPromoId}', _tagNum);
                    } else {
                        fn_loadProductPromotion_chg(appTypeVal, stkIdVal, empChk, custTypeVal, '${orderInfo2.basicInfo.ordPromoId}', _tagNum);
                    }
                } else {
                    fn_loadProductPromotion(appTypeVal, stkIdVal, empChk, custTypeVal, _tagNum);
                }
            }
        ...
        $('#btnCal').click(function() {
            ...
            if($('#appType').val() != '66' && $('#appType').val() != '7759') {
    ...
    $(document).ready(function(){
		...
		doGetComboSepa ('/homecare/selectHomecareDscBranchList.do', '',  ' - ', '', 'dscBrnchId',  'S', ''); //Branch Code
         ...
         doGetComboData('/common/selectCodeList.do', {groupCode :'324'}, '',  'empChk',  'S', 'fn_initCustInfo'); //EMP_CHK
...
//       doGetComboOrder('/common/selectCodeList.do', '322', 'CODE_ID', '', 'promoDiscPeriodTp2', 'S'); //Discount period
    ...
    function fn_initCustInfo(){
        $('#custId').val(modelCustId);
        fn_selectCustInfo();
    }

    function fn_initProduct(){
    	const productChooseOneOption = $('<option>', {
    		value: "",
            text: "Choose One"
    	});
    	const productOption = $('<option>', {
            value: modelProductId,
            text: modelProductNameFull
        });
        $('#ordProduct1').append(productChooseOneOption);
        $('#ordProduct1').append(productOption);
        $('#ordProduct1').val(modelProductId);
        $('#ordProduct1').prop('disabled', true);

        $('#refereNo').val(modelRefNo);
    }

    function fn_initSalesmanCode(){
        $('#salesmanCd').val(modelSalesmanCode);
        $('#salesmanCd').change();
    }

    function fn_initInstallInfo(){
        $('#instEntryNo').val(modelInstallEntryNo);
        $('#instDt').val(modelInstallDt);
    }
    ...
    function fn_loadInstallAddr(custAddId) {
        Common.ajax("GET", "/sales/order/selectCustAddJsonInfo.do", {custAddId : custAddId, 'isHomecare' : 'Y'}, function(custInfo) {
            if(custInfo != null) {
                ...
                } else if($("#appType").val() != '' && $("#appType").val() != '66' && $("#appType").val() != '7759') {
    ...
    function fn_checkDocList(doCheck) {
        ...
        for(var i = 0; i < AUIGrid.getRowCount(docGridID) ; i++) {
            ...
            if(doCheck == true) {
                ...
                if((vAppType == '66' || vAppType == '7759') && vCustType == '964') {
    ...
    function fn_promoChg(_tagNum) {
       ...
       if(_tagNum == '1'){
        ...
        if(promoIdIdx > 0 && promoIdVal != '0') {
        	if(appTypeVal == '66' || appTypeVal == '67' || appTypeVal == '68' || appTypeVal == '7759') {
...
*******************************************************************************/
    ...
    function fn_doSaveOrder() {
        ...
        var orderVO = {
        	salesOrdId	: modelOrdId,
        	salesOrdNoFirst: modelOrdNo,
            salesOrderMVO1 : {
                ...
                defRentAmt              : vAppType == '66' || vAppType == '7759' ? $('#ordRentalFees1').val().trim() : 0,
         ...
         Common.ajax("POST", "/homecare/sales/order/convertTrialRental.do", orderVO, function(result) {
        	 if(result.code == "00"){
        		 ...
        		 Common.alert('<spring:message code="sal.alert.msg.saveFail" />' + DEFAULT_DELIMITER + "<b>"+result.message+"</b>",fn_orderRegPopClose());
         },  function(jqXHR, textStatus, errorThrown) {
        	$("#btnConfirm_RW").show();
            try {
...
*******************************************************************************/
    ...
    function fn_validSales() {
        ...
        if(appTypeIdx <= 0) {
            ...
            if(appTypeVal == '66' || appTypeVal == '7759'  || appTypeVal == '67' || appTypeVal == '68' || appTypeVal == '1412') {
            	if($("#ordProduct1 option:selected").index() > 0) {
                    if($("#ordPromo1 option:selected").index() <= 0) {
                        isValid = false;
     ...
     // ADD COMBO PROMOTION CHECKING
        ...
        if(!FormUtil.checkReqValue($('#hiddenSalesmanId'))) {
            ...
            if($('#hiddenSalesmanId').val() == '1' || $('#hiddenSalesmanId').val() == '2') {
                ...
                if(appTypeVal == '66' || appTypeVal == '7759' || appTypeVal == '67' || appTypeVal == '68') {
    ...
    function fn_validRentPaySet() {
        ...
        if(appTypeIdx > 0 && (appTypeVal == '66' || appTypeVal == '7759' )) {
    ...
    function fn_validBillGroup() {
        ...
        var appTypeIdx  = $("#appType option:selected").index();
        var appTypeVal  = $("#appType").val();
        var grpOptSelYN = (!$('#grpOpt2').is(":checked")) ? false : true;
...
*******************************************************************************/
    ...
    function fn_loadProductPromotion(appTypeVal, stkId, empChk, custTypeVal, tagNum) {
        $('#ordPromo'+tagNum).removeAttr("disabled");

        //Voucher Management
        if(tagNum == '1'){ //Voucher Check only applies for Main Product Promotion
            if(appTypeVal !=66 && appTypeVal != 7759){
                doGetComboData('/sales/order/selectPromotionByAppTypeStock2.do', {appTypeId:appTypeVal,stkId:stkId, empChk:empChk, promoCustType:custTypeVal, exTrade:0, srvPacId:$('#srvPacId').val(), voucherPromotion: voucherAppliedStatus,custStatus: $('#hiddenCustStatusId').val()}, '', 'ordPromo'+tagNum, 'S', 'voucherPromotionCheck'); //Common Code
            } else {
                doGetComboData('/sales/order/selectPromotionByAppTypeStock.do', {appTypeId:appTypeVal,stkId:stkId, empChk:empChk, promoCustType:custTypeVal, exTrade:0, srvPacId:$('#srvPacId').val(), voucherPromotion: voucherAppliedStatus,custStatus: $('#hiddenCustStatusId').val()}, '', 'ordPromo'+tagNum, 'S', 'voucherPromotionCheck'); //Common Code
            }
        }
        else{
        	if(appTypeVal !=66 && appTypeVal != 7759){
                doGetComboData('/sales/order/selectPromotionByAppTypeStock2.do', {appTypeId:appTypeVal,stkId:stkId, empChk:empChk, promoCustType:custTypeVal, srvPacId:$('#srvPacId').val(), voucherPromotion: voucherAppliedStatus,custStatus: $('#hiddenCustStatusId').val()}, '', 'ordPromo'+tagNum, 'S', ''); //Common Code
            } else {
                doGetComboData('/sales/order/selectPromotionByAppTypeStock.do', {appTypeId:appTypeVal,stkId:stkId, empChk:empChk, promoCustType:custTypeVal, srvPacId:$('#srvPacId').val(), voucherPromotion: voucherAppliedStatus,custStatus: $('#hiddenCustStatusId').val()}, '', 'ordPromo'+tagNum, 'S', ''); //Common Code
            }
        }
    }

    //LoadProductPromotion - copy(chage)
    function fn_loadProductPromotion_chg(appTypeVal, stkId, empChk, custTypeVal, promoVal, tagNum) {
        $('#ordPromo'+tagNum).removeAttr("disabled");
        //Voucher Management
        if(appTypeVal !=66 && appTypeVal != 7759){
            doGetComboData('/sales/order/selectPromotionByAppTypeStock2.do', {appTypeId:appTypeVal,stkId:stkId, empChk:empChk, promoCustType:custTypeVal, srvPacId:$('#srvPacId').val(), voucherPromotion: voucherAppliedStatus,custStatus: $('#hiddenCustStatusId').val()}, promoVal, 'ordPromo'+tagNum, 'S', 'fn_promoChg'+tagNum); //Common Code
        } else {
            doGetComboData('/sales/order/selectPromotionByAppTypeStock.do', {appTypeId:appTypeVal,stkId:stkId, empChk:empChk, promoCustType:custTypeVal, srvPacId:$('#srvPacId').val(), voucherPromotion: voucherAppliedStatus,custStatus: $('#hiddenCustStatusId').val()}, promoVal, 'ordPromo'+tagNum, 'S', 'fn_promoChg'+tagNum); //Common Code
        }
    }
...
//              AUIGrid.resize(docGridID);
    ...
    function fn_setProductCombo(){
    	 var stkType = $("#appType").val() == '66'  || $("#appType").val() == '7759' ? '1' : '2';
  ...
  function fn_reloadPromo(tagNum) {
    ...
    doGetComboData('/sales/order/selectPromoBsdCpnt.do', { appTyp:appTyp, stkId:stkId, cpntId:cpntId, empInd:empInd }, '', 'ordPromo'+tagNum, 'S', '');
  }
	        ...
	        if(result.code == "00") {
	        	...
	        	Common.ajax("GET", "/misc/voucher/getVoucherUsagePromotionId.do", {voucherCode: voucherCode, custEmail: voucherEmail}, function(result) {
	        		if(result.length > 0){
	                    ...
	                    if(stkIdx > 0) {
		                	fn_loadProductPromotion(appTypeVal, stkIdVal, empChk, custTypeVal, "1");
	                    }
	        		}
	        		else{
	        			//reset everything
...
<tr>
    ...
    <td><input id="custId" name="custId" type="text" placeholder="Customer ID" class="readonly" readonly/>
    <%-- <a class="search_btn" id="custBtn"><img src="${pageContext.request.contextPath}/resources/images/common/normal_search.gif" alt="search" /></a> --%>
    </td>
    <th scope="row"><spring:message code="sal.text.type" /></th>
    <td><input id="custTypeNm" name="custTypeNm" type="text" placeholder="Customer Type" class="w100p readonly" readonly/>
...
<tr>
    ...
    <td><input id="name" name="name" type="text" placeholder="Customer Name" class="w100p readonly" readonly/></td>
    <th scope="row"><spring:message code="sal.text.nricCompanyNo" /></th>
    <td><input id="nric" name="nric" type="text" placeholder="NRIC/Company No" class="w100p readonly" readonly/></td>
</tr>
<tr>
    <th scope="row"><spring:message code="sal.title.text.tin" /></th>
    <td><input id="tin" name=tin type="text" placeholder="TIN No" class="w100p readonly" readonly/></td>
    <th scope="row"><spring:message code="sal.title.text.sstRegistrationNo" /></th>
    <td><input id="sstRegNo" name="sstRegNo" type="text" placeholder="SST Registration No" class="w100p readonly" readonly/></td>
</tr>
<tr>
    <th scope="row"><spring:message code="sal.text.nationality" /></th>
    <td><input id="nationNm" name="nationNm" type="text" placeholder="Nationality" class="w100p readonly" readonly/>
        <input id="nation" name="nation" type="hidden"/>
    </td>
    <th scope="row"><spring:message code="sal.text.race" /></th>
    <td><input id="race" name="race" type="text" placeholder="Race" class="w100p readonly" readonly/>
        <input id="raceId" name="raceId" type="hidden"/>
    </td>
</tr>
<tr>
    <th scope="row"><spring:message code="sal.text.dob" /></th>
    <td><input id="dob" name="dob" type="text" placeholder="DOB" class="w100p readonly" readonly/></td>
    <th scope="row"><spring:message code="sal.text.gender" /></th>
    <td><input id="gender" name="gender" type="text" placeholder="Gender" class="w100p readonly" readonly/></td>
</tr>
<tr>
    <th scope="row"><spring:message code="sal.text.passportExpire" /></th>
    <td><input id="pasSportExpr" name="pasSportExpr" type="text" placeholder="Passport Expiry" class="w100p readonly" readonly/></td>
    <th scope="row"><spring:message code="sal.text.visaExpire" /></th>
    <td><input id="visaExpr" name="visaExpr" type="text" placeholder="Visa Expiry" class="w100p readonly" readonly/></td>
</tr>
<tr>
    <th scope="row"><spring:message code="sal.text.email" /></th>
    <td><input id="email" name="email" type="text" placeholder="Email Address" class="w100p readonly" readonly/></td>
    <th scope="row"><spring:message code="sal.text.indutryCd" /></th>
    <td><input id="corpTypeNm" name="corpTypeNm" type="text" placeholder="Industry Code" class="w100p readonly" readonly/></td>
</tr>
<tr>
    <th scope="row"><spring:message code="sal.text.employee" /></th>
    <td colspan="3"><select id="empChk" name="empChk" class="w100p" disabled></select></select></td>
</tr>
 <tr>
	...
	<td colspan="3">
		<div style="display:inline-block;width:100%;">
			...
			<input id="marketMessageYes" type="radio" value="1" name="marketingMessageSelection" disabled/><label for="marketMessageYes">Yes</label>
			</div>
			<div style="display:inline-block;">
			<input  id="marketMessageNo" type="radio" value="0" name="marketingMessageSelection" disabled/><label for="marketMessageNo">No</label>
			</div>
		</div>
	</td>
</tr>
...
<tr>
    ...
    <td><input id="refereNo" name="refereNo" type="text" placeholder="" class="w100p readonly" readonly/></td>
...
<tr>
    ...
    <td>
        <input id="salesmanCd" name="salesmanCd" type="text" placeholder="" class="w100p readonly" readonly/>
...
<tr id='trCpntId2' style='visibility:collapse'>
    <th scope="row"><spring:message code="sal.title.text.cpntId" /></th>
    <td>
...
<ul class="right_btns mb10">
    <li class="blind"><p class="btn_grid"><a id="thrdPartyAddCustBtn" href="#"><spring:message code="sal.btn.addNewThirdParty" /></a></p></li>
</ul>
...
<tbody>
<tr>
    <th scope="row"><spring:message code="sal.text.groupOption" /></th>
    <td>
...
<tbody>
<tr>
    <th scope="row" rowspan="5"><spring:message code="sal.text.billingMethod" /></th>
    <td colspan="3">
...
<tr>
    ...
    <label>
       <c:choose>
	    ...
	    <label>
	  <c:choose>
	    <c:when test="${basicInfo.billPost != 0}">
	       <input type="checkbox" onClick="return false" checked/>
	       <span class="txt_box"><spring:message code="sal.text.post" /><i>${basicInfo.fullAddress}</i></span>
	    </c:when>
	    <c:otherwise>
	       <input type="checkbox" onClick="return false"/>
	    ...
	    <label>
	  <c:choose>
	    <c:when test="${basicInfo.billState != 0}">
	       <input type="checkbox" onClick="return false" checked/>
	       <span class="txt_box"><spring:message code="sal.text.eStatement" /><i>${basicInfo.billStateEmail}</i></span>
	    </c:when>
	    <c:otherwise>
	       <input type="checkbox" onClick="return false"/>
...
<tr>
    <th scope="row"><span><spring:message code="sal.text.email" /> 1</span></th>
    <td>
    <span>${basicInfo.mailCntEmail}</span>
    </td>
    <th scope="row"><span><spring:message code="sal.text.email" /> 2</span></th>
    <td>
    <span>${basicInfo.mailCntEmailAdd}</span>
    </td>
</tr>
<tr>
    <th><span><spring:message code="sal.title.text.eInvoicFlag" /></span></th>
    <td>
      <c:choose>
         <c:when test="${basicInfo.eInvFlg != 0}">
             <input type="checkbox" onClick="return false" checked/>
          </c:when>
          <c:otherwise>
             <input type="checkbox" onClick="return false"/>
...
<tbody>
<tr>
    <th scope="row"><spring:message code="sal.title.text.installInfo" /></th>
    <td><input id="instEntryNo" name="instEntryNo" type="text" class="w100p readonly" readonly/>
...
<tr>
    ...
    <td><input id="instDt" name="instDt" type="text" class="w100p readonly" readonly/></td>
