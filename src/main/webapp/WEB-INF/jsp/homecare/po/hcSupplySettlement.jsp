    ...
    var grid_options = {
      ...
      rowCheckDisabledFunction : function(rowIndex, isChecked, item) {
          if(js.String.strNvl(item.poSettlStatus) != ""
        		  && js.String.strNvl(item.poSettlStatusCd) != "10") { // null과 Rejected가 아닌 경우 체크박스 disabeld 처리함
        ...
        */
        ...
        AUIGrid.bind(myGridID, "rowAllChkClick", function( event ) {
            if(event.checked) {
                ...
                // 25 코드 제거하기
                if(uniqueValues.indexOf("25") != -1){
                    uniqueValues.splice(uniqueValues.indexOf("25"),1);
                }
...
function fn_confirmState( state ){
    ...
    $.each(chkList, function(idx, row){
        if(row.item.poSettlStatusCd != "10" ){
            isValid = false;
        }else{
    ...
    var message = state=="20"?"Do you want to Approve?":"Do you want to Reject?";

    Common
	    .confirm(
	        message,
	        function(){
	            Common.ajax("POST", "/homecare/po/hcSettlement/confirmHcSettlement.do"
