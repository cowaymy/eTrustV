...
<script type="text/javaScript" language="javascript">
  ...
  function createAUIGrid() {
    ...
    AUIGrid.bind(myGridID, "treeLazyRequest", function(event) {
      ...
      $.ajax({
        url : "/services/miles/getMilesMeasDetailList.do?clmNo=" + item.clmNo,
  ...
  $(document).ready(function() {
    ...
    $('#memTyp').change(function() {
      if ($('#memTyp').val() == 'CD') {
        ...
        doGetComboSepa('/common/selectBranchCodeList.do', '45', ' - ', '', 'branchId', 'M', 'f_multiCombo');
      } else if ($('#memTyp').val() == 'HT') {
        doGetComboSepa('/common/selectBranchCodeList.do', '48', ' - ', '', 'branchId', 'M', 'f_multiCombo');
      } else {
        doGetComboSepa('/common/selectBranchCodeList.do', '10', ' - ', '', 'branchId', 'M', 'f_multiCombo');
      }
    });
  });
  ...
  function fn_getMilesMeasList() {
    if ($('#startDt').val() == "") {
      Common.alert("<spring:message code='sys.msg.necessary' arguments='Date Range (Start Date)' htmlEscape='false'/>");
      return;
    }

    if ($('#endDt').val() == "") {
      Common.alert("<spring:message code='sys.msg.necessary' arguments='Date Range (End Date)' htmlEscape='false'/>");
      return;
    }
      ...
      <table class="type1">
        ...
        <tbody>
          <tr>
            ...
            <td>
              <div class="date_set w100p">
                <p><input id="startDt" name="startDt" type="text" title="Create start Date" placeholder="DD/MM/YYYY" class="j_date" /></p> <span>To</span>
                <p><input id="endDt" name="endDt" type="text" title="Create end Date" placeholder="DD/MM/YYYY" class="j_date"/></p>
              </div>
            </td>
            <th scope="row">Claim No</th>
            <td>
              <input id="claimID" name="claimID" type="text" title="Create start Date" placeholder="Claim ID" class="w100p" />
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>
              <input id="memCode" name="memCode" type="text" title="" placeholder="Member Code" class="w100p" />
            </td>
            <th scope="row"></th>
            <td>
            </td>
