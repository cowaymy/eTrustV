...
<script type="text/javascript">
  ...
  $(document).ready(
    function() {
      ...
      doGetCombo('/services/inhouse/getASReasonCode.do?RESN_TYPE_ID=336', '', '', 'ddlFilterExchangeCode', 'S', ''); // FILTER CHARGE EXCHANGE CODE
      doGetCombo('/services/inhouse/getBrnchId', '', '', 'branchDSC', 'S', ''); // RECALL ENTRY DSC CODE
      doGetCombo('/services/inhouse/inHouseGetProductMasters.do', '', '', 'productGroup', 'S', ''); // IN HOUSE PRODUCT GROUP
  ...
  function fn_getErrMstList(_ordNo) {
    ...
    doGetCombo('/services/inhouse/getErrMstList.do?SALES_ORD_NO=' + _ordNo, '', errCde, 'ddlErrorCode', 'S', 'fn_setErrCde');
  }
  ...
  function fn_errMst_SelectedIndexChanged() {
    ...
    doGetCombo('/services/inhouse/getErrDetilList.do?DEFECT_TYPE_CODE=' + errCde, '', '', 'ddlErrorDesc', 'S', 'fn_callback_ddlErrorDesc');
  }

  // GET FILTER INFO.
  function fn_getASRulstEditFilterInfo() {
    Common.ajax("GET", "/services/inhouse/getASRulstEditFilterInfo", { AS_RESULT_NO : $('#asData_AS_RESULT_NO').val() },
      function(result) {
  ...
  function fn_getASRulstSVC0109DInfo() {
    Common.ajax("GET", "/services/inhouse/getASRulstSVC0109DInfo", { AS_RESULT_NO : $('#asData_AS_RESULT_NO').val() },
    function(result) {
      if (result != "") {
        // SUCCESS
  ...
  function fn_HasFilterUnclaim() {
    Common.ajax("GET", "/services/inhouse/getTotalUnclaimItem", { asResultId : $('#asData_AS_RESULT_ID').val(), type : "AS" },
            function(result) {
  ...
  function fn_cmbPaymentType() {
    /*if ($('#ddlFilterCode').val() != "") {
      if ($("#ddlFilterPayType").val() == "CHG") {
        ...
        Common.ajaxSync("GET", "/services/inhouse/getStockPrice.do", {
          stkID : pCode
        }, function(result) {
  ...
  function isstckOk(ct, sk) {
    ...
    Common.ajaxSync("GET", "/services/inhouse/getSVC_AVAILABLE_INVENTORY.do", {
      CT_CODE : ct,
      STK_CODE : sk
    }, function(result) {
    	// KR-OHK Serial Check
  ...
  function fn_productGroup_SelectedIndexChanged_2(val) {
    ...
    if (STK_CTGRY_ID != null) {
      ...
      doGetCombo('/services/inhouse/inHouseGetProductDetails.do?STK_CTGRY_ID=' + STK_CTGRY_ID, '', val, 'productCode', 'S', '');
    }
  }
  ...
  function fn_setSaveFormData() {
    ...
    if ($("#requestMod").val() == "INHOUSE") { // IN HOUSE SESSION
      ...
      if ($("#requestMod").val() == "NEW") {
        ...
        Common.ajax("POST", "/services/inhouse/newResultAdd.do", saveForm,
          function(result) {
            if (result.asNo != "") {
              Common.alert("<spring:message code='service.msg.updSucc'/>");
              fn_DisablePageControl();
  ...
  function fn_setASDataInit(ops) {
    ...
    // fn_getASRulstSVC0109DInfo(); //AS_RESULT_NO
    // fn_setCTcodeValue();
      ...
      $('#ddSrvOldFilterSerial').change(function(event) {
          ...
          if($('#ddSrvOldFilterSerial').val() != ""){
              ...
              Common.ajax("GET", "/services/as/selectFilterSerialConfig.do", {ordId:ordId,stkId:stkId}, function(result) {
                  if(result == null || (result.oldSerialNo != $('#ddSrvOldFilterSerial').val()) ){
      ...
      <tr>
       ...
       <td>
           <input type="text" id='stockSerialNo' name='stockSerialNo' value="${orderDetail.basicInfo.lastSerialNo}" class="readonly" readonly/>
            <p class="btn_grid btn_disabled" style="display:none" id="btnSerialEdit"><a id="serialEdit" href="#" onClick="fn_serialModifyPop()">EDIT</a></p>
       </td>
      </tr>
      <tr>
