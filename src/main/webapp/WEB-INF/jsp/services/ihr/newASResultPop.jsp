...
<script type="text/javaScript">
  ...
  $(document).ready(
    function() {
      ...
      fn_getIHRHistoryInfo(); // GET IHR HISTORY INFOR
  ...
  function fn_getErrMstList(_ordNo) {
    ...
    doGetCombo('/services/inhouse/getErrMstList.do?SALES_ORD_NO=' + SALES_ORD_NO, '', '', 'ddlErrorCode', 'S', '');
  }
  ...
  function fn_errMst_SelectedIndexChanged() {
    ...
    doGetCombo('/services/inhouse/getErrDetilList.do?DEFECT_TYPE_CODE=' + DEFECT_TYPE_CODE, '', '', 'ddlErrorDesc', 'S', 'fn_errDetail_SetVal');
  ...
  function fn_getASRulstEditFilterInfo() {
    Common.ajax("GET", "/services/inhouse/getASRulstEditFilterInfo", { REF_REQST : $('#REF_REQST').val() }, function(result) {
      if (result != null) {
        if (result.length > 0) {
          for (i in result) {
  ...
  function fn_checkASEntryCommission(){
    Common.ajax("GET", "/services/inhouse/getASEntryCommission", $("#resultASForm").serialize(), function(result) {
      if (result == "1") {
  ...
  function fn_setSVC0109DInfo(result) {
   console.log(result);
    ...
    if (typeof (productCode) != "undefined") { doGetCombo('/services/inhouse/inHouseGetProductDetails.do?STK_CTGRY_ID=' + result[0].inHuseRepairGrpCode, '', '', 'productCode', 'S', 'fn_inHouseGetProductDetails'); }
  ...
  function createAUIGrid() {
    ...
    regGridID2 = GridCommon.createAUIGrid("reg_grid_wrap2", columnLayout2, "", gridPros);
  }
  ...
  function fn_getIHRHistoryInfo() {
    Common.ajax("GET", "/services/inhouse/getIHRHistoryInfo.do", $("#resultASForm").serialize(), function(result) {
      AUIGrid.setGridData(regGridID2, result);
    });
  }
  ...
  function fn_getASReasonCode2(_obj, _tobj, _v) {
    ...
    Common.ajax("GET", "/services/inhouse/getASReasonCode2.do", { RESN_TYPE_ID : reasonTypeId, CODE : reasonCode },
      function(result) {
        if (result.length > 0) {
  ...
  function getASStockPrice(_PRC_ID) {
    var ret = 0;
    Common.ajaxSync("GET", "/services/inhouse/getASStockPrice.do", { PRC_ID : _PRC_ID }, function(result) {
      try {
  ...
  function fn_ddlStatus_SelectedIndexChanged(ind) {
    ...
    if ($("#ddlStatus").val() == 19) { // RECALL
      doGetCombo('/services/inhouse/getASReasonCode.do?RESN_TYPE_ID=5551', '', '', 'ddlFailReason', 'S', '');
    } else if ($("#ddlStatus").val() == 10) { // CANCEL
      doGetCombo('/services/inhouse/getASReasonCode.do?RESN_TYPE_ID=5550', '', '', 'ddlFailReason', 'S', '');
    } else {
      doGetCombo('/services/inhouse/getASReasonCode.do?RESN_TYPE_ID=0', '', '', 'ddlFailReason', 'S', '');
  ...
  Common.ajax("GET", "/services/inhouse/getASRclInfo.do", $("#resultASForm").serialize(), function(result) {
      if (result.length != 0) {
   ...
   <section class="pop_body">
    ...
    <section class="tap_wrap">
     ...
     <ul class="tap_type1">
      ...
      <li><a href="#" onclick=" javascirpt:AUIGrid.resize(regGridID2, 950,300); "><spring:message code='service.title.asPassIhrEvt' /></a></li>
     </ul>
       ...
       <table class="type1">
        ...
        <tbody>
         ...
         <tr>
          ...
          <td><input type="text" title="" id='def_type' name='def_type' placeholder="" class="" onblur="fn_getASReasonCode2(this, 'def_type' ,'387')" onkeyup="this.value = this.value.toUpperCase();"/>
