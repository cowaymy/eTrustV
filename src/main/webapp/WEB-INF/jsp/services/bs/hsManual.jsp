...
<script type="text/javaScript" language="javascript">
	...
	function fn_getBSListAjax() {
		...
		if (radioVal == 1) { //HS NO CREATE BEFORE
			...
			if ($("#myBSMonth").val() == "") {
				if ($("#txtSalesOrder").val() == ""
						&& $("#txtHsOrderNo").val() == "") {
					Common
							.alert("HS Period or HS Order or Sales Order are required.");
					return false;
				}
			}

			//Added by keyi 20210923
			if ($("#compStartDt").val() != "" && $("#compEndDt").val() != "") {

				var compStartDt = $("#compStartDt").val();
				var compEndDt = $("#compEndDt").val();
				var compStartYear = Number(compStartDt.substring(6));
                var compEndYear = Number(compEndDt.substring(6));
                var compstartMonth = Number(compStartDt.substring(3, 5));
                var compendMonth = Number(compEndDt.substring(3, 5));

				var currentDt = new Date();
				var mm = Number(currentDt.getMonth() + 1); //January is 0!
                var yyyy = Number(currentDt.getFullYear());

                var myBSMonth = $("#myBSMonth").val();
				var BSMonth = Number(myBSMonth.substring(0, 2));
				var BSYear = Number(myBSMonth.substring(3));

				if ((BSMonth == mm) && (BSYear == yyyy)) {
					if ((compStartYear == compEndYear)
							&& (compstartMonth != BSMonth)
							&& (compendMonth != BSMonth)
							&& (compstartMonth == compendMonth)) {
						Common.alert("You can only search for current month");
						return false;
					}
				} else {
					if ((compStartYear == compEndYear)
							&& Math.abs(compstartMonth - compendMonth) > 3) {
						Common
								.alert("Please enter search period within three(3) months");
						return false;
					}
				}
			}
	...
	function fn_getHSAddListAjax() {
		...
		if (checkedItems.length <= 0) {
			...
			Common
					.alert('Only availbale to entry a result with single HS order');
			return;
		} else if (checkedItems[0]["code"] != "ACT") {
			Common
					.alert('Only availbale to entry a result<br/>for the HS order status in Active');
			return;
		} else {
			var str = "";
			var custStr = "";
	...
	function fn_createHSOrderChecking(salesOrdNo) {
		Common.ajaxSync("GET", "/services/bs/createHSOrderChecking.do", {
			salesOrderNo : salesOrdNo
		}, function(result) {
			msg = result.message;
		});
		return msg;
		...
		$("#hSConfiguration")
				.click(
						function() {
							...
							var radioVal = $(
									"input:radio[name='searchDivCd']:checked")
									.val();
							...
							if (checkedItems.length <= 0) {
								...
								if (brnchCnt > 0) {
									Common
											.alert("Not Avaialable to Create HS Order With Several CDB in Single Time.");
									return;
								}
										...
										.ajax(
												...
												function(result) {
													...
													if (result.message == "success") {
														Common
																.alert("There is already exist for HS order for this month");
														return;
													} else {
														...
														if (msg == "") {
															...
															Common
																	.confirm(
																			'Create HS Order Confirmation'
																					+ DEFAULT_DELIMITER
																					+ "<b>"
																					+ msg
																					+ "</b>",
																			fn_selectHSConfigListPop(
																					jsonObj,
																					saleOrdList,
																					brnchId),
																			fn_selfClose);
	...
	function fn_HSMobileFailureListing() {
		...
		$("#reportFormHSLst #reportFileName").val(
				'/services/HS_Mobile_Fail_excel.rpt');
			...
			function() {
				...
				AUIGrid.bind(myGridID, "cellClick", function(event) {
					schdulId = AUIGrid.getCellValue(myGridID, event.rowIndex,
							"schdulId");
					salesOrdId = AUIGrid.getCellValue(myGridID, event.rowIndex,
							"salesOrdId");
					hsStuscd = AUIGrid.getCellValue(myGridID, event.rowIndex,
							"stusCodeId");
					result = AUIGrid.getCellValue(myGridID, event.rowIndex,
							"result");
				});

				AUIGrid.bind(myGridID, "cellDoubleClick", function(event) {
					var radioVal = $("input:radio[name='searchDivCd']:checked")
							.val();
	...
	function fn_hsReportIndividualGroup() {
		Common.popupDiv("/services/bs/hsReportIndividualGroupPop.do", null,
				null, true, '');
	}
		...
		if (checkedItems.length <= 0) {
			...
			if (checkedItems[0]["stkId"] == "1427") {
				if (checkedItems[0]["code"] != "FAL"
						&& checkedItems[0]["code"] != "COM") {
					Common
							.alert('Ombak only available to reverse for the HS order with FAIL/COM status');
					valid = false;
				}
			} else {
				if (checkedItems[0]["code"] != "COM") {
					Common
							.alert('Only available to reverse for the HS order with COM status');
					valid = false;
				}
			}
		}
	...
	function fn_hsReversal(i) {
		...
		if (fn_hsReversalValid()) {
			...
			Common.confirm("Are you sure want to reverse this HS ?",
					function() {
						...
						Common.ajax("GET", url, {
							schdulId : schdulId,
							salesOrdId : salesOrdId,
							serialRequireChkYn : serialRequireChkYn,
							revInd : i,
							stkId : stkId,
						...
						<table class="type1">
							...
							<tbody>
								...
								<tr>
									...
									<td><p style="width: 70%">
											<input id="myInstallMonth" name="myInstallMonth" type="text"
												title="기준년월" placeholder="MM/YYYY" class="j_date2 w100p" />
										</p>
										<p class="btn_gray">
									...
									<!-- Added by Keyi 20210922 -->
									<th scope="row">Completed Date</th>
									<td>
										<div class="date_set w100p">
											<p>
												<input type="text" title="Complete Start Date"
													placeholder="DD/MM/YYYY" class="j_date" id="compStartDt"
													name="compStartDt" />
											</p>
											<!-- date_set start -->
											<span>To</span>
											<p>
												<input type="text" title="Complete End Date"
													placeholder="DD/MM/YYYY" class="j_date" id="compEndDt"
													name="compEndDt" />
											</p>
										</div> <!-- date_set end -->
									</td>
								...
								<dd>
									<ul class="btns">
										...
										<c:if test="${PAGE_AUTH.funcUserDefine7 == 'Y'}">
											...
											<li><p class="link_btn type2">
													<a href="#"
														onclick="javascript:fn_hsReportIndividualGroup()">HS
              ...
              Current Month Setting(Old system version)</a>
										...
										<li>
											<p class="link_btn type2">
												<a href="#" onclick="fn_HSRptCustSign()"> <spring:message
														code='service.btn.hsRptCustSign' />
												</a>
											</p>
										</li>
									</ul>
