...
<script type="text/javaScript">
			...
			.ready(
					function() {
						if ($("#userType").val() == "2"
							...
							doGetCombo(
									"/services/bs/report/selectCodyBranch.do",
									'', '', 'branchCmb', 'S',
									'fn_setDefaultBranch');
							doGetCombo("/services/bs/report/getCdUpMem.do", '',
									'', 'CMGroup', 'S', 'fn_setDefaultCM');
							doGetCombo(
									"/services/bs/report/selectCodyList2.do",
									'', '', 'codyList', 'S',
									'fn_setDefaultCody');
							$("#clearbtn").hide();
						} else {
							...
							doGetCombo(
									"/services/bs/report/safetyLevelList.do",
									'', '', 'safetyLv', 'S', '');
							doGetCombo("/services/bs/report/selectRegion.do",
									'', '', 'regionCmb', 'S', '');//region
						}
						...
						$("#regionCmb")
								.change(
										function() {
											doGetCombo(
													"/services/bs/report/reportBranchCodeList.do",
													$("#regionCmb").val(), '',
													'branchCmb', 'S', ''); //branch by region
										});
						...
						$("#branchCmb")
								.change(
										function() {
											doGetCombo(
													"/services/bs/report/selectCMGroupList.do",
													$("#branchCmb").val(), '',
													'CMGroup', 'S', '');
											if ($("#branchCmb").val() != '0') {
												$("#safetyLv").prop("disabled",
														true);
											} else {
												$("#safetyLv").prop("disabled",
														false);
						...
						$("#CMGroup")
								.change(
										function() {
											doGetCombo(
													"/services/bs/report/selectCodyList.do",
													$("#CMGroup").val(), '',
													'codyList', 'S', '');
										});
	...
	function fn_openReport() {
		if (fn_validation()) {
			...
			var forecastDt2 = $("#forecastMonth").val().substring(3, 7) + "-"
					+ $("#forecastMonth").val().substring(0, 2);
			...
			if ($("#branchCmb").val()) {
				...
				var forecastDt = $("#forecastMonth").val().substring(3, 7)
						+ "-" + $("#forecastMonth").val().substring(0, 2)
						+ "-01";
				...
				console.log("Date" + $("#forecastMonth").val().substring(3, 7)
						+ "-" + $("#forecastMonth").val().substring(0, 2));
				console.log("Region ID :  " + $("#regionCmb").val());
				console.log("fn_simplifyFormula "
						+ fn_simplifyFormula($("#safetyLv").val()))

				$("#hsFilterForm #reportFileName").val(
						'/services/BSFilterForecast_AllBranches.rpt');
				$("#hsFilterForm #reportDownFileName").val(
						"BSFilterForecast_" + month + date.getFullYear());
				//$("#hsFilterForm #viewType").val("PDF");
				$("#hsFilterForm #V_FORECASTDATE").val(forecastDt2);
				$("#hsFilterForm #V_REGION").val($("#regionCmb").val());
