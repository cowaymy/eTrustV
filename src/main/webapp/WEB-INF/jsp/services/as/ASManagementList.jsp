...
<script type="text/javaScript">
	...
	function fn_searchASManagement() {
		...
		if (fn_getDateGap(startDate, endDate) > 180) {
			Common
					.alert('Start date can not be more than 180 days before the end date.');
			//
			return;
		}
	...
	function fn_newASPop() {
		...
		Common.popupDiv("/services/as/ASReceiveEntryPop.do", {
			in_ordNo : ordno
		}, null, true, '_NewEntryPopDiv1');
	...
	function fn_newASResultPop() {
		...
		if (asStusId != "ACT") {
			Common.alert("<b>[" + asNo + "] already has [" + asStusId
					+ "] result.  .</br> Result entry is disallowed.</b>");
			return;
		}

		var param = "?ord_Id=" + salesOrdId + "&ord_No=" + salesOrdNo
				+ "&as_No=" + asNo + "&as_Id=" + asid + "&refReqst=" + refReqst;
		Common.popupDiv("/services/as/ASNewResultPop.do" + param, null, null,
				true, '_newASResultDiv1');
	}
	...
	function fn_asAppViewPop() {
		...
		Common.popupDiv("/services/as/asResultViewPop.do" + param, null, null,
				true, '_newASResultDiv1');
	}
	...
	function fn_asResultViewPop() {
		...
		Common.popupDiv("/services/as/asResultEditViewPop.do" + param, null,
				null, true, '_newASResultDiv1');
	...
	function fn_asInhouseAddOrderPop() {
		...
		Common.popupDiv("/services/as/resultASReceiveEntryPop.do?salesOrderId="
				+ salesOrdId + "&ordNo=" + salesOrdNo + "&asResultId="
				+ asResultId, null, null, true, '_resultNewEntryPopDiv1');
	...
	function fn_asResultEditPop() {
		...
		if (asTypeID == "AOAS") {
					...
					.ajax(
							...
							function(result) {
											...
											if (result[a].occur > 1) {
												if (result[a].no != "") {
													Common
															.alert("<b>["
																	+ asNo
																	+ "] is not allow to edit.</br> AS information edit is disallowed.");
													return;
												}
											}
										}
								...
								var param = "?ord_Id=" + salesOrdId
										...
										+ asResultId;
								Common.popupDiv(
										"/services/as/asResultEditViewPop.do"
			...
			Common.popupDiv("/services/as/asResultEditViewPop.do" + param,
					null, null, true, '_newASResultDiv1');
		}
	}
	...
	function fn_asResultEditBasicPop() {
		...
		var param = "?ord_Id=" + salesOrdId + "&ord_No=" + salesOrdNo
				...
				+ asResultId;
		Common.popupDiv("/services/as/asResultEditBasicPop.do" + param, null,
				null, true, '_newASResultBasicDiv1');
	...
	function fn_assginCTTransfer() {
		...
		for ( var i in selectedItems) {
			...
			if (asBrnchId != selectedItems[i].item.asBrnchId) {
				Common
						.alert("<b>Can't CT tranfer in multiple branch selection.</b>");
				return;
			}
		}

		Common.popupDiv("/services/as/assignCTTransferPop.do", null, null,
				true, '_assginCTTransferDiv');
	}
	...
	$(document).ready(
			function() {
				...
				AUIGrid.bind(myGridID, "cellDoubleClick", function(event) {
					var asid = AUIGrid.getCellValue(myGridID, event.rowIndex,
							"asId");
					var asNo = AUIGrid.getCellValue(myGridID, event.rowIndex,
							"asNo");
					var asStusId = AUIGrid.getCellValue(myGridID,
							event.rowIndex, "asStusId");
					var salesOrdNo = AUIGrid.getCellValue(myGridID,
							event.rowIndex, "salesOrdNo");
					var salesOrdId = AUIGrid.getCellValue(myGridID,
							event.rowIndex, "asSoId");

					var param = "?ord_Id=" + salesOrdId + "&ord_No="
							+ salesOrdNo + "&as_No=" + asNo + "&as_Id=" + asid;
					Common.popupDiv("/services/as/asResultViewPop.do" + param,
							null, null, true, '_newASResultDiv1');
				});
				...
				},
				{
					...
					renderer : {
						...
						onclick : function(rowIndex, columnIndex, value, item) {
							...
							var AS_ID = AUIGrid.getCellValue(myGridID,
									rowIndex, "asId");
							var AS_NO = AUIGrid.getCellValue(myGridID,
									rowIndex, "asNo");
							var asStusId = AUIGrid.getCellValue(myGridID,
									rowIndex, "code1");
							var ordno = AUIGrid.getCellValue(myGridID,
									rowIndex, "salesOrdNo");
							var ordId = AUIGrid.getCellValue(myGridID,
									rowIndex, "asSoId");
							...
							Common.popupDiv(
									"/services/as/resultASReceiveEntryPop.do?mod=VIEW&salesOrderId="
											+ ordId + "&ordNo=" + ordno
											+ "&AS_NO=" + AS_NO, null, null,
									true, '_viewEntryPopDiv1');
						}
					}
				}
	...
	function fn_asLogBookList() {
		Common.popupDiv("/services/as/report/asLogBookListPop.do", null, null,
				true, '');
	}
	function fn_asRawData() {
		Common.popupDiv("/services/as/report/asRawDataPop.do", null, null,
				true, '');
	}
	function fn_asSummaryList() {
		Common.popupDiv("/services/as/report/asSummaryListPop.do", null, null,
				true, '');
	}
	function fn_asYsList() {
		Common.popupDiv("/services/as/report/asYellowSheetPop.do", null, null,
				true, '');
	}
	function fn_ledger() {
		...
		if (selectedItems.length <= 0) {
			...
			if (asStusId == "ACT") {
				Common
						.alert("<b>["
								+ AS_NO
								+ "] do no has any result yet. Ledger view is disallowed.</b>");
			} else {
				Common.popupDiv("/services/as/report/asLedgerPop.do?ASRNO="
						+ asrNo, null, null, true, '');
	...
	function fn_invoice() {
		...
		if (asStusId != "COM") {
			Common
					.alert("<b>["
							+ AS_NO
							+ "] is not in complete status. Print invoice is disallowed.</b>");
		} else {
			if (asTotalAmt <= 0) {
				Common.alert("<b>[" + AS_NO
						+ "] has no charges. Print invoice is disallowed.</b>");
			} else {
				...
				$("#reportForm #reportDownFileName").val(
						"ASInvoice_" + day + month + date.getFullYear());
	...
	function fn_getDateGap(sdate, edate) {
		...
		var gap = (keyEndDate.getTime() - keyStartDate.getTime()) / 1000 / 60
				/ 60 / 24;
