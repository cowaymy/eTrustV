...
var updcolumn    = [{dataField: "rnum",headerText :"<spring:message code='log.head.rnum'/>"                ,width:  "14%"    ,height:30 , visible:false},
    ...
    $(document).ready(function(){
        ...
        $("#newView").hide();
        $("#updView").hide();
        $('#utrnsitstus').val('').prop("disabled", true);
        $('#ucourier').val('').prop("disabled", true);
        ...
        doGetComboData('/logistics/sirim/selectWarehouseLocByUserBranch.do', '', '','sfrloc', 'S' , '');
        ...
        doGetComboData('/logistics/sirim/selectWarehouseLocByUserBranch.do', '', '','nfrloc', 'S' , '');

        //paramdata = { grade : 'B' , sLoc : '' , equv : 'N'};
        //doGetComboData('/common/selectStockLocationList.do', paramdata, '','ntoloc', 'S' , '');
        doGetComboData('/logistics/sirim/selectWarehouseLoc.do', '', '','ntoloc', 'S' , '');
        ...
        {  $("#vtrnsitid"  ).val(event.item.trnsitid    );
        	$("#trnsitno"  ).text(event.item.trnsitno    );
    ...
    $(function(){
        ...
        $("#showall > a").click(function(){
        	var param = "trnsitid="+$("#vtrnsitid").val();
            detailSearchAjax("/logistics/sirim/selectSirimToTransit.do" , param , deGrid);
        });
        $("#showpen > a").click(function(){
        	var param = "trnsitid="+$("#vtrnsitid").val()+"&statusid=44";
            detailSearchAjax("/logistics/sirim/selectSirimToTransit.do" , param , deGrid);
        });
        $("#showcomp > a").click(function(){
        	var param = "trnsitid="+$("#vtrnsitid").val()+"&statusid=4";
            detailSearchAjax("/logistics/sirim/selectSirimToTransit.do" , param , deGrid);
        });
        $("#showincom > a").click(function(){
        	var param = "trnsitid="+$("#vtrnsitid").val()+"&statusid=50";
            detailSearchAjax("/logistics/sirim/selectSirimToTransit.do" , param , deGrid);
        });
        $("#updall > a").click(function(){
            var param = "trnsitid="+$("#htranid").val();
            detailSearchAjax("/logistics/sirim/selectSirimToTransit.do" , param , updGrid);
        });
        $("#updpen > a").click(function(){
            var param = "trnsitid="+$("#htranid").val()+"&statusid=44";
            detailSearchAjax("/logistics/sirim/selectSirimToTransit.do" , param , updGrid);
        });
        $("#updcomp > a").click(function(){
            var param = "trnsitid="+$("#htranid").val()+"&statusid=4";
            detailSearchAjax("/logistics/sirim/selectSirimToTransit.do" , param , updGrid);
        });
        $("#updincom > a").click(function(){
            var param = "trnsitid="+$("#htranid").val()+"&statusid=50";
            detailSearchAjax("/logistics/sirim/selectSirimToTransit.do" , param , updGrid);
        });
        ...
        $("#updtran").click(function(){
        	...
        	if (selectedItems.length > 0){
        		...
        		Common.ajax("GET","/logistics/sirim/selectSirimModDetail.do",param,function(result){
                    ...
                    if (result.data[0].stsi == '36' || result.data[0].stsi == '8'){
                    	$("#updbutton").hide();
                    }
                    var cparam ={};
    ...
    function getSave(){
    	...
    	Common.ajax("POST",url,data,function(result){
            Common.alert('<spring:message code="sys.msg.success"/>' + DEFAULT_DELIMITER +  "<b>" + "Transit No: " +result.data+"</b>" );
...
<input type='hidden' name='vtrnsitid'    id='vtrnsitid'/>
...
<tbody>
    ...
    <tr>
        <th scope="row">Status</td>
        <td ><select id="utrnsitstus" class="w100p disabled"  name="utrnsitstus"></select></td>
        <th scope="row">Transit By</td>
        <td ID="utrnsitby"></td>
    </tr>
    <tr>
        ...
        <td ID="ulocation"></td>
        <th scope="row">Courier</td>
        <td><select id="ucourier" class="w100p disabled"  name="ucourier"></select></td>
    </tr>
    <tr>
        <th scope="row">Total Sirim Transit</td>
