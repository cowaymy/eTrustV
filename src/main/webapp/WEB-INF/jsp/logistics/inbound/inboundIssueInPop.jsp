...
$(document).ready(function(){
    ...
    $("#btnPopIssueSave").click(function(){
       ...
       var obj = $("#inBoundIssueInForm").serializeJSON();
       obj.gridList = check;
       ...
       if(Common.confirm("Do you want to save?", function(){
           ...
           Common.ajax("POST", "/logistics/inbound/receiptSerial.do", obj, function(result) {
        	    ...
        	    for( var key in result.data ){
        	      ...
        	      if( key == "mdnNo" ){
        	    	  mdnNo = result.data[key];
                  }
        	   }

                 if(errCode == '000') {
                     var msg =result.message + "<br>MDN NO : " + mdnNo;
                     Common.alert(msg);
                 } else {
...
function fn_inBoundIssueInListAjax() {
...
function fn_scanSearchPop(){
    if(Common.checkPlatformType() == "mobile") {
        ...
        Common.popupDiv("/logistics/SerialMgmt/scanSearchPop.do", $("#inBoundIssueInForm").serializeJSON(), null, true, '_scanSearchPop');
    }
}
</script>
...
<section class="pop_body"><!-- pop_body start -->
    ...
    <form id="inBoundIssueInForm" name="inBoundIssueInForm" method="POST">
        <input type="hidden" name="zTrnscType" id="zTrnscType" value="UMPO" />
