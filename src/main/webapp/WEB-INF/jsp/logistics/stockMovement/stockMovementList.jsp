...
<script type="text/javaScript" language="javascript">
	...
	var rescolumnLayout = [
			...
			{
				dataField : "reqstqty",
				headerText : "<spring:message code='log.head.giqty'/>",
				width : 120,
				height : 30
			},
			{
	...
	var reqcolumnLayout = [ {
		dataField : "delyno",
		headerText : "<spring:message code='log.head.deliveryno'/>",
		width : 120,
		height : 30
	}, {
		dataField : "ttype",
		...
		dataField : "gidt",
		headerText : "<spring:message code='log.head.gidate'/>",
		width : 120,
		height : 30
	}, {
		dataField : "grdt",
		headerText : "<spring:message code='log.head.grdate'/>",
		width : 120,
		height : 30
	}, {
		dataField : "itmcd",
	...
	var mtrcolumnLayout = [ {
		...
		headerText : "<spring:message code='log.head.matrldocno'/>",
		width : 200,
		height : 30
	}, {
		dataField : "matrlDocItm",
		headerText : "<spring:message code='log.head.matrldocitm'/>",
		width : 120,
		height : 30
	}, {
		dataField : "trnscTypeCode",
		headerText : "<spring:message code='log.head.transfertype'/>",
		width : 120,
		height : 30
	}, {
		dataField : "invntryMovType",
		headerText : "<spring:message code='log.head.transfertypedtl'/>",
		width : 120,
		height : 30
	}, {
		dataField : "matrlDocYear",
		headerText : "<spring:message code='log.head.matrldocyear'/>",
		width : 120,
		height : 30
	}, {
		...
		headerText : "<spring:message code='log.head.debtcrditindict'/>",
		width : 120,
		height : 30
	}, {
		dataField : "matrlNo",
		headerText : "<spring:message code='log.head.matrlno'/>",
		width : 120,
		height : 30
	}, {
		dataField : "itmName",
		headerText : "<spring:message code='log.head.matrlname'/>",
		width : 200,
		height : 30
	}, {
		dataField : "qty",
		...
		dataField : "itmCode",
		headerText : "<spring:message code='log.head.itmcode'/>",
		width : 120,
		height : 30
	}, {
		dataField : "stockTrnsfrReqst",
		headerText : "<spring:message code='log.head.reqno'/>",
		width : 120,
		height : 30
	}, {
		dataField : "rcloc",
		...
		dataField : "rclocnm",
		headerText : "<spring:message code='log.head.from'/>",
		width : 200,
		height : 30
	}, {
		dataField : "rqlocnm",
		headerText : "<spring:message code='log.head.to'/>",
		width : 200,
		height : 30
	}, {
		dataField : "uom",
	...
	$(document)
			.ready(
					function() {
						...
						listGrid = AUIGrid.create("#main_grid_wrap",
								rescolumnLayout, resop);
						subGrid = GridCommon.createAUIGrid("#sub_grid_wrap",
								reqcolumnLayout, "", reqop);
						mdcGrid = GridCommon.createAUIGrid("#mdc_grid",
								mtrcolumnLayout, "", reqop);
						$("#sub_grid_wrap").hide();
						$("#mdc_grid").hide();
				...
				function() {
					...
					if (selectedItem.length <= 0) {
						...
						if (reqstDt.substring(0, 7) != todayMonth) {
							Common.confirm(
									"SMO not current month. Will be cancelled instead of delete.</br> "
											+ reqstono, fn_delete);
						} else {
							Common.confirm(
									"<spring:message code='sys.common.alert.delete'/></br> "
											+ reqstono, fn_delete);
						}
					}
				});
	...
	function fn_delete() {
		...
		var docno = selectedItem[0].item.docno;
		if (docno != null || docno != undefined) {
			Common.alert(reqsmono + " is connected with " + docno
					+ " order. Deletion is not allowed.</br> ", "");
		} else {
	...
	function fn_deleteAjax(reqsmono) {
		...
		Common.ajax("GET", url, {
			...
			Common.alert("" + result.message + "</br> Delete : " + reqsmono,
					locationList);
		});
	...
	<section class="search_result">
		...
		<ul class="right_btns">
			...
			<c:if test="${PAGE_AUTH.funcUserDefine1 == 'Y'}">
				<li><p class="btn_grid">
						<a id="new2">New SMO By Forecast Report</a>
					</p></li>
				<li><p class="btn_grid">
						<a id="new3">New SMO for On-Loan Unit</a>
					</p></li>
			</c:if>
			<c:if test="${PAGE_AUTH.funcChange == 'Y'}">
				<li><p class="btn_grid">
						<a id="delete">Delete/Cancel</a>
					</p></li>
			</c:if>
		</ul>
