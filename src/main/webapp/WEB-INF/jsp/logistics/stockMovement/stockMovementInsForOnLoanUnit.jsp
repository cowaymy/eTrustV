...
<script type="text/javaScript" language="javascript">
	...
	$(document).ready(function() {
		...
		doGetCombo('/common/selectCodeList.do', '15', '61', 'cType', 'M', 'f_multiCombo');
		doGetCombo('/common/selectCodeList.do', '11', '54', 'catetype', 'M', 'f_multiCombo'); //청구처 리스트 조회
		doSysdate(0, 'docdate');
		...
		$('#save').click(function() {
			...
			for (var i = 0; i < items.add.length; i++) {
				if(items.add.length > 1){
					Common.alert("Only allow request one Material.");
					bool = false;
					break;
				}
				if (items.add[i].rqty == 0) {
					Common.alert('Please enter the Request Qty.');
					bool = false;
					break;
				}
				if(items.add[i].rqty > 1){
					Common.alert('Only allow to request 1 Qty.');
					bool = false;
					break;
				}
			}
			if (bool && f_validatation('save')) {
				...
				Common.ajax("POST", "/logistics/stockMovement/StockMovementForOnLoanUnit.do", dat, function(result) {
					if (result.code == '99') {
	...
	function fn_setDefaultSelection() {
		...
		Common.ajax("GET", "/logistics/stockMovement/selectDefToLocationB.do", '', function(result) {
				...
				<tbody>
					...
					<tr>
						...
						<td colspan="3"><select class="w100p" id="smtype"
							name="smtype" disabled><option>Movement Type Selected</option></select></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td colspan="3"><input id="asno" name="asno"
							type="text" placeholder="AS No"
							class="w100p" /></td>
					</tr>
					<tr>
						<th scope="row">Location Type</th>
						<td><select class="w100p" id="locationType" disabled name="locationType">
			...
			<table class="type1">
				...
				<tbody>
					<tr>
						...
						<td colspan="3"><select class="w100p" id="cType" name="cType" disabled></select>
						</td>
						<th scope="row">Category</th>
						<td colspan="3"><select class="w100p" id="catetype" name="catetype" disabled></select></td>
