...
<script type="text/javaScript">
	...
	$(document).ready(
			function() {
				...
				if (MEM_TYPE == '6' || MEM_TYPE == '4') {
					...
					Common.ajax("GET", "/sales/rcms/selectRosCaller", {
						userName : userName
					}, function(result) {
						if (result.length > 0) {
				...
				//Customer Type
				CommonCombo.make("customerType", "/common/selectCodeList.do", {
					groupCode : '8'
				}, '964', {
					isShowChoose : false
				});
				//Company Type
				CommonCombo.make("companyType", "/common/selectCodeList.do", {
					groupCode : '95'
				}, null, {
					isShowChoose : false,
					isCheckAll : false,
				...
				//Opening Aging Month
				CommonCombo.make("openMonth", "/common/selectCodeList.do", {
					groupCode : '330'
				}, '4|!|5|!|6', {
					id : "code",
					name : "codeName",
				...
				CommonCombo.make('cmbRaceId', "/common/selectCodeList.do", {
					groupCode : '2'
				}, '', {
				...
				$('#excelDown').click(
						function() {
							GridCommon.exportTo("excelGrid", 'xlsx',
									"RCMS Assign List");
						});
	...
	function fn_companyList() {
		...
		Common.ajax("GET", "/common/selectCodeList.do", {
			groupCode : '95'
		}, function(result) {
			companyList = result;
	...
	function createGrid() {
		...
		var assignColLayout = [
				...
				{
					dataField : "custId",
					headerText : '<spring:message code="sal.title.text.customerBrId" />',
					width : '7%',
					editable : false
				},
				{
				...
				},
				{
					...
					headerText : '<spring:message code="sal.title.text.companyBrType" />',
					width : '10%',
					editable : false
				},
				...
				{
					dataField : "rentAmt",
					headerText : '<spring:message code="sal.title.text.currBrOs" />',
					width : '7%',
					editable : false,
					dataType : "numeric",
					formatString : "#,##0.00",
				...
				{
					dataField : "unBillAmt",
					headerText : '<spring:message code="sal.text.unbillAmount" />',
					width : '10%',
					editable : false
				},
				...
				{
					dataField : "assigned",
					headerText : '<spring:message code="sal.title.text.assigned" />',
					width : '7%',
					editable : false,
					visible : false
				},
				{
					dataField : "sensitiveFg",
					headerText : '<spring:message code="sal.title.text.sensitive" />',
					width : '5%',
					editable : false
				},
				{
					dataField : "rem",
					headerText : '<spring:message code="sal.title.text.sensitiveRem" />',
					width : '10%',
					editable : false
				},
				{
					dataField : "etrFg",
					headerText : '<spring:message code="sal.title.text.etr" />',
					width : '5%',
					editable : false
				},
				{
					dataField : "etrRem",
					headerText : '<spring:message code="sal.title.text.otrRem" />',
					width : '10%',
					editable : false
				} ];
		...
		var excelColLayout = [
				...
				/* {dataField : "race", headerText : '<spring:message code="sal.text.race" />', width : 75}, */
				{
					...
					headerText : '<spring:message code="sal.text.rentalAmount" />',
					width : 75,
					dataType : "numeric",
					formatString : "#,##0.00"
				},
				{
					dataField : "colctTrget",
					headerText : '<spring:message code="sal.title.text.openOsBrTarget" />',
					width : 100,
					dataType : "numeric",
					formatString : "#,##0.00"
				},
				...
				{
					dataField : "openMthAging",
					headerText : '<spring:message code="sal.title.text.openAgingBrMonth" />',
					width : 95
				},
				{
					dataField : "unBillAmt",
				...
				{
					dataField : "updAgentDt",
					headerText : '<spring:message code="sal.title.text.assignedDt" />',
					width : 100
				},
				{
					dataField : "sensitiveFg",
		...
		assignGrid = GridCommon.createAUIGrid("#assignGrid", assignColLayout,
				"", assignOptions);
		excelGrid = GridCommon.createAUIGrid("#excelGrid", excelColLayout, "",
				assignOptions);
		...
		AUIGrid.bind(assignGrid, "cellClick", function(event) {
			$("#orderNo").val(
					AUIGrid.getCellValue(assignGrid, event.rowIndex,
							"salesOrdNo"));
			$("#salesOrdId").val(
					AUIGrid.getCellValue(assignGrid, event.rowIndex,
							"salesOrdId"));
	...
	function auiCellEditignHandler(event) {
		if (event.type == "cellEditBegin") {
			console.log("에디팅 시작(cellEditBegin) : ( " + event.rowIndex + ", "
					+ event.columnIndex + " ) " + event.headerText
					+ ", value : " + event.value);
	...
	function fn_selectListAjax() {
		...
		if ($("#rentalStatus").val() == "") {
			Common
					.alert('<spring:message code="sal.alert.msg.plzSelRentalStus" />');
			return;
		}
		...
		if (!(MEM_TYPE == '4' || MEM_TYPE == '6')) {
			if ($("#rosCaller").val() == null) {
				Common
						.alert('<spring:message code="sal.alert.msg.rosCaller" />');
				return;
			}
		}
	...
	function fn_save() {
		...
		if (editedRowItems.length <= 0) {
			Common
					.alert('<spring:message code="sal.alert.msg.noUpdateItem" />');
			return;
		}
		console.log(editedRowItems);
		var param = GridCommon.getEditData(assignGrid);

		Common.ajax("POST", "/sales/rcms/saveAssignAgent", param, function(
				result) {
	...
	function fn_uploadPop() {
		Common.popupDiv("/sales/rcms/uploadAssignAgentPop.do", null,
				fn_selectListAjax, true, "uploadAssignAgentPop");
	}
	...
	function fn_edit() {
		...
		Common.popupDiv("/sales/rcms/updateRemarkPop.do", null, '', true,
				"updateRemarkPop");
	}

	/* Report Start*/
	function fn_badAccReport() {
		Common.popupDiv("/sales/rcms/badAccReportPop.do", null, null, true,
				"badReportPop");
	}

	function fn_assignListReport() {
		Common.popupDiv("/sales/rcms/assignListReportPop.do", null, null, true,
				"assignListReportPop");
	}

	function fn_eTRSummaryListReport() {
		Common.popupDiv("/sales/rcms/eTRSummaryListReportPop.do", null, null,
				true, "eTRSummaryListReportPop");
	}

	function fn_assignSummaryListReport() {
		Common.popupDiv("/sales/rcms/assignSummaryListReportPop.do", null,
	...
	function viewRentalLedger() {
		...
		if (gridObj.length > 0) {
			...
			Common.popupWin("ledgerForm",
					"/sales/order/orderLedger2ViewPop.do", {
						width : "1000px",
						height : "720",
						resizable : "no",
						scrollbars : "no"
			...
			Common
					.alert("<spring:message code='pay.alert.selectTheOrderFirst'/>");
			return;
		}
...
<section id="content">
	...
	<aside class="title_line">
		...
		<h2>
			<spring:message code="sal.title.text.rcmsAssignAgent" />
		</h2>

		<ul class="right_btns">
			<c:if test="${PAGE_AUTH.funcView == 'Y'}">
				<li><p class="btn_blue">
						<a href="#" onClick="fn_invoicePop()"><spring:message
								code="sal.btn.taxInvoice" /></a>
					</p></li>
				<li><p class="btn_blue">
						<a href="#" id="btnSearch"
							onclick="javascript:fn_selectListAjax();"><span
							class="search"></span>
						<spring:message code="sal.btn.search" /></a>
					</p></li>
				<li><p class="btn_blue">
						<a href="#" id="btnClear" onclick="javascript:fn_clear();"><span
							class="clear"></span>
	...
	<section class="search_table">
		...
		<form id="editForm" name="editForm" action="#" method="post">
			<input type="hidden" id="orderNo" name="orderNo" /> <input
				type="hidden" id="salesOrdId" name="salesOrdId" />
		</form>
			...
			<table class="type1">
				...
				<tbody>
					<tr>
						...
						<th scope="row"><spring:message code="sal.text.rentalStatus" /><span
							class="must">*</span></th>
						<td><select id="rentalStatus" name="rentalStatus"
							class="multy_select w100p" multiple="multiple">
						</select></td>
					</tr>
					<tr>
						<th scope="row"><spring:message code="sal.text.custType" /><span
							class="must">*</span></th>
						<td><select id="customerType" name="customerType"
							class="w100p" onchange="javascript:fn_customerChng();"></select>
						</td>
						<th scope="row"><spring:message
								code="sal.title.text.companyType" /></th>
						<td><select id="companyType" name="companyType"
							class="multy_select w100p" multiple="multiple">
						</select></td>
						<th scope="row"><spring:message
								code="sal.title.text.openAgingBrMth" /></th>
						<td><select id="openMonth" name="openMonth"
							class="multy_select w100p" multiple="multiple">
						</select>
					</tr>
					<tr>
						<th scope="row"><spring:message
								code="sal.title.text.rosCaller" /></th>
						<td><select id="rosCaller" name="rosCaller"
							class="multy_select w100p" multiple="multiple">
						</select></td>
						...
						<td><input type="checkbox" id="over60" name="over60"
							value="Y" /></td>
					</tr>
					<tr>
						...
						<td><select id="cmbRaceId" name="raceId"
							class="multy_select w100p" multiple="multiple"></select></td>
						<th scope="row"><spring:message code="sal.text.nricCompanyNo" /></th>
						<td><input type="text" title="" placeholder="" class="w100p"
							id="nric" name="nric" /></td>
					</tr>
					<tr>
						<th scope="row"><spring:message
								code="sal.title.text.assigned" /></th>
						<td>
							<p>
								<select id="assignYn" name="assignYn" class="w100p"></select>
							</p>
						</td>
						<th scope="row"><spring:message
								code="sal.title.text.sensitive" /></th>
						<td>
							<p>
								<select id="sensitiveYn" name="sensitiveYn" class="w100p"></select>
							</p>
						</td>
						<th scope="row"><spring:message code="sal.title.text.etr" /></th>
						<td>
							<p>
								<select id="etrYn" name="etrYn" class="w100p"></select>
							</p>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><select id="bsMonth" name="bsMonth"
							class="w100p">
							    <option value = "">Choose One</option>
								<option value="1" >HS</option>
								<option value="2" >No HS</option>
						</select></td>
					...
					<dd>
						<ul class="btns">
							...
							<c:if test="${PAGE_AUTH.funcPrint == 'Y'}">
								<li><p class="link_btn type2">
										<a onclick="javascript: fn_badAccReport()"><spring:message
												code="sal.title.text.badaccRaw" /></a>
									</p></li>
								<li><p class="link_btn type2">
										<a onclick="javascript: fn_eTRSummaryListReport()"><spring:message
												code="sal.title.text.etrSummaryListRaw" /></a>
									</p></li>
								<li><p class="link_btn type2">
	...
	<section class="search_result">
			...
			<c:if test="${PAGE_AUTH.funcPrint == 'Y'}">
				<li><p class="btn_grid">
						<a href="#" id="excelDown"><spring:message
								code="sal.title.text.download" /></a>
					</p></li>
			</c:if>
			<c:if test="${PAGE_AUTH.funcChange == 'Y'}">
				<li><p class="btn_grid">
						<a href="#" onclick="javascript:fn_edit();"><spring:message
								code="sal.title.text.edit" /></a>
					</p></li>
			</c:if>
		...
		<article class="grid_wrap">
			...
			<div id="excelGrid"
				style="width: 100%; height: 300px; margin: 0 auto; display: none"></div>
		</article>
		<!-- grid_wrap end -->
	</section>
	<!-- search_result end -->
