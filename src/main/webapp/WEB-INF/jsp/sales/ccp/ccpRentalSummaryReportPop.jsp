...
function fn_report(){
	...
	if($("input[name='searchby']:checked").val() == "btnDaily"){
	    if(!($("#dpDateFr").val() == null || $("#dpDateFr").val().length == 0)){
	        whereSQL += " AND TO_DATE(ArgM.GOV_AG_CRT_DT, 'dd/MM/yyyy') >= TO_DATE('"+$("#dpDateFr").val()+"', 'dd/MM/yyyy')";
	    }
	    if(!($("#dpDateTo").val() == null || $("#dpDateTo").val().length == 0)){
            whereSQL += " AND TO_DATE(ArgM.GOV_AG_CRT_DT, 'dd/MM/yyyy') <= TO_DATE('"+$("#dpDateTo").val()+"', 'dd/MM/yyyy')";
        }

	    if(!($("#dtStartFr").val() == null || $("#dtStartFr").val().length == 0) && !($("#dtStartTo").val() == null || $("#dtStartTo").val().length == 0)){
            whereSQL += " AND TO_DATE(ArgM.GOV_AG_START_DT, 'dd/MM/yyyy') BETWEEN TO_DATE('"+$("#dtStartFr").val()+"', 'dd/MM/yyyy') AND TO_DATE('"+$("#dtStartTo").val()+"', 'dd/MM/yyyy')";
        }
	    if(!($("#dtExpiryFr").val() == null || $("#dtExpiryFr").val().length == 0) && !($("#dtExpiryTo").val() == null || $("#dtExpiryTo").val().length == 0)){
            whereSQL += " AND TO_DATE(ArgM.GOV_AG_END_DT, 'dd/MM/yyyy') BETWEEN TO_DATE('"+$("#dtExpiryFr").val()+"', 'dd/MM/yyyy') AND TO_DATE('"+$("#dtExpiryTo").val()+"', 'dd/MM/yyyy')";
        }
	}else if($("input[name='searchby']:checked").val() == "btnMonthly"){
		if(!($("#dpMthYear").val() == null || $("#dpMthYear").val().length == 0)){
            whereSQL += " AND EXTRACT(YEAR FROM ArgM.GOV_AG_CRT_DT) ='"+$("#dpMthYear").val().substring(6,10)+"' AND EXTRACT(MONTH FROM ArgM.GOV_AG_CRT_DT) = '"+$("#dpMthYear").val().substring(3,5)+"'";
        }
	}
	...
	if($("#cmbAgrType option:selected").index() > 0){
		 ...
		 argType = $("#cmbAgrType option:selected").text().trim();
	}else{
		argType = "All";
	}
    ...
    $("#viewType").val("PDF");
    $("#reportFileName").val("/sales/RentalAgrSummaryPDF.rpt");

    $("#reportDownFileName").val("RentalArgSummary_"+date+(new Date().getMonth()+1)+new Date().getFullYear());
...
<input type="hidden" id="reportFileName" name="reportFileName" value="/sales/RentalAgrSummaryPDF.rpt" />
