...
$(document).ready(function(){
	...
	doGetComboData('/sales/promotion/selectProductCompntPromotionList.do', {appType: '${promoInfo.promoAppTypeId}' , stkId : '1427' , effectStartDt : $('#prodCompntEffectStartDt').val() , effectEndDt : $('#prodCompntEffectEndDt').val()  , config : 1 }, '${promoInfo.promoId}' , 'prodCompntPromoId', 'S' , '');
	doGetComboData('/sales/promotion/selectProductCompnt.do', '', '', 'productCompntId', 'S', ''); // Component

	createProductCompntItemGrid();
	fn_selectPromotionProdCompntItemList('${promoInfo.promoId}','1427','${promoInfo.promoAppTypeId}');
...
$(function() {
	  ...
	  $("#_saveBtn").click(function() {
		  if (fn_validSaveInfo())
			  ...
			  fn_saveUpdateProductCompntConfigItem();
	    });
});

function fn_selectPromotionProdCompntItemList(promoId,stkId,appTypeId) {
    Common.ajax("GET", "/sales/promotion/selectProductCompntConfigItmList.do", { promoId : promoId , stkId : stkId , appTypeId : appTypeId }, function(result) {
        AUIGrid.setGridData(productCompntItemGridID, result);
    });
}
...
function addItmToGrid() {
    // Retrieve input values
    var compntId = $("#productCompntId").val();
    var prdCompnt = '';
...
function fn_saveUpdateProductCompntConfigItem(){
	var prodCompnt = GridCommon.getEditData(productCompntItemGridID);
	var productCompntConfigVO = {
			prodCompntList : prodCompnt ,
			appType : $('#prodCompntAppTypeId').val() ,
			stkId : $('#prodCompntProductId').val(),
			promoId : $('#prodCompntPromoId').val()
	};

	Common.ajax("POST", "/sales/promotion/updateProductCompntConfig.do", productCompntConfigVO, function(result) {
...
<tr>
    ...
    <td colspan="3">
    <select id="prodCompntAppTypeId" name="prodCompntAppTypeId" class="w100p" disabled></select>
    </td>
</tr>
<tr>
    <th scope="row"><spring:message code='sal.text.product'/><span class="must">*</span></th>
    <td colspan="3">
    <select id="prodCompntProductId" name="prodCompntProductId" class="w100p" disabled></select>
    </td>
</tr>
<tr>
    <th scope="row"><spring:message code='sal.title.text.promo'/><span class="must">*</span></th>
    <td colspan="3">
    <select id="prodCompntPromoId" name="prodCompntPromoId" class="w100p" disabled></select>
    </td>
</tr>
</tbody>
</table><!-- table end -->
...
<tr>
    ...
    <td colspan="3">
    <select id="productCompntId" name="productCompntId" class="w100p"></select>
    </td>
</tr>
<tr>
