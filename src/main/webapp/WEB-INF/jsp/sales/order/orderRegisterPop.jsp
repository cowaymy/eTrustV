...
<script type="text/javaScript" language="javascript">
			...
			.ready(
					function() {
						...
						doGetComboOrder('/common/selectCodeList.do', '10',
								'CODE_ID', '', 'appType', 'S', ''); //Common Code
						doGetComboOrder('/common/selectCodeList.do', '19',
								'CODE_NAME', '', 'rentPayMode', 'S', ''); //Common Code
						doGetComboOrder('/common/selectCodeList.do', '17',
								'CODE_NAME', '', 'billPreferInitial', 'S', ''); //Common Code
						...
						$(".auto_file")
								.append(
										"<label><span class='label_text'><a href='#'>File</a></span><input type='text' class='input_text' readonly='readonly' /></label>");
	...
	function createAUIGrid() {
		...
		docGridID = GridCommon.createAUIGrid("grid_doc_wrap", columnLayout, "",
				gridPros);
	}

	// 리스트 조회.
	function fn_selectDocSubmissionList() {
		Common.ajax("GET", "/sales/order/selectDocSubmissionList.do", {
			typeCodeId : '248'
		}, function(result) {
						...
						function(result) {
							...
							if (result != null && result.length == 1) {
								...
								if (custInfo.codeName == 'Government') {
									Common
											.alert('<spring:message code="sal.alert.msg.gvmtCust" />');
								}
							} else {
				...
				.ajaxSync(
						...
						function(custInfo) {
							...
							if (custInfo != null) {
								...
								console.log("custInfo.areaId.substring(0,2):"
										+ custInfo.areaId.substring(0, 2));

								if ("DM" == custInfo.areaId.substring(0, 2)) {
									Common
											.alert('<spring:message code="sal.alert.msg.invalidAddr" />'
													+ DEFAULT_DELIMITER
													+ '<spring:message code="sal.alert.msg.oldAddrNewAddr" />');
									/*
									fn_clearInstallAddr();
									$('#liInstNewAddr').removeClass("blind");
									$('#liInstSelAddr').removeClass("blind");
								...
								if ($("#appType").val() == '66') {
									...
									if (custInfo.gstChk == '1') {
										$("#gstChk").val('1').prop("disabled",
												true);
										$("#pBtnCal").removeClass("blind");
										//fn_tabOnOffSet('REL_CER', 'SHOW');
										fn_tabOnOffSet('REL_CER', 'HIDE');
									} else {
										$("#gstChk").val('0').prop("disabled",
												true);
										$('#pBtnCal').addClass("blind");
										fn_tabOnOffSet('REL_CER', 'HIDE');
									}
			...
			if (doCheck == true) {
				...
				if (vAppType == '66' && vCustType == '964') {
					if (vNational == 'MALAYSIA') {
						if (vCodeId == '250' || vCodeId == '1244'
								|| vCodeId == '271') {
							AUIGrid.setCellValue(docGridID, i, "chkfield", 1);

							if (docDefaultChk == false)
								docDefaultChk = true;
						}
					} else {
						if (vCodeId == '939' || vCodeId == '940'
								|| vCodeId == '1244') {
							AUIGrid.setCellValue(docGridID, i, "chkfield", 1);
	...
	function fn_loadInstallationCntcPerson(custCntcId) {
		...
		Common.ajax("GET", "/sales/order/selectCustCntcJsonInfo.do", {
			custCntcId : custCntcId
		}, function(instCntcInfo) {

			if (instCntcInfo != null) {

				console.log("fn_loadInstallationCntcPerson hiddenInstCntcId:"
						+ instCntcInfo.custCntcId);
				console.log("fn_loadInstallationCntcPerson instCntcName    :"
						+ instCntcInfo.name);
				console.log("fn_loadInstallationCntcPerson instInitial     :"
						+ instCntcInfo.custInitial);
				console.log("fn_loadInstallationCntcPerson instCntcEmail   :"
						+ instCntcInfo.email);
	...
	$(function() {
		...
		$('#mstCntcNewAddBtn').click(function() {
			...
			Common.popupDiv('/sales/customer/updateCustomerNewContactPop.do', {
				"custId" : $('#hiddenCustId').val(),
				"callParam" : "ORD_REGISTER_CNTC_OWN"
		...
		$('#mstCntcSelAddBtn').click(function() {
			...
			Common.popupDiv("/sales/customer/customerConctactSearchPop.do", {
				custId : $('#hiddenCustId').val(),
				callPrgm : "ORD_REGISTER_CNTC_OWN"
			}, null, true);
		});
		$('#mstCntcNewAddBtn2')
				.click(
							...
							Common
									.popupDiv(
											'/sales/customer/updateCustomerNewAddContactPop.do',
											{
												"custId" : $('#hiddenCustId')
														.val(),
												"callParam" : "ORD_REGISTER_CNTC_ADD"
											}, null, true, '_editDiv3New');
						});
		$('#mstCntcSelAddBtn2').click(function() {
			...
			Common.popupDiv("/sales/customer/customerConctactSearchPop.do", {
				custId : $('#hiddenCustId').val(),
				callPrgm : "ORD_REGISTER_CNTC_ADD"
			}, null, true);
		});
		...
		$('#instNewAddrBtn').click(function() {
			...
			Common.popupDiv("/sales/customer/updateCustomerNewAddressPop.do", {
				custId : $('#hiddenCustId').val(),
				callParam : "ORD_REGISTER_INST_ADD"
			}, null, true);
		});
		$('#instSelAddrBtn').click(function() {
			...
			Common.popupDiv("/sales/customer/customerAddressSearchPop.do", {
				custId : $('#hiddenCustId').val(),
				callPrgm : "ORD_REGISTER_INST_ADD"
			}, null, true);
		});
		$('#billGrpBtn').click(function() {
			...
			Common.popupDiv("/sales/customer/customerBillGrpSearchPop.do", {
				custId : $('#hiddenCustId').val(),
				callPrgm : "ORD_REGISTER_BILL_GRP"
			}, null, true);
		});
				...
				function() {
					...
					Common.popupDiv(
							"/sales/customer/customerCreditCardAddPop.do", {
								custId : vCustId,
								callPrgm : "ORD_REGISTER_PAYM_CRC"
							}, null, true);
				});
				...
				function() {
					...
					Common.popupDiv(
							"/sales/customer/customerBankAccountAddPop.do", {
								custId : vCustId,
								callPrgm : "ORD_REGISTER_BANK_ACC"
							}, null, true);
				});
		...
		$('#trialNoBtn').click(function() {
			...
			Common.popupDiv("/sales/order/orderSearchPop.do", {
				callPrgm : "ORD_REGISTER_SALE_ORD",
				indicator : "SearchTrialNo"
			});
		});
				...
				function() {
					...
					if ($("#billMthdEstm").is(":checked")) {
						...
						$('#billMthdEmailTxt1').removeAttr("disabled").val(
								$('#custCntcEmail').val().trim());
						$('#billMthdEmailTxt2').removeAttr("disabled").val(
								$('#srvCntcEmail').val().trim());
					}
				});
						...
						function() {
							...
							if (idx > 0) {
								if (FormUtil.isEmpty($('#hiddenCustId').val())) {
									$('#appType').val('');
									Common
											.alert('<spring:message code="sal.alert.msg.plzSelCust" />');
									...
									case '66': //RENTAL
										...
										//?FD문서에서 아래 항목 빠짐
										$('[name="advPay"]').removeAttr(
												"disabled");
										$('#installDur').val('').prop(
												"readonly", true).addClass(
												"readonly");

										$("#gstChk").val('0').removeAttr(
												"disabled");
										$('#pBtnCal').addClass("blind");
										fn_tabOnOffSet('REL_CER', 'HIDE');
									...
									if (custTypeId != '964'
											|| (custTypeId == '964' && nationalityId != '1')) {
										doGetComboData(
												'/sales/order/selectServicePackageList2.do',
												{
		...
		$('#ordProudct')
				.change(
						function() {
							...
							if (FormUtil.checkReqValue($('#exTrade'))) {
								Common
										.alert('<spring:message code="sal.alert.msg.saveSalOrdSum" />'
												+ DEFAULT_DELIMITER
												+ '<spring:message code="sal.alert.msg.plzSelExTrade" />');
								$('#ordProudct').val('');
								return;
							}
							...
							$('#trialNoChk').prop("checked", false).prop(
									"disabled", true);
							$('#trialNo').val('').addClass("readonly");
							...
							var appTypeIdx = $("#appType option:selected")
									.index();
							var appTypeVal = $("#appType").val();
							var custTypeVal = $("#typeId").val();
							var stkIdx = $("#ordProudct option:selected")
									.index();
							var stkIdVal = $("#ordProudct").val();
						...
						function() {
							...
							if (rentPayModeIdx > 0) {
								if (rentPayModeVal == '133'
									...
									Common
											.alert('<spring:message code="sal.alert.msg.notProvideSvc" arguments="'+rentPayModeVal+'"/>');
									fn_clearRentPayMode();
								} else {
									if (rentPayModeVal == '131') {
										if ($('#thrdParty').is(":checked")
												&& FormUtil.isEmpty($(
														...
														.val())) {
											Common
													.alert('<spring:message code="sal.alert.msg.plzSelThirdPartyFirst" />');
										} else {
											$('#sctCrCard')
													.removeClass("blind");
										}
				...
				.click(
						function() {
							if ($("#ordProudct option:selected").index() <= 0) {
								Common
										.alert('<spring:message code="sal.alert.msg.plzSelProd" />');
								return false;
							}
							var appTypeName = $('#appType option:selected')
									.text();
		...
		$('#ordPromo').change(
				function() {
					...
					//          $('#relatedNo').val('').prop("readonly", true).addClass("readonly");
					$('#trialNoChk').prop("checked", false).prop("disabled",
							true);
					$('#trialNo').val('').addClass("readonly");
					...
					if (promoIdIdx > 0 && promoIdVal != '0') {
						 ...
						 */
						if (appTypeVal == '66' || appTypeVal == '67'
								|| appTypeVal == '68') {
		...
		$('[name="ordSaveBtn"]').click(
				function() {
					if (bulkOrderYn == 'Y'
							&& FormUtil.checkReqValue($('#hiddenCopyQty'))) {
	...
	function fn_preCheckSave() {
		...
		if (!FormUtil.checkReqValue($('#certRefFile'))) {
			 ...
			 */
			msg += '<spring:message code="sal.alert.msg.certRefFile1" />'
					+ '<spring:message code="sal.alert.msg.certRefFile2" />'
					+ '<spring:message code="sal.alert.msg.certRefFile3" />';
		}
		...
		if (!isValid) {
			Common.confirm(
					'<spring:message code="sal.alert.msg.cnfrmToSave" />'
							+ DEFAULT_DELIMITER + msg, fn_hiddenSave);
		} else {
			...
			if ($("#ordPromo option:selected").index() > 0) {
				...
				if ($("#exTrade").val() == 1 || $("#exTrade").val() == 2) {
					...
					$('#txtOldOrderID').val();
					$('#relatedNo').val();
					Common.popupDiv("/sales/order/oldOrderPop.do", {
						custId : $('#hiddenCustId').val(),
						salesOrdNo : $('#relatedNo').val()
					}, null, true);
				} else {
	...
	function fn_doSaveOrder() {
		...
		//----------------------------------------------------------------------
		var vAppType = $('#appType').val();
		var vDefRentAmt = vAppType == '66' ? $('#ordRentalFees').val().trim()
				: 0;
		var vCustBillId = $('input:radio[name="grpOpt"]:checked').val() == 'exist' ? $(
				'#hiddenBillGrpId').val()
				: 0;
		var vBindingNo = FormUtil.isEmpty($('#txtOldOrderID').val().trim()) ? $(
				'#relatedNo').val().trim()
				: $('#hiddenOldOrderId').val().trim();
		var vCnvrSchemeId;
		...
		var vIssuNric = FormUtil.isNotEmpty($('#rentPayIC').val().trim()) ? $(
				'#rentPayIC').val().trim()
				: $('#thrdParty').is(":checked") ? $('#thrdPartyNric').val()
						.trim() : $('#nric').val().trim();
		//----------------------------------------------------------------------
		// accClaimAdtVO
		//----------------------------------------------------------------------
		...
		Common
				.ajax(
						"POST",
						"/sales/order/registerOrder.do",
						orderVO,
						function(result) {

							Common.alert(
									'<spring:message code="sal.alert.msg.ordSaved" />'
											+ DEFAULT_DELIMITER + "<b>"
											+ result.message + "</b>",
									fn_orderRegPopClose());
							...
							try {
								...
								Common
										.alert('<spring:message code="sal.alert.title.saveFail" />'
												+ DEFAULT_DELIMITER
												+ '<b><spring:message code="sal.alert.msg.failSaveOrd" /></b>');
							} catch (e) {
								console.log(e);
	...
	function fn_validSales() {
					...
					if (mm <= '12') {
						if (dd <= '22') {
							if (appTypeVal == '66' || appTypeVal == '67'
								...
								} else if ($('#compType').val() == '1'
										|| $('#compType').val() == '2') {
									if ($('#ordPromo').val() != '31736'
											&& $('#ordPromo').val() != '31723') {
										isValid = false;
										msg += '* <spring:message code="sal.alert.msg.plzSelPromoCde" /><br>';
									}
		...
		if (!FormUtil.checkReqValue($('#hiddenSalesmanId'))) {
			...
			if ($('#hiddenSalesmanId').val() == '1'
					|| $('#hiddenSalesmanId').val() == '2') {

				if (appTypeVal == '66' || appTypeVal == '67'
						|| appTypeVal == '68') {

					if (FormUtil.checkReqValue($('#departCd'))
							|| FormUtil.checkReqValue($('#grpCd'))
							|| FormUtil.checkReqValue($('#orgCd'))) {

						var memType = $('#hiddenSalesmanTypeId').val() == '1' ? "HP"
								: "Cody";
	...
	function fn_loadBankAccount(bankAccId) {
		...
		Common.ajax("GET", "/sales/order/selectCustomerBankDetailView.do", {
			...
			if (rsltInfo != null) {
				...
				$("#accBank")
						.val(rsltInfo.bankCode + ' - ' + rsltInfo.bankName);
	...
	function fn_loadTrialNo(trialNo) {
		...
		if (FormUtil.isNotEmpty(trialNo)) {
			...
			Common.ajax("GET", "/sales/order/selectTrialNo.do", {
				salesOrdNo : trialNo
			}, function(trialInfo) {
	...
	function fn_loadProductComponent(stkId) {
		...
		console.log("fn_loadProductComponent ::: "
				+ $('#compType option').length);
		...
		case 'PAY_CHA':
			if (opt == 'SHOW') {
				if ($('#tabPC').hasClass("blind"))
					$('#tabPC').removeClass("blind");
				if ($('#atcPC').hasClass("blind"))
					$('#atcPC').removeClass("blind");
			} else if (opt == 'HIDE') {
				if (!$('#tabPC').hasClass("blind"))
		...
		case 'REL_CER':
			if (opt == 'SHOW') {
				if ($('#tabRC').hasClass("blind"))
					$('#tabRC').removeClass("blind");
				if ($('#atcRC').hasClass("blind"))
					$('#atcRC').removeClass("blind");
			} else if (opt == 'HIDE') {
				if (!$('#tabRC').hasClass("blind"))
					$('#tabRC').addClass("blind");
				if (!$('#atcRC').hasClass("blind"))
					$('#atcRC').addClass("blind");
...
<div id="popup_wrap" class="popup_wrap">
 ...
 <header class="pop_header">
  <!-- pop_header start -->
  <h1>
   <spring:message code="sal.title.text.newOrder" />
  </h1>
  <ul class="right_opt">
   <li><p class="btn_blue2">
     <a href="#" id="btnOrdRegClose"><spring:message
 ...
 <section class="pop_body">
  ...
  <section class="tap_wrap">
   ...
   <ul class="tap_type1 num4">
    <li id="tabCS"><a id="aTabCS" href="#"
     onClick="javascript:chgTab('cst');" class="on">Customer</a></li>
    <li id="tabMC"><a id="aTabMS" href="#"
     onClick="javascript:chgTab('cnt');">Master Contact</a></li>
    <li id="tabSO"><a id="aTabSO" href="#"
     onClick="javascript:chgTab('sal');">Sales Order</a></li>
    <li id="tabPC"><a id="aTabPC" href="#"
     onClick="javascript:chgTab('pay');">Payment Channel</a></li>
    <li id="tabBD"><a id="aTabBD" href="#"
     onClick="javascript:chgTab('bil');">Billing Detail</a></li>
    <li id="tabIN"><a id="aTabIN" href="#"
     onClick="javascript:chgTab('ins');">Installation</a></li>
    <li id="tabDC"><a id="aTabDC" href="#"
     onClick="javascript:chgTab('doc');">Documents</a></li>
    <li id="tabRC"><a id="aTabRC" href="#"
     onClick="javascript:chgTab('rlf');">Relief Certificate</a></li>
   </ul>
   <!--****************************************************************************
   ...
   <article class="tap_area">
    ...
    <section class="search_table">
     ...
     <form id="searchForm" name="searchForm" action="#" method="post">
      <input id="searchCustId" name="custId" type="hidden" /> <input
       id="hiddenCustId" name="custId" type="hidden" /> <input
       id="hiddenOldOrderId" name="hiddenOldOrderId" type="hidden" /> <input
       id="hiddenCopyQty" name="hiddenCopyQty" type="hidden" />
     </form>
     <form id="custForm" name="custForm" action="#" method="post">
       ...
       <tbody>
        <tr>
         <th scope="row"><spring:message code="sal.text.customerId" /><span
          class="must">*</span></th>
         <td><input id="custId" name="custId" type="text" title=""
          placeholder="Customer ID" class="" /><a class="search_btn"
          id="custBtn"><img
         ...
         <td><input id="custTypeNm" name="custTypeNm" type="text"
          title="" placeholder="Customer Type" class="w100p" readonly />
          <input id="typeId" name="typeId" type="hidden" /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.name" /></th>
         <td><input id="name" name="name" type="text" title=""
          placeholder="Customer Name" class="w100p" readonly /></td>
         <th scope="row"><spring:message
           code="sal.text.nricCompanyNo" /></th>
         <td><input id="nric" name="nric" type="text" title=""
          placeholder="NRIC/Company No" class="w100p" readonly /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.text.nationality" /></th>
         <td><input id="nationNm" name="nationNm" type="text"
          title="" placeholder="Nationality" class="w100p" readonly />
          <input id="nation" name="nation" type="hidden" /></td>
         <th scope="row"><spring:message code="sal.text.race" /></th>
         <td><input id="race" name="race" type="text" title=""
          placeholder="Race" class="w100p" readonly /> <input
          id="raceId" name="raceId" type="hidden" /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.dob" /></th>
         <td><input id="dob" name="dob" type="text" title=""
          placeholder="DOB" class="w100p" readonly /></td>
         <th scope="row"><spring:message code="sal.text.gender" /></th>
         <td><input id="gender" name="gender" type="text" title=""
          placeholder="Gender" class="w100p" readonly /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.text.passportExpire" /></th>
         <td><input id="pasSportExpr" name="pasSportExpr"
          type="text" title="" placeholder="Passport Expiry"
          class="w100p" readonly /></td>
         <th scope="row"><spring:message code="sal.text.visaExpire" /></th>
         <td><input id="visaExpr" name="visaExpr" type="text"
          title="" placeholder="Visa Expiry" class="w100p" readonly /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.email" /></th>
         <td><input id="email" name="email" type="text" title=""
          placeholder="Email Address" class="w100p" readonly /></td>
         <th scope="row"><spring:message code="sal.text.indutryCd" /></th>
         <td><input id="corpTypeNm" name="corpTypeNm" type="text"
          title="" placeholder="Industry Code" class="w100p" readonly /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.employee" /><span
          class="must">*</span></th>
         <td colspan="3"><select id="empChk" name="empChk"
          class="w100p"></select></select></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.remark" /></th>
         <td colspan="3"><textarea id="custRem" name="custRem"
           cols="20" rows="5" placeholder="Remark" readonly></textarea></td>
        </tr>
       </tbody>
      </table>
      <!-- table end -->
      <ul class="center_btns">
       <li><p class="btn_blue">
         <a id="saveBtn" name="ordSaveBtn" href="#"><spring:message
           code="sal.btn.ok" /></a>
        </p></li>
      </ul>
     </form>
...
*****************************************************************************-->
   <article class="tap_area">
    ...
    <aside class="title_line">
     <!-- title_line start -->
     <h3>
      <spring:message code="sal.title.text.ownPurchCntc" />
     </h3>
    </aside>
    <!-- title_line end -->
    <ul class="right_btns mb10">
     <li id="liMstCntcNewAddr" class="blind"><p class="btn_grid">
       <a id="mstCntcNewAddBtn" href="#"><spring:message
         code="sal.btn.addNewContact" /></a>
      </p></li>
     <li id="liMstCntcSelAddr" class="blind"><p class="btn_grid">
...
------------------------------------------------------------------------------->
    <section class="search_table">
     <!-- search_table start -->
     <form id="ownerPurchsForm" name="ownerPurchsForm" action="#"
      method="post">
      <table class="type1">
       <!-- table start -->
       <caption>table</caption>
       ...
       <tbody>
        <tr>
         ...
         <td><input id="custInitial" name="custInitial" type="text"
          title="" placeholder="" class="w100p" /></td>
         <th scope="row"><spring:message code="sal.text.name" /></th>
         <td><input id="custCntcName" name="custCntcName"
          type="text" title="" placeholder="" class="w100p" /> <input
          id="hiddenCustCntcId" name="custCntcId" type="hidden" /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.title.text.telMOne" /></th>
         <td><input id="custCntcTelM" name="custCntcTelM"
          type="text" title="" placeholder="" class="w100p" /></td>
         <th scope="row"><spring:message
           code="sal.title.text.telROne" /></th>
         <td><input id="custCntcTelR" name="custCntcTelR"
          type="text" title="" placeholder="" class="w100p" /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.title.text.telOOne" /></th>
         <td><input id="custCntcTelO" name="custCntcTelO"
          type="text" title="" placeholder="" class="w100p" /></td>
         <th scope="row"><spring:message
           code="sal.title.text.extNo" />(1)</th>
         <td><input id="custCntcExt" name="custCntcExt" type="text"
          title="" placeholder="" class="w100p" /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.title.text.telFOne" /></th>
         <td><input id="custCntcTelF" name="custCntcTelF"
          type="text" title="" placeholder="" class="w100p" /></td>
         <th scope="row"><spring:message
           code="sal.title.text.eamilOne" /></th>
...
------------------------------------------------------------------------------->
    ...
    <!-- title_line end -->
    <ul class="right_btns mb10">
     <li id="liMstCntcNewAddr2" class="blind"><p class="btn_grid">
       <a id="mstCntcNewAddBtn2" href="#">Add New Contact</a>
      </p></li>
     <li id="liMstCntcSelAddr2" class="blind"><p class="btn_grid">
       <a id="mstCntcSelAddBtn2" href="#">Select Another Contact</a>
      </p></li>
    </ul>
    <section class="search_table">
     <!-- search_table start -->
     <form id="addSvcCntcForm" name="custCntcForm" action="#"
      method="post">
      <input id="srvCntcId" name="srvCntcId" type="hidden" /> <input
       id="srvInitial" name="srvInitial" type="hidden" />
      <table class="type1">
       <!-- table start -->
       <caption>table</caption>
       <colgroup>
       ...
       <tbody>
        <tr>
         ...
         <td colspan="3"><input id="srvCntcName" name="srvCntcName"
          type="text" title="" placeholder="" class="w100p" /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.title.text.telMTwo" /></th>
         <td><input id="srvCntcTelM" name="srvCntcTelM" type="text"
          title="" placeholder="" class="w100p" /></td>
         <th scope="row"><spring:message
           code="sal.title.text.telRTwo" /></th>
         <td><input id="srvCntcTelR" name="srvCntcTelR" type="text"
          title="" placeholder="" class="w100p" /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.title.text.telOTwo" /></th>
         <td><input id="srvCntcTelO" name="srvCntcTelO" type="text"
          title="" placeholder="" class="w100p" /></td>
         <th scope="row"><spring:message
           code="sal.title.text.extNo" />(2)</th>
         <td><input id="srvCntcExt" name="srvCntcExt" type="text"
          title="" placeholder="" class="w100p" /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.title.text.telFTwo" /></th>
         <td><input id="srvCntcTelF" name="srvCntcTelF" type="text"
          title="" placeholder="" class="w100p" /></td>
         <th scope="row"><spring:message
           code="sal.title.text.emailTwo" /></th>
         <td><input id="srvCntcEmail" name="srvCntcEmail"
          type="text" title="" placeholder="" class="w100p" /></td>
        </tr>
       </tbody>
      </table>
     ...
     <table class="type1">
      ...
      <tbody>
       <tr>
        ...
        <th scope="row"><spring:message
          code="sal.text.exTradeRelatedNo" /></th>
        <td><p>
          <select id="exTrade" name="exTrade" class="w100p"></select>
         </p>
         <p>
          <input id="relatedNo" name="relatedNo" type="text" title=""
           placeholder="Related Number" class="w100p readonly" readonly />
         </p> <a id="btnRltdNo" href="#" class="search_btn blind"><img
          src="${pageContext.request.contextPath}/resources/images/common/normal_search.gif"
          alt="search" /></a></td>
       </tr>
       <tr>
        ...
        <td><input id="installDur" name="installDur" type="text"
         title="" placeholder="Installment Duration (1-36 Months)"
         class="w100p readonly" readonly /></td>
        <th scope="row"><spring:message code="sal.text.ordDate" /><span
         class="must">*</span></th>
        <td>${toDay}</td>
       </tr>
       <tr>
        <th scope="row"><spring:message code="sal.text.refNo" /><span
         class="must">*</span></th>
        <td><input id="refereNo" name="refereNo" type="text"
         title="" placeholder="" class="w100p" /></td>
        <th scope="row"><spring:message code="sal.text.salManCode" /><span
         class="must">*</span></th>
        <td><input id="salesmanCd" name="salesmanCd" type="text"
         title="" placeholder="" class="" /> <input
         id="hiddenSalesmanId" name="salesmanId" type="hidden" /> <a
         id="memBtn" href="#" class="search_btn"><img
          src="${pageContext.request.contextPath}/resources/images/common/normal_search.gif"
          alt="search" /></a></td>
       </tr>
       <tr>
        <th scope="row"><spring:message code="sal.text.poNo" /><span
         class="must">*</span></th>
        <td><input id="poNo" name="poNo" type="text" title=""
         placeholder="" class="w100p" /></td>
        <th scope="row"><spring:message
          code="sal.title.text.salesmanType" /></th>
        <td><input id="salesmanType" name="salesmanType"
         type="text" title="" placeholder="Salesman Type"
         class="w100p readonly" readonly /> <input
         id="hiddenSalesmanTypeId" name="salesmanTypeId" type="hidden" /></td>
       </tr>
       <tr>
        <th scope="row"><spring:message code="sal.text.product" /><span
         class="must">*</span></th>
        <td><select id="ordProudct" name="ordProudct" class="w100p"
         disabled></select> <select id="compType" name="compType"
         class="w100p blind"></select></td>
        <th scope="row"><spring:message code="sal.text.salManName" /></th>
        <td><input id="salesmanNm" name="salesmanNm" type="text"
         title="" placeholder="Salesman Name" class="w100p readonly"
         readonly /></td>
       </tr>
       <tr>
        <th scope="row"><spring:message code="sal.title.text.promo" /><span
         class="must">*</span></th>
        <td><select id="ordPromo" name="ordPromo" class="w100p"
         disabled></select> <input id="txtOldOrderID"
         name="txtOldOrderID" type="hidden" /></td>
        <th scope="row"><spring:message
          code="sal.title.text.salesmanNric" /></th>
        <td><input id="salesmanNric" name="salesmanNric"
         type="text" title="" placeholder="Salesman NRIC"
         class="w100p readonly" readonly /></td>
       </tr>
       <tr>
        <th scope="row"><spring:message
        ...
        <td><input id="departCd" name="departCd" type="text"
         title="" placeholder="Department Code" class="w100p readonly"
         readonly /> <input id="departMemId" name="departMemId"
         type="hidden" /></td>
       </tr>
       <tr>
        <th scope="row"><spring:message
        ...
        <td><input id="grpCd" name="grpCd" type="text" title=""
         placeholder="Group Code" class="w100p readonly" readonly /> <input
         id="grpMemId" name="grpMemId" type="hidden" /></td>
       </tr>
       <tr>
        <th scope="row"><spring:message
          code="sales.promo.discPeriod" />/<br> <spring:message
          code="sal.title.text.finalRentalFees" /></th>
        <td><p>
        ...
        <td><input id="orgCd" name="orgCd" type="text" title=""
         placeholder="Organization Code" class="w100p readonly" readonly />
       ...
       <tr>
        ...
        <td><label><input id="trialNoChk" name="trialNoChk"
          type="checkbox" disabled /><span></span></label> <input id="trialNo"
         name="trialNo" type="text" title="" placeholder="Trial No"
         style="width: 210px;" class="readonly" readonly /> <input
         id="trialId" name="trialId" type="hidden" /> <a
         id="trialNoBtn" name="trialNoBtn" href="#"
         class="search_btn blind"><img
          src="${pageContext.request.contextPath}/resources/images/common/normal_search.gif"
          alt="search" /></a></td>
       </tr>
       <tr>
        <th scope="row"><spring:message code="sal.text.remark" /></th>
        <td colspan="3"><textarea id="ordRem" name="ordRem"
          cols="20" rows="5" placeholder="Remark"></textarea></td>
       </tr>
       <tr>
        <th scope="row"><spring:message code="sal.text.rentPay" /><span
         class="must">*</span></th>
        <td colspan="3"><span><spring:message
           code="sal.msg.6month" /></span> <input id="advPayYes" name="advPay"
         type="radio" value="1" disabled /><span>Yes</span> <input
         id="advPayNo" name="advPay" type="radio" value="0" disabled /><span>No</span></td>
       </tr>
      </tbody>
     </table>
     <!-- table end -->
...
*****************************************************************************-->
   <article id="atcPC" class="tap_area">
    <!-- tap_area start -->
    <section class="search_table">
     <!-- search_table start -->
     ...
     <section id="sctThrdParty" class="blind">
      ...
      <ul class="right_btns mb10">
       <li><p class="btn_grid">
         <a id="thrdPartyAddCustBtn" href="#"><spring:message
...
------------------------------------------------------------------------------->
      <form id="thrdPartyForm" name="thrdPartyForm" action="#"
       method="post">
       <table class="type1">
        <!-- table start -->
        <caption>table</caption>
        ...
        <tbody>
         <tr>
          ...
          <td><input id="thrdPartyId" name="thrdPartyId"
           type="text" title="" placeholder="Third Party ID" class="" />
           <a href="#" class="search_btn" id="thrdPartyBtn"><img
            src="${pageContext.request.contextPath}/resources/images/common/normal_search.gif"
            alt="search" /></a> <input id="hiddenThrdPartyId"
           name="hiddenThrdPartyId" type="hidden" title=""
           placeholder="Third Party ID" class="" /></td>
          <th scope="row"><spring:message code="sal.text.type" /></th>
          <td><input id="thrdPartyType" name="thrdPartyType"
           type="text" title="" placeholder="Costomer Type"
           class="w100p readonly" readonly /></td>
         </tr>
         <tr>
          <th scope="row"><spring:message code="sal.text.name" /></th>
          <td><input id="thrdPartyName" name="thrdPartyName"
           type="text" title="" placeholder="Customer Name"
           class="w100p readonly" readonly /></td>
          <th scope="row"><spring:message
            code="sal.text.nricCompanyNo" /></th>
          <td><input id="thrdPartyNric" name="thrdPartyNric"
           type="text" title="" placeholder="NRIC/Company Number"
           class="w100p readonly" readonly /></td>
         </tr>
        </tbody>
       </table>
        ...
        <tbody>
         <tr>
          ...
          <td><input id="rentPayIC" name="rentPayIC" type="text"
           title="" placeholder="NRIC appear on DD/Passbook"
           class="w100p" /></td>
         </tr>
        </tbody>
       </table>
...
------------------------------------------------------------------------------->
       ...
       <table class="type1 mb1m">
        ...
        <tbody>
         <tr>
          ...
          <td><input id="rentPayCRCNo" name="rentPayCRCNo"
           type="text" title="" placeholder="Credit Card Number"
           class="w100p readonly" readonly /> <input
           id="hiddenRentPayCRCId" name="rentPayCRCId" type="hidden" />
           <input id="hiddenRentPayEncryptCRCNoId"
           name="hiddenRentPayEncryptCRCNoId" type="hidden" /></td>
          <th scope="row"><spring:message
            code="sal.text.creditCardType" /></th>
          <td><input id="rentPayCRCType" name="rentPayCRCType"
           type="text" title="" placeholder="Credit Card Type"
           class="w100p readonly" readonly /></td>
         </tr>
         <tr>
          <th scope="row"><spring:message
            code="sal.text.nameOnCard" /></th>
          <td><input id="rentPayCRCName" name="rentPayCRCName"
           type="text" title="" placeholder="Name On Card"
           class="w100p readonly" readonly /></td>
          <th scope="row"><spring:message code="sal.text.expiry" /></th>
          <td><input id="rentPayCRCExpiry" name="rentPayCRCExpiry"
           type="text" title="" placeholder="Credit Card Expiry"
           class="w100p readonly" readonly /></td>
         </tr>
         <tr>
          <th scope="row"><spring:message code="sal.text.issueBank" /></th>
          <td><input id="rentPayCRCBank" name="rentPayCRCBank"
           type="text" title="" placeholder="Issue Bank"
           class="w100p readonly" readonly /> <input
          ...
          <td><input id="rentPayCRCCardType"
     ...
     <section id="sctDirectDebit" class="blind">
      <aside class="title_line">
       <!-- title_line start -->
       <h3>
        <spring:message code="sal.title.text.directDebit" />
       </h3>
      </aside>
      <!-- title_line end -->
      <ul class="right_btns mb10">
...
------------------------------------------------------------------------------->
       ...
       <table class="type1">
        ...
        <tbody>
         <tr>
          ...
          <td><input id="rentPayBankAccNo" name="rentPayBankAccNo"
           type="text" title="" placeholder="Account Number readonly"
           class="w100p readonly" readonly /> <input
           id="hiddenRentPayBankAccID" name="hiddenRentPayBankAccID"
           type="hidden" /></td>
          <th scope="row"><spring:message
            code="sal.text.accountType" /></th>
          <td><input id="rentPayBankAccType"
           name="rentPayBankAccType" type="text" title=""
         ...
         <tr>
          ...
          <td><input id="accName" name="accName" type="text"
           title="" placeholder="Account Holder" class="w100p readonly"
           readonly /></td>
          <th scope="row"><spring:message
            code="sal.text.issueBankBranch" /></th>
          <td><input id="accBranch" name="accBranch" type="text"
           title="" placeholder="Issue Bank Branch"
           class="w100p readonly" readonly /></td>
         </tr>
         <tr>
          <spring:message code="sal.text.ddcChnl" var="ddcChnl"/> <!-- DEDUCTION CHANNEL -->
          <th scope="row">${ddcChnl}</th>
          <td><input id="ddcChl" name="ddcChl"
           type="text" title="" placeholder="${ddcChnl}"
           class="w100p readonly" readonly /> <input
           id="hiddenAccBankId" name="hiddenDdcChl" type="hidden" /></td>
          <th scope="row"><spring:message code="sal.text.issueBank" /></th>
          <td><input id="accBank" name="accBank"
           type="text" title="" placeholder="Issue Bank"
           class="w100p readonly" readonly /> <input
           id="hiddenAccBankId" name="hiddenAccBankId" type="hidden" /></td>
         </tr>
        </tbody>
       </table>
       <!-- table end -->
   ...
   <!--****************************************************************************
    Billing Detail
*****************************************************************************-->
   <article id="atcBD" class="tap_area">
    <!-- tap_area start -->
    <section class="search_table">
     <!-- search_table start -->
     ...
     <table class="type1">
      ...
      <tbody>
       <tr>
        <th scope="row"><spring:message code="sal.text.groupOption" /><span
         class="must">*</span></th>
...
------------------------------------------------------------------------------->
     <section id="sctBillMthd" class="blind">
      <form id="billMthdForm" name="billMthdForm" action="#"
       method="post">
       <table class="type1 mb1m">
        <!-- table start -->
        <caption>table</caption>
        ...
        <tbody>
         <tr>
          ...
          <td colspan="3"><label><input id="billMthdPost"
            name="billMthdPost" type="checkbox" /><span><spring:message
              code="sal.text.post" /></span></label></td>
         </tr>
         <tr>
          <td colspan="3"><label><input id="billMthdSms"
            name="billMthdSms" type="checkbox" /><span><spring:message
         ...
         <tr>
          <td><label><input id="billMthdEstm"
            name="billMthdEstm" type="checkbox" /><span><spring:message
...
------------------------------------------------------------------------------->
     <section id="sctBillAddr" class="blind">
      <form id="billAddrForm" name="billAddrForm" action="#"
       method="post">
        ...
        <tbody>
         <tr>
          ...
          <td colspan="3"><input id="billAddrDtl"
           ...
           placeholder="Address Detail" class="w100p readonly" readonly /></td>
         </tr>
         <tr>
         ...
         <tr>
          ...
          <td><input id="billCity" name="billCity" type="text"
           title="" placeholder="City" class="w100p readonly" readonly /></td>
         ...
         <tr>
          ...
          <td><input id="billState" name="billState" type="text"
           title="" placeholder="State" class="w100p readonly" readonly /></td>
...
------------------------------------------------------------------------------->
      <section class="search_table">
       <!-- search_table start -->
       <form id="billPreferForm" name="billPreferForm" action="#"
        method="post">
         ...
         <tbody>
          ...
          <tr>
           ...
           <td><input id="billPreferTelO" name="billPreferTelO"
            type="text" title="" placeholder="Tel(Office)" class="w100p"
            readonly /></td>
...
------------------------------------------------------------------------------->
       ...
       <table class="type1">
        ...
        <tbody>
         <tr>
          ...
          <td><input id="billGrp" name="billGrp" type="text"
           title="" placeholder="Billing Group" class="readonly"
           readonly /><a id="billGrpBtn" href="#" class="search_btn"><img
            src="${pageContext.request.contextPath}/resources/images/common/normal_search.gif"
            alt="search" /></a> <input id="hiddenBillGrpId"
           name="billGrpId" type="hidden" /></td>
          <th scope="row"><spring:message
            code="sal.text.billingType" /><span class="must">*</span></th>
          <td><input id="billType" name="billType" type="text"
           title="" placeholder="Billing Type" class="w100p readonly"
           readonly /></td>
         </tr>
         <tr>
          <th scope="row"><spring:message
            code="sal.text.billingAddress" /></th>
          <td colspan="3"><textarea id="billAddr" name="billAddr"
     ...
     <table class="type1">
      ...
      <tbody>
       <tr>
        <th scope="row"><spring:message code="sal.text.remark" /></th>
        <td><textarea id="billRem" name="billRem" cols="20"
          rows="5" readonly></textarea></td>
       </tr>
      </tbody>
     </table>
     <!-- table end -->
...
*****************************************************************************-->
   <article class="tap_area">
    ...
    <section class="search_table">
     ...
     <!-- title_line end -->
     <ul class="right_btns mb10">
      <li id="liInstNewAddr" class="blind"><p class="btn_grid">
        <a id="instNewAddrBtn" href="#"><spring:message
          code="sal.btn.addNewAddr" /></a>
       </p></li>
      <li id="liInstSelAddr" class="blind"><p class="btn_grid">
        <a id="instSelAddrBtn" href="#"><spring:message
          code="sal.title.text.selectAnotherAddress" /></a>
       </p></li>
     </ul>
     <!------------------------------------------------------------------------------
...
------------------------------------------------------------------------------->
     <form id="instAddrForm" name="instAddrForm" action="#"
      method="post">
      <input id="hiddenCustAddId" name="custAddId" type="hidden" /> <input
       id="validAreaIdYN" name="validAreaIdYN" type="hidden" value="Y" />
      <table class="type1">
       <!-- table start -->
       <caption>table</caption>
       <colgroup>
       ...
       <tbody>
        <tr>
         <th scope="row"><spring:message
           code="sal.text.addressDetail" /><span class="must">*</span></th>
         <td colspan="3"><input id="instAddrDtl" name="instAddrDtl"
          type="text" title="" placeholder="Address Detail"
          class="w100p readonly" readonly /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.street" /><span
          class="must">*</span></th>
         <td colspan="3"><input id="instStreet" name="instStreet"
          type="text" title="" placeholder="Street"
          class="w100p readonly" readonly /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.area" /><span
          class="must">*</span></th>
         <td colspan="3"><input id="instArea" name="instArea"
          type="text" title="" placeholder="Area" class="w100p readonly"
          readonly /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.city" /><span
          class="must">*</span></th>
         <td><input id="instCity" name="instCity" type="text"
          title="" placeholder="City" class="w100p readonly" readonly /></td>
         <th scope="row"><spring:message code="sal.text.postCode" /><span
          class="must">*</span></th>
         <td><input id="instPostCode" name="instPostCode"
          type="text" title="" placeholder="Post Code"
          class="w100p readonly" readonly /></td>
        </tr>
        <tr>
         <th scope="row"><spring:message code="sal.text.state" /><span
          class="must">*</span></th>
         <td><input id="instState" name="instState" type="text"
          title="" placeholder="State" class="w100p readonly" readonly /></td>
         <th scope="row"><spring:message code="sal.text.country" /><span
          class="must">*</span></th>
         <td><input id="instCountry" name="instCountry" type="text"
          title="" placeholder="Country" class="w100p readonly" readonly /></td>
        </tr>
       </tbody>
      </table>
      <!-- table end -->
     ...
     <section id="tbInstCntcPerson" class="blind">
      ...
      <!-- title_line end -->
      <ul class="right_btns mb10">
       <li id="liInstNewAddr2" class="blind"><p class="btn_grid">
         <a href="#"><spring:message code="sal.btn.addNewAddr" /></a>
        </p></li>
       <li id="liInstSelAddr2" class="blind"><p class="btn_grid">
...
------------------------------------------------------------------------------->
      <form id="instCntcForm" name="instCntcForm" action="#"
       method="post">
       ...
       <table class="type1">
        ...
        <tbody>
         <tr>
          <th scope="row"><spring:message code="sal.text.name" /><span
           class="must">*</span></th>
          <td><input id="instCntcName" name="instCntcName"
           type="text" title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"><spring:message code="sal.text.initial" /></th>
          <td><input id="instInitial" name="instInitial"
           type="text" title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"><spring:message code="sal.text.gender" /></th>
          <td><input id="instGender" name="instGender" type="text"
           title="" placeholder="" class="w100p" readonly /></td>
         </tr>
         <tr>
          <th scope="row"><spring:message code="sal.text.nric" /></th>
          <td><input id="instNric" name="instNric" type="text"
           title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"><spring:message code="sal.text.dob" /></th>
          <td><input id="instDob" name="instDob" type="text"
           title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"><spring:message code="sal.text.race" /></th>
          <td><input id="instRaceId" name="instRaceId" type="text"
           title="" placeholder="" class="w100p" readonly /></td>
         </tr>
         <tr>
          <th scope="row"><spring:message code="sal.text.email" /></th>
          <td><input id="instCntcEmail" name="instCntcEmail"
           type="text" title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"><spring:message code="sal.text.dept" /></th>
          <td><input id="instDept" name="instDept" type="text"
           title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"><spring:message code="sal.text.post" /></th>
          <td><input id="instPost" name="instPost" type="text"
           title="" placeholder="" class="w100p" readonly /></td>
         </tr>
         <tr>
          <th scope="row"><spring:message
            code="sal.title.text.telM" /></th>
          <td><input id="instCntcTelM" name="instCntcTelM"
           type="text" title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"><spring:message
            code="sal.title.text.telR" /></th>
          <td><input id="instCntcTelR" name="instCntcTelR"
           type="text" title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"><spring:message
            code="sal.title.text.telO" /></th>
          <td><input id="instCntcTelO" name="instCntcTelO"
           type="text" title="" placeholder="" class="w100p" readonly /></td>
         </tr>
         <tr>
          <th scope="row"><spring:message
            code="sal.title.text.telF" /></th>
          <td><input id="instCntcTelF" name="instCntcTelF"
           type="text" title="" placeholder="" class="w100p" readonly /></td>
          <th scope="row"></th>
          <td></td>
          <th scope="row"></th>
...
------------------------------------------------------------------------------->
     <form id="installForm" name="installForm" action="#" method="post">
      <table class="type1">
       ...
       <tbody>
        <tr>
         <th scope="row"><spring:message code="sal.text.zeroGst" /><span
          class="must">*</span></th>
         <td>
          <p>
           <select id="gstChk" name="gstChk" class="w100p"></select>
          </p>
          <p id="pBtnCal" class="btn_sky blind">
           <a id="btnCal" href="#">Calculation</a>
          </p>
         </td>
         <th scope="row"><spring:message
           code="sal.title.text.dscBrnch" /><span class="must">*</span></th>
         <td><select id="dscBrnchId" name="dscBrnchId"
          class="w100p" disabled></select></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
           code="sal.title.text.perferInstDate" /><span class="must">*</span></th>
         <td><input id="prefInstDt" name="prefInstDt" type="text"
          title="Create start Date"
          placeholder="Prefer Install Date (dd/MM/yyyy)"
         ...
         <td>
          <div class="time_picker">
           <!-- time_picker start -->
           <input id="prefInstTm" name="prefInstTm" type="text" title=""
            placeholder="Prefer Install Time (hh:mi tt)"
        ...
        <tr>
         <th scope="row"><spring:message
          ...
          class="must">*</span></th>
         <td colspan="3"><textarea id="speclInstct"
           name="speclInstct" cols="20" rows="5"></textarea></td>
        </tr>
       </tbody>
      </table>
      <!-- table end -->
...
*****************************************************************************-->
   <article id="atcRC" class="tap_area">
    <!-- tap_area start -->
    <section class="search_table">
     <!-- search_table start -->
     <form id="fileUploadForm" name="fileUploadForm"
       ...
       <tbody>
        <tr>
         ...
         <td><input id="certRefNo" name="certRefNo" type="text"
          title="" placeholder="Cert Reference No" class="w100p" /></td>
         <th scope="row"><spring:message
        ...
        <tr>
         ...
         <td colspan="3"><textarea id="txtCertRemark"
           name="txtCertRemark" cols="20" rows="5"></textarea></td>
        </tr>
        <tr>
         <th scope="row"><spring:message
      ...
      <!-- table end -->
      <ul class="left_opt">
       <li><span class="red_text">**</span> <span
        class="brown_text"><spring:message
          code="sal.msg.zeroGstMsg" /></span></li>
      </ul>
      <ul class="center_btns">
       <li><p class="btn_blue">
         <a name="ordSaveBtn" href="#"><spring:message
