...
<div class="popup_wrap">
    <header class="pop_header">
        <h1>Pre-CCP Quota – File Upload</h1>
        <ul class="right_opt">
    ...
    <section class="pop_body">
        <p style="color: #25527c; padding-left: 5px;">Pre-CCP Quota – File Upload</p><br/>
        <form method="post">
            <table class="type1">
        ...
        <ul class="center_btns mt20">
            ...
            <li><p class="btn_blue2 big"><a href="${pageContext.request.contextPath}/resources/download/sales/PreCcpQuotaUploadTemplate.csv">Download Template</a></p></li>
        </ul>
 </section>
</div>
...
<script>
    ...
    const validation = () =>{
          if (document.getElementById("quotaFile").parentElement.parentElement.querySelector("input[type=file]").files.length == 0) {
                Common.alert("Kindly upload Pre-CCP Quota File.")
                return false;
          }

          if (document.getElementById("quotaFile").parentElement.parentElement.querySelector("input[type=file]").files[0].type !="text/csv") {
              Common.alert("Kindly upload Pre-CCP Quota File with CSV format only.")
              return false;
          }

          if (document.getElementById("quotaFile").parentElement.parentElement.querySelector("input[type=file]").files[0].size > 2000000) {
              Common.alert("The file size of Pre-CCP Quota File cannot upload over 2MB.")
              return false;
          }
    ...
    $("#btnUploadFile").click((e)=>{
        ...
        if(validation()){
            ...
            formData.append("newUpload", document.getElementById("quotaFile").parentElement.parentElement.querySelector("input[type=file]").files[0]);
            fetch("/sales/ccp/submitQuota.do",{
                method: "POST",
                body: formData
            })
            .then((r)=> {
            	if(r.status == 500){
            		Common.removeLoader();
            		Common.alert("Upload failed! \n Kindly raise ticket with uploaded file attached.");
            	}else{
            		return r.json();
            	}
            })
