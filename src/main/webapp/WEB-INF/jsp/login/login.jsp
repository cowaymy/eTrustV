    ...
    <article class="login_box"><!-- login_box start -->
        <form id="loginForm" name="loginForm" method="post">
            ...
            <p><input type="password" title="PASSWORD" placeholder="PASSWORD" id="password" name="password"
                      value="ivy123"/></p>
            <p class="id_save"><label><input id="saveId" type="checkbox"/><span><spring:message
                    code='sys.info.id'/> <spring:message code='sys.btn.save'/></span></label></p>
            <p class="lang">
                <span><spring:message code='sys.info.lang'/> <spring:message code='sys.info.select'/></span>
                <select id="language" name="language">
                    <c:forEach var="list" items="${languages}">
                        <option value="${list.language}" <c:if
                                test="${param.language eq list.language}"> selected </c:if>>${list.language}</option>
                    </c:forEach>
                </select>
            </p>
            <p class="login_btn"><a href="javascript:void(0);" onclick="javascript:fn_login();"><spring:message
                    code='sys.btn.login'/></a></p>
            <ul class="login_opt">
