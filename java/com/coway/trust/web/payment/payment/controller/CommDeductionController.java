...
public class CommDeductionController {
	...
	public ResponseEntity<ReturnMessage> csvUpload(MultipartHttpServletRequest request, SessionVO sessionVO) throws IOException, InvalidFormatException {
		...
		String message = "";
		String memType = request.getParameter("cmbMemberType").toString();
		...
		if(sessionVO.getUserId() > 0){
    		...
    		for (CommDeductionVO vo : vos) {
    			LOGGER.debug("getOrderNo : {}, getmCode : {}, getAmount : {}, getPaidMonth : {}, getDeductCode : {}, getAppType : {} ", vo.getOrderNo(), vo.getmCode(), vo.getAmount(), vo.getPaidMonth(), vo.getDeductCode(), vo.getAppType());
    			...
    			hm.put("deductCode", vo.getDeductCode());
    			hm.put("appType", vo.getAppType());
    			list.add(hm);
    		}
    		...
    		String yy = String.valueOf(yyyy).substring(2, 4);
    		...
    		m.put("fileRefNo", "COM" + yy + mm + memType);
    		m.put("totalRecords", list.size());
    		m.put("totalAmount", sumAmount(vos));
    		m.put("fileStatus", 1);
    		m.put("memTypeId",memType);
    		m.put("actionDate",dt.substring(0,6));
    		...
    		if(result > 0){
    			...
    			message = "Failed to save : Only one upload is allowed per month";
    		}
		}else{
