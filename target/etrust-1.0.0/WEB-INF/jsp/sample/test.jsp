...
####################################
 ...
 // AUIGrid 생성 후 반환 ID
    ...
    function fn_getSampleListAjax() {
        //  validation check
/*
        if (!isValid("agencyId", "가맹점ID 항목은 필수 값입니다.")) {
            return false;
        }
        
        if (!isValid("agencyName", "가맹점명 항목은 필수 값입니다.")) {
            return false;
        }*/
    ...
    function fn_save(){
        Common.ajax("POST", "/sample/saveSample.do", $("#detailForm").serializeObject(), function(result) {
            console.log("성공.");
        ...
        $.ajax({
            type : "POST",
            url : "/sample/saveSample.do",
            dataType : "json",
            data : JSON.stringify($("#detailForm").serializeObject()),
            contentType : "application/json;charset=UTF-8",
            beforeSend: function(){
            },
            ...
            <div id="main">
                ...
                <div id="divDetail">
                    <form id="detailForm" method="post" action="">
                        ...
                        <input type="button" class="btn" onclick="javascript:fn_save();" value="전송"/>
                    </form>
                </div>
            </div>
        </div>
