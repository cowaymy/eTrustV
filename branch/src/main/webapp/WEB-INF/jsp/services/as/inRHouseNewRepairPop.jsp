...
<script type="text/javaScript">
  ...
  $(document).ready(
    function() {
      ...
      $("#resultASAllForm #asInAsNo").val(selectedItems[0].item.salesOrdNo);
      $("#resultASAllForm #asOpenAsNo").val(selectedItems[0].item.asNo);
      $("#resultASAllForm #asCloseAsNo").val(selectedItems[0].item.onAsNo);
      $("#resultASAllForm #asResultCrtDt").val(selectedItems[0].item.inAppntDt);
      $("#resultASAllForm #asInHouseComDt").val(selectedItems[0].item.inAsComDt);

      $("#resultASAllForm #ddlDSCCode").val(selectedItems[0].item.asBrnchId);
      $("#resultASAllForm #ddlDSCCodeText").val(selectedItems[0].item.asBrnchDesc);
      $("#resultASAllForm #ddlCTCode").val(selectedItems[0].item.asCtId);
      $("#resultASAllForm #ddlCTCodeText").val(selectedItems[0].item.memCode);
  ...
  function fn_openField_Cancel() {
    ...
    /*$('#def_type').val("").attr("disabled", "disabled");
    $('#def_code').val("").attr("disabled", "disabled");
    $('#def_part').val("").attr("disabled", "disabled");
    $('#solut_code').val("").attr("disabled", "disabled");
    $('#def_def').val("").attr("disabled", "disabled");*/
    ...
    /*$('#def_type_text').val("").attr("disabled", "disabled");
    $('#def_code_text').val("").attr("disabled", "disabled");
    $('#def_part_text').val("").attr("disabled", "disabled");
    $('#def_def_text').val("").attr("disabled", "disabled");
    $('#solut_code_text').val("").attr("disabled", "disabled");*/

    $('#def_type_text').attr("disabled", "disabled");
    $('#def_code_text').attr("disabled", "disabled");
    $('#def_part_text').attr("disabled", "disabled");
    $('#def_def_text').attr("disabled", "disabled");
    $('#solut_code_text').attr("disabled", "disabled");
  ...
  function fn_setSaveFormData() {
    ...
    var asResultM = {
      AS_SO_ID : $("#resultASForm #ORD_ID").val(),
      REF_REQUEST : $("#resultASForm #AS_ID").val(),

      AS_CT_ID : $('#resultASAllForm #ddlCTCode').val(),
      AS_SETL_DT : $('#resultASAllForm #dpSettleDate').val(),
      AS_SETL_TM : $('#resultASAllForm #tpSettleTime').val(),
      AS_RESULT_STUS_ID : $('#resultASAllForm #ddlStatus').val(),
      AS_FAIL_RESN_ID : $('#resultASAllForm #ddlFailReason').val(),
      AS_REN_COLCT_ID : 0,
      AS_CMMS : $("#resultASAllForm #iscommission").prop("checked") ? '1' : '0',
      AS_BRNCH_ID : $('#resultASAllForm #ddlDSCCode').val(),
      AS_WH_ID : $('#resultASAllForm #ddlWarehouse').val(),
      AS_RESULT_REM : $('#resultASAllForm #txtRemark').val(),
      AS_MALFUNC_ID : $('#resultASAllForm #ddlErrorCode').val(),
      AS_MALFUNC_RESN_ID : $('#resultASAllForm #ddlErrorDesc').val(),
      AS_DEFECT_TYPE_ID : $('#resultASAllForm #def_type_id').val(),
      AS_DEFECT_GRP_ID : 0,
      AS_DEFECT_ID : $('#resultASAllForm #def_code_id').val(),
      AS_DEFECT_PART_GRP_ID : 0,
      AS_DEFECT_PART_ID : $('#resultASAllForm #def_part_id').val(),
      AS_DEFECT_DTL_RESN_ID : $('#resultASAllForm #def_def_id').val(),
      AS_SLUTN_RESN_ID : $('#resultASAllForm #solut_code_id').val(),
      AS_WORKMNSH : $('#resultASAllForm #txtLabourCharge').val(),
      AS_FILTER_AMT : $('#resultASAllForm #txtFilterCharge').val(),
      AS_ACSRS_AMT : 0,
      AS_TOT_AMT : $('#resultASAllForm #txtTotalCharge').val(),
      ...
      IN_HUSE_REPAIR_PROMIS_DT : $("#inHouseRepair_div #promisedDate").val(),
      IN_HUSE_REPAIR_GRP_CODE : $("#productGroup").val(),
      IN_HUSE_REPAIR_PRODUCT_CODE : $("#productCode").val(),
      IN_HUSE_REPAIR_SERIAL_NO : $("#serialNo").val(),
      APPNT_DT : $("#resultASAllForm #APPNT_DT").val(),
      AS_MEM_ID : $('#resultASAllForm #ddlCTCode').val(),
      AS_STUS_ID : $('#resultASAllForm #ddlStatus').val(),
      AS_TYPE_ID : '2713'
    }
    ...
    Common.ajax("POST", "/services/as/saveASInHouseEntry.do", saveForm,
      function(result) {
        if (result.data != "") {
          Common.alert("<b>AS result successfully saved.</b>  </BR> New ASNo["
                      + result.asNo
                      + "] New ResultNo ["
                      + result.asResultNo + "]</b>");
          $("#txtResultNo").html("<font size='3' color='red'> <b> "
                               + result.asResultNo
                               + " </b></font>");
          fn_DisablePageControl();
          fn_selInhouseList();
  ...
  function fn_validRequiredField_Save_ResultInfo() {
    ...
    if (FormUtil.checkReqValue($("#ddlStatus"))) {
      rtnMsg += "* <spring:message code='sys.msg.necessary' arguments='Status' htmlEscape='false'/> </br>";
      rtnValue = false;
    } else {
      if ($("#ddlStatus").val() == 4 || $("#ddlStatus").val() == 1) {
        ...
        if (FormUtil.checkReqValue($("#tpSettleTime"))) {
          rtnMsg += "* <spring:message code='sys.msg.necessary' arguments='Sattle Time' htmlEscape='false'/> </br>";
          rtnValue = false;
        }
  ...
  function fn_loadAsRst() {
	  console.log("IN");
    var selectedItems = AUIGrid.getSelectedItems(inHouseRGridID)
    var refReqst = "";
    var asRslt = selectedItems[0].item.asResultNo;

    Common.ajax("GET", "/services/inhouse/getASRulstSVC0004DInfo.do",{REF_REQST:refReqst , AS_RESULT_NO:asRslt}, function(result) {
      if(result != null ){
        ...
        asErrorCode = result[0].asMalfuncId;
        ...
        $("#inHouseRepair_div #promisedDate").val(result[0].inHuseRepairPromisDt);
        $("#inHouseRepair_div #productGroup").val(result[0].inHuseRepairGrpCode);
        $("#inHouseRepair_div #productCode").val(result[0].inHuseRepairProductCode);
        $("#inHouseRepair_div #serialNo").val(result[0].inHuseRepairSerialNo);
        $("#inHouseRepair_div #inHouseRemark").val(result[0].inHuseRepairRem);
      ...
      <table class="type1">
       ...
       <tbody>
        <tr>
         ...
         <td><input title="" class="readonly w100p" type="text"
          placeholder="" disabled="disabled" id="asInAsNo" name=asInAsNo>
         </td>
        </tr>
        <tr>
         <th scope="row">Open AS Order No</th>
         <td><input type="text" title=""
          placeholder="Open AS Order No " disabled="disabled"
          class="readonly" id="asOpenAsNo" name="asOpenAsNo" /></td>
         <th scope="row">Close AS Order No</th>
         <td><input type="text" title=""
          placeholder="Close AS Order No" disabled="disabled"
       ...
       <table class="type1">
        ...
        <tbody>
         ...
         <tr>
          ...
          <td><input type="hidden" title="" placeholder="" class=""
           id='ddlCTCode' name='ddlCTCode' value='${USER_ID}' /> <input
           type="text" title="" placeholder="" class=""
           id='ddlCTCodeText' name='ddlCTCodeText' disabled="disabled" value='${USER_NAME}' />
          </td>
         </tr>
         <tr>
       ...
       <table class="type1">
        ...
        <tbody>
         <tr>
          ...
          <td><input type="text" title="" id='def_type' disabled="disabled" name='def_type' placeholder="ex) DT3" class="" onblur="fn_getASReasonCode2(this, 'def_type' ,'387')" onkeyup="this.value = this.value.toUpperCase();" /> <input
           type="hidden" title="" id='def_type_id' name='def_type_id' placeholder="" class="" />
