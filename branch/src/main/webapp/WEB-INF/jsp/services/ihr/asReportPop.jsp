...
<script type="text/javaScript">
  $(document).ready(
    function() {
      ...
      doGetCombo('/services/inhouse/report/selectMemCodeList.do', '', '', 'CTCodeFrom', 'S', '');
      doGetCombo('/services/inhouse/report/selectMemCodeList.do', '', '', 'CTCodeTo', 'S', '');
    });
  ...
  function fn_openGenerate() {
    if (fn_validation()) {
      ...
      if ($("#asAppDtFr").val() != '') {
        ...
        if (appDate2 != '') {
          whereSql += " AND (AE.AS_APPNT_DT >= TO_DATE('" + appDate1 + "', 'DD/MM/YYYY') AND AE.AS_APPNT_DT <= TO_DATE('" + appDate2 + "' , 'DD/MM/YYYY')) ";
        } else {
          whereSql += "";
        }
        ...
        if (reqDate1 != '' && reqDate2 != '') {
          whereSql += " AND (AE.AS_REQST_DT >= TO_DATE('" + reqDate1 + "', 'DD/MM/YYYY') AND AE.AS_REQST_DT <= TO_DATE('" + reqDate2 + "' , 'DD/MM/YYYY')) ";
        } else {
          whereSql += "";
        }
      }
      ...
      $("#reportFormIHR #V_FSONO1").val(fSONO1);
      $("#reportFormIHR #V_FSONO2").val(fSONO2);
      $("#reportFormIHR #V_FMEMCTID1").val(fMemCTID1);
      $("#reportFormIHR #V_FMEMCTID2").val(fMemCTID2);
      $("#reportFormIHR #V_ORDERBY").val(orderBy);
      $("#reportFormIHR #V_DSCBRANCH").val(dscBranch);
      $("#reportFormIHR #V_WHERESQL").val(whereSql);
      $("#reportFormIHR #reportFileName").val('/services/IHReport.rpt');
      $("#reportFormIHR #viewType").val("PDF");
      $("#reportFormIHR #reportDownFileName").val("IHReport_" + day + month + date.getFullYear());

      var option = {
        isProcedure : true, // procedure 로 구성된 리포트 인경우 필수.
      };

      Common.report("reportFormIHR", option);
    }
  }
...
<div id="popup_wrap" class="popup_wrap">
 ...
 <header class="pop_header">
  <!-- pop_header start -->
  <h1><spring:message code='service.btn.asRpt' /></h1>
  <ul class="right_opt">
 ...
 <section class="pop_body">
    ...
    <table class="type1">
     ...
     <tbody>
      ...
      <tr>
       <td colspan="2">
        <p>
         <span style="color:red;font-weight:bold">* Please Choose either Appointment Date or Request Date</span>
        </p>
       </td>
  ...
  <ul class="center_btns">
   ...
   <li><p class="btn_blue2 big">
     <a href="#" onclick="javascript:$('#reportFormIHR').clearForm();"><spring:message code='service.btn.Clear' /></a>
    </p></li>
  </ul>
 </section>
 <!-- pop_body end -->
