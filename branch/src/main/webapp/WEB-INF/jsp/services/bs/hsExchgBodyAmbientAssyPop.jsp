...
function fn_report(viewType){
    ...
    if($("#cmbSort :selected").index() > -1){
        ...
        if($("#cmbSort :selected").val() == "1"){
            ...
            orderBySQL = " ORDER BY D.CODE, D.NAME, som.SALES_ORD_NO ";
    ...
    $("#reportDownFileName").val("ExchgBodyAmbientAssyRawData_"+date+(new Date().getMonth()+1)+new Date().getFullYear());


     if(viewType == "EXCEL"){
        $("#form #viewType").val("EXCEL");
        $("#form #reportFileName").val("/services/hsExchgBodyAmbientAssy_Excel.rpt");
    }
...
CommonCombo.make('cmbAppType', '/services/bs/getAppTypeList', {codeId : 10} , '', {type: 'M'});
