...
var advKeyInLayout = [ 
    {dataField : "orNo",headerText : "OR No",width : 100 , editable : false},
    {dataField : "trNo",headerText : "TR No",width : 100 , editable : false},
	{dataField : "payItmRefDt",headerText : "<spring:message code='pay.head.transactionDate'/>",width : 100 , editable : false},
	{dataField : "brnch",headerText : "Brnch",width : 100 , editable : false},
...
function fn_searchAdvMatchList(){
	...
	Common.ajax("POST","/payment/selectReconciliationList.do", $("#searchForm").serializeJSON(), function(result){
...
function fn_mapping(){	
	...
	if( advKeyInItems.length < 1 || bankStmtItem.length < 1){
		...
		Common.alert("Please check Pos Key In List and Bank Statement List");
		return;
	}else{
		 ...
		 console.log("advKeyInItems.length==>" +advKeyInItems.length);
		 for (var i = 0; i < advKeyInItems.length ; i++) {
			 
			 console.log("advKeyInItems.length==>" +advKeyInItems.length);
			 
			 totAmtSum  +=  Number(advKeyInItems[i].item.totAmt) ;
			 
			 selGroupSeq  = selGroupSeq +","+advKeyInItems[i].item.groupSeq;
	     }
		  
		console.log("totAmtSum==>" +totAmtSum);
		
		
		keyInRowItem = advKeyInItems[0];
		stateRowItem = bankStmtItem[0];

		keyInAmount = Number(totAmtSum) - Number(keyInRowItem.item.bankChgAmt);
		...
		acc_id = stateRowItem.item.fBankJrnlAccId;
...
function fn_saveMapping(withPop){
	...
	Common.confirm("<spring:message code='pay.alert.wantToSave'/>",function (){
	    Common.ajax("POST", "/payment/savePosKeyPaymentMapping.do", $("#entryForm").serializeJSON(), function(result) {
...
<section id="content"><!-- content start -->
    ...
    <aside class="title_line"><!-- title_line start -->
        ...
        <h2>Reconciliation for POS </h2>
        <ul class="right_btns">
            <li><p class="btn_blue"><a href="javascript:fn_mapping();"><span class="search"></span>Match</a></p></li>
