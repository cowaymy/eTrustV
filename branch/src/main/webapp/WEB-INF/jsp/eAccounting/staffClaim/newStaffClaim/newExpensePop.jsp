...
function fn_addDtlsPop() {
    ...
    Common.popupDiv("/newStaffClaim/newDtlsPop.do", {callType:"new", claimNo: claimNo, type: "nc"}, null, true, "newStaffDtlsClaimPop");
...
function fn_addCMDtlsPop() {
    ...
    Common.ajax("GET", "/newStaffClaim/checkCM.do", {claimNo : claimNo}, function(result) {
        console.log(result);

        if(result.cnt == 0) {
            Common.popupDiv("/newStaffClaim/newDtlsPop.do", {callType:"new", claimNo: claimNo, type: "cm"}, null, true, "newStaffDtlsClaimPop");
        } else {
            Common.popupDiv("/newStaffClaim/newDtlsPop.do", {callType:"new", claimNo: claimNo, type: "cm", clamUn : result.clamUn}, null, true, "newStaffDtlsClaimPop");
        }
...
function fn_confirmClaim() {
    ...
    if($("#claimType").val() == "J4") {
        Common.ajax("GET", "/newStaffClaim/getClaimNo.do", null, function(result) {
            console.log(result);
            ...
            Common.ajax("POST", "/newStaffClaim/saveClaim.do", genClaimData, function(result1) {
                console.log(result1);
            })
        })
    }
...
function fn_removeClaim(clmNo, clmUn, source, clmType) {
    ...
    Common.confirm("Do you wish to delete this claim?", function() {
        ...
        Common.ajax("GET", "/newStaffClaim/removeClaim.do", {claimNo : clmNo, clamUn : clmUn, clmType : clmType}, function(result) {
            console.log(result);
        });
    });
...
function fn_approveLinePop() {
    ...
    Common.ajax("POST", "/eAccounting/staffClaim/checkOnceAMonth.do?_cacheId=" + Math.random(), {clmType : $("#claimType").val(), memAccId : $("#newMemAccId").val(), clmMonth : $("#newClmMonth").val()}, function(result) {
        ...
        if(result.data > 0) {
            ...
            Common.popupDiv("/newStaffClaim/approveLinePop.do", {claimNo : claimNo}, null, true, "approveLineSearchPop");
        }
    });
}
 ...
 function fn_setEvent() {
    $("#form_newStaffClaim :text").change(function(){
     ...
     AUIGrid.bind(newGridID, "cellDoubleClick", function(event) {
         ...
         Common.popupDiv("/newStaffClaim/viewDtlsPop.do", {claimNo: claimNo, clmUn: event.item.clamUn, claimType: event.item.claimType}, null, true, "viewStaffDtlsClaimPop");
     });
}
    ...
    if(fn_checkEmpty()) {
        ...
        if($(":input:radio[name=expGrp]:checked").val() == "1") {
            ...
            if(clmSeq == 0) {
                ...
                //Common.ajaxFile("/newStaffClaim/cmFileUpload.do", formData, function(result) {
                Common.ajaxFile("/newStaffClaim/ncFileUpload.do", formData, function(result) {
                    console.log(result);
                    ...
                    Common.ajax("POST", "/newStaffClaim/saveClaimDtls.do", data1, function(result1) {
                        console.log(result1);

                        AUIGrid.setGridData(newGridID, result1.summaryGrid);
                        AUIGrid.setGridData(hNewGridID, result1.hiddenGrid)

                        Common.ajax("GET", "/newStaffClaim/getTotal.do", {claimNo: claimNo}, function(result2) {
                            console.log(result2);
