...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_26T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_26T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_26T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      Common
                          .ajax(
                              "GET",
                              "/commission/calculation/selectDataCMM026T",
                              $("#form_26")
                                  .serialize(),
                              function(result) {
                                console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_26T.do",
        GridCommon.getEditData(myGridID_26T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    ...
    myGridID_26T = AUIGrid
        .create("#grid_wrap_26T", columnLayout3, gridPros);
  }
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_26T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0026T",
            $("#form_26").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
  ...
  function fn_AlldownFile() {
    var data = {
      "searchDt" : $("#CMM0026T_Dt").val(),
      "code" : $("#code_26T").val(),
      "codeId" : $("#orgGroup_26").val()
    };
    Common
        .ajax(
            "GET",
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_26">
   <input type="hidden" name="code" id="code_26T" value="${code}" /> <input
    type="hidden" id="fileName" name="fileName" value="Award.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_26T"><span class="search"></span>
   ...
   <table class="type1 mt10">
    ...
    <tbody>
     <tr>
      ...
      <td><input type="text" id="emplyCd_26T" name="emplyCd"
     ...
     <!-- <tr>
      ...
      <td colspan=5><select id="useYnCombo_26T" name="useYnCombo"
       style="width: 100px;">
