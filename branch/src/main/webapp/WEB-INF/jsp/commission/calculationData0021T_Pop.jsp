...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_21T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_21T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_21T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      Common
                          .ajax(
                              "GET",
                              "/commission/calculation/selectDataCMM021T",
                              $("#form_21")
                                  .serialize(),
                              function(result) {
                                console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_21T.do",
        GridCommon.getEditData(myGridID_21T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    ...
    myGridID_21T = AUIGrid
        .create("#grid_wrap_21T", columnLayout3, gridPros);
  }
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_21T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0021T",
            $("#form_21").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_21">
   <input type="hidden" name="actionType" id="actionType21T"
    value="${actionType }" /> <input type="hidden" name="code"
    id="code_21T" value="${code}" /> <input type="hidden" id="fileName"
    name="fileName" value="CancellationReturn.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_21T"><span class="search"></span>
    ...
    <tbody>
     <tr>
      ...
      <td><input type="text" title="Create start Date"
       placeholder="DD/MM/YYYY" name="searchDt" id="CMM0021T_Dt"
       class="j_date2" value="${searchDt_pop }" /></td>
      <th scope="row"><spring:message
        code='commissiom.text.excel.ordId' /></th>
      <td><input type="text" id="ordId_21T" name="ordId"
       style="width: 100px;" maxlength="10" onkeydown="onlyNumber(this)">
      </td>
      <th scope="row"><spring:message
        code='commission.text.retPersonCd' /></th>
      <td><input type="text" id="retPCd_21T" name="retPCd"
     ...
     <!-- <tr>
      ...
      <td colspan="5"><select id="useYnCombo_21T" name="useYnCombo"
       style="width: 100px;">
