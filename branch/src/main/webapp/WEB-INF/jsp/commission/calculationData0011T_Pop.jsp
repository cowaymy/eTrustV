...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_11T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_11T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_11T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      Common
                          .ajax(
                              "GET",
                              "/commission/calculation/selectDataCMM011T",
                              $("#form_11T")
                                  .serialize(),
                              function(result) {
                                console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_11T.do",
        GridCommon.getEditData(myGridID_11T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    var columnLayout3 = [
        ...
        {
          dataField : "rcordId",
          headerText : "<spring:message code='commissiom.text.excel.rcordId'/>",
          style : "my-column",
          editable : false
        },
        {
    ...
    myGridID_11T = AUIGrid
        .create("#grid_wrap_11T", columnLayout3, gridPros);
  }
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_11T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0011T",
            $("#form_11T").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
                ...
                var salesPersonCd = $("#salesPersonCd_11T")
                    .val();
  ...
  function fn_AlldownFile() {
    var data = {
      "searchDt" : $("#CMM0011T_Dt").val(),
      "code" : $("#code_11T").val(),
      "codeId" : $("#orgGroup_11").val()
    };
    Common
        .ajax(
            "GET",
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_11T">
   <input type="hidden" name="code" id="code_11T" value="${code}" /> <input
    type="hidden" id="fileName" name="fileName"
    value="rentalNoutrightSales.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_11T"><span class="search"></span> <spring:message
   ...
   <table class="type1 mt10">
    ...
    <tbody>
     <tr>
      ...
      <td><input type="text" id="salesPersonCd_11T"
     ...
     <tr>
      ...
      <td colspan='5'><input type="text" id="ordId_11T" name="ordId"
       style="width: 100px;" maxlength="10" onkeydown="onlyNumber(this)">
      </td>
      <!-- <th scope="row"><spring:message
        code='commission.text.isExclude' /></th>
      <td colspan="3"><select id="useYnCombo_11T" name="useYnCombo"
       style="width: 100px;">
