...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_09T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_09T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_09T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      Common
                          .ajax(
                              "GET",
                              "/commission/calculation/selectDataCMM009T",
                              $("#form_09T")
                                  .serialize(),
                              function(result) {
                                console
  ...
  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_09T.do",
        GridCommon.getEditData(myGridID_09T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    ...
    myGridID_09T = AUIGrid
        .create("#grid_wrap_09T", columnLayout3, gridPros);
  }
  ...
  function checkAll(isChecked) {
    ...
    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_09T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0009T",
            $("#form_09T").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
  ...
  function fn_AlldownFile() {
    var data = {
      "searchDt" : $("#CMM0009T_Dt").val(),
      "code" : $("#code_09T").val(),
      "codeId" : $("#orgGroup_09").val()
    };
    Common
        .ajax(
            "GET",
...
<div id="popup_wrap2" class="popup_wrap">
 ...
 <section class="pop_body" style="max-height: 600px;">
  ...
  <!-- title_line end -->
  <form id="form_09T">
   <input type="hidden" name="code" id="code_09T" value="${code}" /> <input
    type="hidden" id="fileName" name="fileName"
    value="HappyCallPerformance.xlsx" />
   <ul class="right_btns">
    <li><p class="btn_blue">
      <a href="#" id="search_09T"><span class="search"></span>
   ...
   <table class="type1 mt10">
    ...
    <tbody>
     <tr>
      ...
      <th scope="row"><spring:message
        code='commission.text.svcPersonCd' /></th>
      <td><input type="text" id="svcPersonCd_09T"
       name="svcPersonCd" style="width: 100px;" maxlength="10"
       onkeydown="onlyNumber(this)"> <a id="svcMemBtn" href="#"
     ...
     <tr>
      ...
      <td colspan='5'><input type="text" id="ordId_09T" name="ordId"
       style="width: 100px;" maxlength="10"></td>
      <!-- <th scope="row"><spring:message
        code='commission.text.isExclude' /></th>
      <td colspan="3"><select id="useYnCombo_09T" name="useYnCombo"
       style="width: 100px;">
