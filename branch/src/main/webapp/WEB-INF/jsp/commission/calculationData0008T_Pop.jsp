...
<script type="text/javaScript">
  ...
  $(document)
      .ready(
          function() {
            ...
            // ready 이벤트 바인딩
            AUIGrid.bind(myGridID_08T, "ready",
                function(event) {
                  gridDataLength = AUIGrid
                      .getGridData(myGridID_08T).length; // 그리드 전체 행수 보관
                });

            // cellClick event.
            AUIGrid.bind(myGridID_08T, "cellClick",
                function(event) {
                ...
                .click(
                    function() {
                      ...
                      if (svcPersonCd == ""
                          || svcPersonCd == null) {
                        Common
                            .alert("<spring:message code='sys.msg.necessary' arguments='Svc Person Id' htmlEscape='false'/>");
                        return false;
                      } else {
                        Common
                            .ajax(
                                "GET",
                                "/commission/calculation/selectDataCMM008T",
                                $("#form_08T")
                                    .serialize(),
                                function(result) {
                                  console
  ...
  function fn_loadOrderSalesman(memId, memCode) {
    $("#svcPersonCd_08T").val(memCode);
    console.log('fn_loadOrderSalesman memId:' + memId);
    console.log('fn_loadOrderSalesman memCd:' + memCode);
  }

  function fn_saveExculde() {
    Common.ajax("POST", "/commission/calculation/updatePrdData_08T.do",
        GridCommon.getEditData(myGridID_08T), function(result) {
          // 공통 메세지 영역에 메세지 표시.
  ...
  function createAUIGrid() {
    ...
    myGridID_08T = AUIGrid
        .create("#grid_wrap_08T", columnLayout3, gridPros);
  }

  // 전체 체크 설정, 전체 체크 해제 하기
  function checkAll(isChecked) {

    var rowCount = AUIGrid.getRowCount(myGridID_08T);

    if (isChecked) { // checked == true == 1
      for (var i = 0; i < rowCount; i++) {
        AUIGrid.updateRow(myGridID_08T, {
          "isExclude" : 1
        }, i);
      }
  ...
  function fn_downFile() {
    Common
        .ajax(
            "GET",
            "/commission/calculation/cntCMM0008T",
            $("#form_08T").serialize(),
            function(result) {
              var cnt = result;
              if (cnt > 0) {
                ...
                if (svcPersonCd == "" || svcPersonCd == null) {
                  ...
                  var svcPersonCd = $("#svcPersonCd_08T")
                      .val();
  ...
  function fn_AlldownFile() {
    var data = {
      "searchDt" : $("#CMM0008T_Dt").val(),
      "code" : $("#code_08T").val(),
      "codeId" : $("#orgGroup_08").val()
    };
    Common
        .ajax(
            "GET",
    ...
    <section class="pop_body" style="max-height:600px;"><!-- pop_body start -->
       <aside class="title_line"><!-- title_line start -->
        ...
        <form id="form_08T">
           ...
           <input type="hidden" id="fileName" name="fileName" value="HSresult.xlsx"/>
           <ul class="right_btns">
            ...
            <table class="type1 mt10"><!-- table start -->
                ...
                <tbody>
                    <tr>
                        ...
                        <td><select id="orgGroup_08" name="codeId" style="width: 100px;">
                                <c:forEach var="list" items="${orgGrList }">
                                    <option value="${list.cdid}">${list.cd}</option>
                                </c:forEach>
                        </select></td>
                        <th scope="row"><spring:message code='commission.text.svcPersonCd'/><span class="must">*</span></th>
                        <td>
