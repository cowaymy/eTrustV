...
var attendeeReColumnLayout = [ {
    ...
    renderer : {
        ...
        list : [{"coursMemShirtSize":"S","name":"S"},
                {"coursMemShirtSize":"M","name":"M"},
                {"coursMemShirtSize":"L","name":"L"},
                {"coursMemShirtSize":"XL","name":"XL"},
                {"coursMemShirtSize":"2XL","name":"2XL"},
                {"coursMemShirtSize":"3XL","name":"3XL"},
                {"coursMemShirtSize":"4XL","name":"4XL"},
                {"coursMemShirtSize":"5XL","name":"5XL"},
                {"coursMemShirtSize":"6XL","name":"6XL"},
    ...
    renderer : {
        type : "DropDownListRenderer",
        list : [{"coursTestResult":"P","name":"Pass"},
                {"coursTestResult":"F","name":"Fail"},
                {"coursTestResult":"AB","name":"Absent"}
                ], //key-value Object 로 구성된 리스트
...
var footerObject = [ {
    ...
    positionField : "coursMemShirtSize"
}, {
    dataField : "",
...
function setAUIGrid(jsonData) {
    ...
    if(typeof firstRow == "undefined") {
        Common.alert("* Unable convert file. Please try again.");
        $("#fileSelector").val("");
        return;
    }
    ...
    $.each(firstRow, function(key , value) {
        ...
        $.each(value, function(k, v) {
            ...
            if(k.trim() == "SHIRT_SIZE"){  //Template
                keyValue.attendeeShirtSize = v;
            }
        });
    ...
    if(gridArray == null || gridArray.length <= 0){
        Common.alert("* Template has been tempered. Please try again. ");
        $("#fileSelector").val("");
        return;
    }
...
function fn_setGridDataByUploadData(array) {
    ...
    if(existArray.length > 0) {
        ...
        for (var idx = 0; idx < existArray.length; idx++) {
            ...
            if(attendDay < 0 ){
                Common.alert("Can not key-in negative day");
                return false;
            }
            ...
            AUIGrid.setCellValue(attendeeReGridID, rows[0], "coursMemShirtSize", existArray[idx].attendeeShirtSize);
        }
    }
}
...
<div id="popup_wrap" class="popup_wrap">
    ...
    <section class="pop_body">
        ...
        <!-- grid_wrap start -->
        <article class="grid_wrap" id="courseRe_grid_wrap">
        </article>
        <!-- grid_wrap end -->
        ...
        <!-- grid_wrap start -->
        <article class="grid_wrap" id="attendeeRe_grid_wrap">
        </article>
        <!-- grid_wrap end -->
