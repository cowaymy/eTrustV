...
$("#dpJoinedDateTo").attr("readonly", "readonly");
	if("${SESSION_INFO.userTypeId}" == "1"  || "${SESSION_INFO.userTypeId}" == "2" ){
        ...
        $("#hiddenLvl").val("${memLvl}");
        $("#hiddenLvl").attr("class", "w100p readonly");
        $("#hiddenLvl").attr("readonly", "readonly");
	}
...
function btnGeneratePDF_Click(){
    if(validRequiredField() == true){
        ...
        if(!($("#memCode").val() == null || $("#memCode").val().length == 0)){
        	...
        	if(($("#hiddenLvl").val() == "1")){
                whereSQL += " AND EE.MEM_CODE = '"+$("#memCode").val()+"'";
        	}

            if(($("#hiddenLvl").val() == "2")){
                whereSQL += " AND DD.MEM_CODE = '"+$("#memCode").val()+"'";
            }

            if(($("#hiddenLvl").val() == "3")){
                whereSQL += " AND CC.MEM_CODE = '"+$("#memCode").val()+"'";
            }

            if(($("#hiddenLvl").val() == "4")){
                whereSQL += " AND A.MEM_CODE = '"+$("#memCode").val()+"'";
            }
        ...
        if(!($("#hiddenLvl").val() == null || $("#hiddenLvl").val().length == 0)){
            memLvl = $("#hiddenLvl").val();
        ...
        $("#searchForm #reportFileName").val("/organization/OrganizationListing_PDF.rpt");
        $("#reportDownFileName").val("OrganizationListing"+date+(new Date().getMonth()+1)+new Date().getFullYear()+"_"+memCode);
        orderBySQL += " ORDER BY EE.MEM_CODE, DD.MEM_CODE, CC.MEM_CODE, A.MEM_CODE ";
        ...
        $("#searchForm #V_MEMLVL").val(memLvl);
...
function btnGenerateExcel_Click(){
    if(validRequiredField() == true){
        ...
        $("#searchForm #reportFileName").val("/organization/OrganizationListing_Excel.rpt");
        $("#reportDownFileName").val("OrganizationListing"+date+(new Date().getMonth()+1)+new Date().getFullYear()+"_"+memCode);
        orderBySQL += " ORDER BY EE.MEM_CODE, DD.MEM_CODE, CC.MEM_CODE, A.MEM_CODE ";
...
<tr>
    ...
    <input id="hiddenName" name="hiddenName"  type="hidden"  title="" placeholder="Name" class="" />
    <input id="hiddenLvl" name="hiddenLvl"  type="hidden"  title="" placeholder="Lvl" class="" /></td>
...
<input type="hidden" id="V_MEMLVL" name="V_MEMLVL" value="" />
