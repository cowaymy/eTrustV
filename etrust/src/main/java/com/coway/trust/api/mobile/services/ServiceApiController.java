...
public class ServiceApiController {
	...
	@ApiOperation(value = "HeartServiceResult 등록", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	@RequestMapping(value = "/HeartServiceResult", method = RequestMethod.GET)
	public ResponseEntity<List<HeartServiceResultDto>> heartServiceResult(
			@ModelAttribute HeartServiceResultForm heartServiceResultForm) throws Exception {

		Map<String, Object> params = HeartServiceResultForm.createMap(heartServiceResultForm);

		List<EgovMap> heartServiceResult = MSvcLogApiService.heartServiceResult(params);
		
		for (int i = 0; i < heartServiceResult.size(); i++) {
			LOGGER.debug("heartServiceResult    값 : {}", heartServiceResult.get(i));
		...
		List<HeartServiceResultDto> list = heartServiceResult.stream().map(r -> HeartServiceResultDto.create(r))
				.collect(Collectors.toList());
